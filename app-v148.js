...
(function(){
  ...
  window.v148OpenItemsPdf=openItemsPdf;
  ...
  function faxBoxPage(){
    ...
    const cards=[...groups.entries()].map(([g,items],gi)=>{
      ...
      const rows=sorted.map(it=>`<tr data-fax-row="${esc(it.key)}">
        <td class="v148-check"><input type="checkbox" class="v148-fax-check" data-key="${esc(it.key)}"></td>
      ...
      return `<section class="card v148-fax-card" style="margin-top:12px;padding:14px" data-fax-card="${gi}"><div class="row" style="align-items:flex-start"><div><h3 style="margin:0 0 3px">${esc(d.name||'送信先未設定')}</h3></div><span class="pill">${items.length}件</span></div>
        <div class="tablewrap" style="margin-top:10px;overflow:auto"><table class="v148-compact-table"><colgroup><col class="c0"><col class="c1"><col class="c2"><col class="c3"><col class="c4"><col class="c5"><col class="c6"><col class="c7"></colgroup><thead><tr><th><input type="checkbox" class="v148-fax-all" aria-label="全選択"></th><th>依頼日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>削除</th><th>番号</th></tr></thead><tbody>${rows}</tbody></table></div>
    ...
    app.querySelectorAll('.v148-fax-all').forEach(all=>all.onchange=()=>{const card=all.closest('.v148-fax-card');card.querySelectorAll('.v148-fax-check').forEach(c=>c.checked=all.checked)});
    app.querySelectorAll('[data-fax-checked]').forEach(b=>b.onclick=async()=>{
      const card=b.closest('.v148-fax-card'),keys=[...card.querySelectorAll('.v148-fax-check:checked')].map(c=>c.dataset.key),map=new Map(box.map(x=>[x.key,x])),items=keys.map(k=>map.get(k)).filter(Boolean);
...
(function(){
  ...
  function shipmentHistory(){
    ...
    /* v148: 履歴画面は上部タイトル文字なし。固定ナビは維持。 */
    setHeader('');setNavVisible(false);
    ...
    app.innerHTML=`<section class="card v148-history-card" style="margin-top:14px;padding:12px">
      <div class="tablewrap" style="overflow:auto">
        <table class="v148-compact-table v148-history-table">
    ...
    const selects=[...document.querySelectorAll('.v148-filter-row select[data-col]')];
    const stateFilter={};
    let sortCol='',sortDir='';
...
/* ===== v148: compact selectable FAXBOX/history tables ===== */
(function(){
 const st=document.createElement('style');st.textContent=`
 .v148-compact-table{min-width:720px;width:100%;font-size:11px;table-layout:fixed}
 .v148-compact-table th,.v148-compact-table td{padding:7px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center}
 .v148-compact-table .c0{width:30px}.v148-compact-table .c1{width:82px}.v148-compact-table .c2{width:48px}.v148-compact-table .c3{width:105px}.v148-compact-table .c4{width:105px}.v148-compact-table .c5{width:44px}.v148-compact-table .c6{width:50px}.v148-compact-table .c7{width:66px}
 .v148-compact-table input[type=checkbox]{width:18px;height:18px;margin:0;vertical-align:middle}
 `;document.head.appendChild(st);
})();
/* ===== /v148 ===== */


/* ===== v148: shipment history filters ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 body:has(.v148-history-card) header h1:empty{display:none}
 body:has(.v148-history-card) header:has(h1:empty){min-height:18px;padding-top:0;padding-bottom:0}
 .v148-history-card{margin-bottom:calc(120px + env(safe-area-inset-bottom))}
 .v148-history-table{min-width:680px}
 .v148-history-table th,.v148-history-table td{padding:6px 3px;font-size:10.5px}
 .v148-history-table col.c1{width:76px}
 .v148-history-table col.c2{width:44px}
 .v148-history-table col.c3{width:92px}
 .v148-history-table col.c4{width:92px}
 .v148-history-table col.c5{width:40px}
 .v148-history-table col.c6{width:44px}
 .v148-history-table col.c7{width:58px}
 .v148-filter-row th{padding:3px 2px;background:#f4f7fb}
 .v148-filter-row select{width:100%;min-width:0;height:28px;padding:1px 14px 1px 3px;font-size:9.5px;border:1px solid #cbd5e1;border-radius:5px;background:#fff;color:#17324f}
 .v148-filter-row select:disabled{opacity:.45}
 `;
 document.head.appendChild(st);
})();
/* ===== /v148 ===== */
