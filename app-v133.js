...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    window.__v58ShipmentCoopLower=true;window.__v63ShipmentHeaderLarge=true;window.__v133SanmaeShipmentTall=true;window.__v133SanmaeShipmentMerged=true;try{return v55RetitleStockCanvas(smReportCanvas(y,null),'釧路産棹前昆布　出 荷 依 頼 書',y,s,70,34);}finally{window.__v58ShipmentCoopLower=false;window.__v63ShipmentHeaderLarge=false;window.__v133SanmaeShipmentTall=false;window.__v133SanmaeShipmentMerged=false;}
...
async function v130TopBackup(){
  try{
    ...
    const name='昆布在庫出荷管理_'+v133BackupTimestamp()+'.json';
...
/* ===== v133: 自動保存 + 入力途中下書き復元 + 日時入りバックアップ名 ===== */
(function(){
  const SNAP_KEY='kombu-v133-autosave-snapshot';
  const DRAFT_KEY='kombu-v133-form-drafts';
  let saveTimer=0;

  window.v133BackupTimestamp=function(){
  ...
  function safeAllSave(){
    ...
    }catch(e){console.warn('v133 autosave snapshot',e)}
  }
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v133';
    return r;
  };
})();
/* ===== /v133 ===== */
...
/* ===== v133: shipment list bulk-to-FAXBOX + shipment archive ===== */
(function(){
  const FAX_KEY='kombu-v99-fax-box', HIST_KEY='kombu-v133-shipment-history';
  const clone=o=>JSON.parse(JSON.stringify(o));
  ...
  function shipmentHistory(){
    ...
    app.innerHTML=`<section class="card" style="margin-top:14px;padding:14px">
      ...
      <tbody id="v133HistBody">${rows}</tbody></table></div></section>`;
    const body=document.getElementById('v133HistBody');if(body)body.onclick=e=>{const tr=e.target.closest('[data-hid]');if(tr&&e.target.closest('[data-hopen]'))openGlobalShipment(tr.dataset.hprod,tr.dataset.hid)};
    document.body.dataset.v119NavMode='shipment';
    if(typeof window.v133EnsureHistoryNav==='function')window.v133EnsureHistoryNav();
  }
  window.v133ShipmentHistory=shipmentHistory;
  ...
  globalThis.v76ShipmentMenu=function(){
    ...
    if(head && !document.getElementById('v133BulkFax')){
      const right=head.querySelector('.muted');
      const b=document.createElement('button');b.id='v133BulkFax';b.className='mini';
  ...
  function upgradeNav(){
    const nav=document.getElementById('v119ShipmentNav');if(!nav)return;
    if(!document.getElementById('v133History')){
      const b=document.createElement('button');b.id='v133History';b.setAttribute('aria-label','出荷指示履歴');b.title='出荷指示履歴';
  ...
  window.v133EnsureHistoryNav=upgradeNav;
