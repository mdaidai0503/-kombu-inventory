...
function _stockCanvasPage(year){
 ...
 const shHead=!!window.__v63ShipmentHeaderLarge,shCoopMerge=!!window.__v134KushiroShipmentCoopMerged; text('組合名',tableX+coopW/2,tableY+(h1+h2)/2,shHead?18:14,'center',true);text('区分',xCoop+seasonW/2,tableY+(h1+h2)/2,shHead?18:14,'center',true);text('計',xData+dataW+totalW/2,tableY+(h1+h2)/2,shHead?18:14,'center',true);
...
function smReportCanvas(y,ship=null){const W=1684,H=1191,margin=34,c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d'),cols=smItems(),title=ship?'出 荷 指 示 書（釧路産棹前昆布）':'釧 路 産 棹 前 昆 布　在 庫 集 計 表';x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle='#000';x.strokeStyle='#222';const f=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif`,t=(v,xx,yy,z=13,a='center',b=false)=>{x.font=f(z,b);x.textAlign=a;x.textBaseline='middle';x.fillText(String(v??''),xx,yy)};t(title,margin,38,28,'left',true);t(`${y}年産`,W-margin,38,16,'right',true);if(ship){t(`指示番号：${ship.id}　出荷日：${ship.shipDate||''}`,W-margin,62,13,'right');t(`出荷先：${ship.dest?.name||''}　${ship.dest?.address||''}　TEL ${ship.dest?.phone||''}`,margin,62,12,'left')}const ty=ship?85:70,tw=W-margin*2,coopW=92,seasonW=45,totalW=58,dataW=tw-coopW-seasonW-totalW,colW=dataW/cols.length,shipmentTall=!!window.__v134SanmaeShipmentTall,shipmentMerged=!!window.__v134SanmaeShipmentMerged,h1=shipmentTall?32:28,h2=shipmentTall?32:28,rowH=shipmentTall?60:39,footH=shipmentTall?40:34,tableH=h1+h2+S_COOPS.length*(S_SEASONS.length+1)*rowH+footH;x.lineWidth=.55;x.strokeRect(margin,ty,tw,tableH);const xData=margin+coopW+seasonW;[margin+coopW,xData,xData+dataW].forEach(xx=>{x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+tableH);x.stroke()});const shHead=!!window.__v63ShipmentHeaderLarge;t('組合名',margin+coopW/2,ty+(h1+h2)/2,shHead?16:12);t('区分',margin+coopW+seasonW/2,ty+(h1+h2)/2,shHead?16:12);t('計',xData+dataW+totalW/2,ty+(h1+h2)/2,shHead?16:12);let ci=0;S_GROUPS.forEach(g=>{const gx=xData+ci*colW,gw=g.items.length*colW;x.strokeRect(gx,ty,gw,h1);t(g.name,gx+gw/2,ty+h1/2,shHead?14:10,'center',true);g.items.forEach((it,j)=>{const xx=gx+j*colW;x.strokeRect(xx,ty+h1,colW,h2);t(it,xx+colW/2,ty+h1+h2/2,shHead?12:8)});ci+=g.items.length});const mt=smMatrix(),lines=ship?.lines||null;const qFor=(coop,se,c)=>lines?lines.filter(l=>l.year===y&&l.coop===coop&&l.season===se&&l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0):(mt[[y,coop,se,c.group,c.item].join('|')]||0);let yy=ty+h1+h2;for(const coop of S_COOPS){x.lineWidth=1.5;x.beginPath();x.moveTo(margin,yy);x.lineTo(margin+tw,yy);x.stroke();x.lineWidth=.55;for(let si=0;si<S_SEASONS.length;si++){const se=S_SEASONS[si];if(si===0)t(coop,margin+coopW/2,yy+(shipmentMerged?rowH*2:(window.__v58ShipmentCoopLower?rowH*1.7:rowH/2)),shipmentMerged?18:12,'center',true);t(se,margin+coopW+seasonW/2,yy+rowH/2,13,'center',true);let rt=0;cols.forEach((cc,j)=>{const q=qFor(coop,se,cc),xx=xData+j*colW;rt+=q;x.beginPath();x.moveTo(xx,yy);x.lineTo(xx,yy+rowH);x.stroke();if(q)t(fmt(q),xx+colW/2,yy+rowH/2,shipmentMerged?18:13)});if(rt)t(fmt(rt),xData+dataW+totalW/2,yy+rowH/2,shipmentMerged?18:13);x.beginPath();x.moveTo(shipmentMerged?margin+coopW:margin,yy+rowH);x.lineTo(margin+tw,yy+rowH);x.stroke();yy+=rowH}t('小計',margin+coopW+seasonW/2,yy+rowH/2,11);let st=0;cols.forEach((cc,j)=>{const q=S_SEASONS.reduce((a,se)=>a+qFor(coop,se,cc),0),xx=xData+j*colW;st+=q;if(q)t(fmt(q),xx+colW/2,yy+rowH/2,11)});if(st)t(fmt(st),xData+dataW+totalW/2,yy+rowH/2,11);x.beginPath();x.moveTo(margin,yy+rowH);x.lineTo(margin+tw,yy+rowH);x.stroke();yy+=rowH}x.lineWidth=1.5;x.beginPath();x.moveTo(margin,yy);x.lineTo(margin+tw,yy);x.stroke();x.lineWidth=.55;t('合計',margin+(coopW+seasonW)/2,yy+footH/2,12,'center',true);let gt=0;cols.forEach((cc,j)=>{const q=S_COOPS.reduce((a,co)=>a+S_SEASONS.reduce((b,se)=>b+qFor(co,se,cc),0),0),xx=xData+j*colW;gt+=q;if(q)t(fmt(q),xx+colW/2,yy+footH/2,shHead?18:12)});if(gt)t(fmt(gt),xData+dataW+totalW/2,yy+footH/2,shHead?18:12);return c}
...
function v55CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    window.__v58ShipmentCoopLower=true;window.__v63ShipmentHeaderLarge=true;window.__v134KushiroShipmentCoopMerged=true;try{return v55RetitleStockCanvas(_stockCanvasPage(y),'釧路産昆布　出 荷 依 頼 書',y,s,112,44);}finally{window.__v58ShipmentCoopLower=false;window.__v63ShipmentHeaderLarge=false;window.__v134KushiroShipmentCoopMerged=false;}
...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    window.__v58ShipmentCoopLower=true;window.__v63ShipmentHeaderLarge=true;window.__v134SanmaeShipmentTall=true;window.__v134SanmaeShipmentMerged=true;try{return v55RetitleStockCanvas(smReportCanvas(y,null),'釧路産棹前昆布　出 荷 依 頼 書',y,s,70,34);}finally{window.__v58ShipmentCoopLower=false;window.__v63ShipmentHeaderLarge=false;window.__v134SanmaeShipmentTall=false;window.__v134SanmaeShipmentMerged=false;}
...
(function(){
  ...
  function ensureShipmentNav(){
    ...
    if(!nav){
      ...
      nav.querySelector('#v124Draft').onclick=()=>{const b=document.getElementById('v114Save')||document.getElementById('saveDraft');if(b)b.click()};
    ...
    const draft=nav.querySelector('#v124Draft');if(draft)draft.style.display=screenKind==='form'?'block':'none';
...
async function v130TopBackup(){
  try{
    ...
    const name='昆布在庫出荷管理_'+v134BackupTimestamp()+'.json';
...
/* ===== v134: 自動保存 + 入力途中下書き復元 + 日時入りバックアップ名 ===== */
(function(){
  const SNAP_KEY='kombu-v134-autosave-snapshot';
  const DRAFT_KEY='kombu-v134-form-drafts';
  let saveTimer=0;

  window.v134BackupTimestamp=function(){
  ...
  function safeAllSave(){
    ...
    }catch(e){console.warn('v134 autosave snapshot',e)}
  }
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v134';
    return r;
  };
})();
/* ===== /v134 ===== */
...
/* ===== v134: shipment list bulk-to-FAXBOX + shipment archive ===== */
(function(){
  const FAX_KEY='kombu-v99-fax-box', HIST_KEY='kombu-v134-shipment-history';
  const clone=o=>JSON.parse(JSON.stringify(o));
  ...
  function bulkVisibleToFax(){
    ...
    const box=load(FAX_KEY), hist=load(HIST_KEY), histMap=new Map(hist.map(x=>[x.key,x]));
    let n=0;
    trs.forEach(tr=>{
      ...
      histMap.set(k,{...item,archivedAt:new Date().toISOString()}); n++;
    });
    save(FAX_KEY,box); save(HIST_KEY,[...histMap.values()]);
    alert(n+'件をFAX BOXへ送り、出荷指示履歴へ移動しました。');
    v76ShipmentMenu();
  }
  function shipmentHistory(){
    ...
    setHeader('出荷指示履歴');setNavVisible(false);
    const rows=hist.map(it=>`<tr data-hprod="${it.product}" data-hid="${esc(it.id||'')}">
      <td>${esc(it.shipDate||'')}</td><td><b>${esc(label(it.product))}</b></td>
      <td>${esc(it.source?.name||'')}</td><td>${esc(it.dest?.name||'')}</td>
      <td style="text-align:center">${fmt(it.qty||0)}</td><td><button class="mini" data-hopen="1">開く</button></td>
    </tr>`).join('')||'<tr><td colspan="6" class="empty">出荷指示履歴はありません</td></tr>';
    app.innerHTML=`<section class="card" style="margin-top:14px;padding:14px">
      <div class="row"><h2 style="margin:0">出荷指示履歴</h2><span class="muted" style="font-size:12px">FAX BOXへ送信済</span></div>
      <div class="tablewrap" style="margin-top:10px;overflow:auto"><table style="min-width:620px;font-size:12px">
      <thead><tr><th>出荷日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>開く</th></tr></thead>
      <tbody id="v134HistBody">${rows}</tbody></table></div></section>`;
    const body=document.getElementById('v134HistBody');if(body)body.onclick=e=>{const tr=e.target.closest('[data-hid]');if(tr&&e.target.closest('[data-hopen]'))openGlobalShipment(tr.dataset.hprod,tr.dataset.hid)};
    document.body.dataset.v119NavMode='shipment';
    if(typeof window.v134EnsureHistoryNav==='function')window.v134EnsureHistoryNav();
  }
  window.v134ShipmentHistory=shipmentHistory;

  const baseMenu=globalThis.v76ShipmentMenu;
  globalThis.v76ShipmentMenu=function(){
    const r=baseMenu.apply(this,arguments);
    const archived=new Set(load(HIST_KEY).map(x=>x.key));
    document.querySelectorAll('#v76ShipBody tr[data-gprod][data-gid]').forEach(tr=>{if(archived.has(key(tr.dataset.gprod,tr.dataset.gid)))tr.remove()});
    ...
    if(head && !document.getElementById('v134BulkFax')){
      const right=head.querySelector('.muted');
      const b=document.createElement('button');b.id='v134BulkFax';b.className='mini';
  ...
  function upgradeNav(){
    const nav=document.getElementById('v119ShipmentNav');if(!nav)return;
    if(!document.getElementById('v134History')){
      const b=document.createElement('button');b.id='v134History';b.setAttribute('aria-label','出荷指示履歴');b.title='出荷指示履歴';
  ...
  window.v134EnsureHistoryNav=upgradeNav;
