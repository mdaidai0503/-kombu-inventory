...
async function v130TopBackup(){
  try{
    ...
    const name='昆布在庫出荷管理_'+v131BackupTimestamp()+'.json';
...
/* ===== v131: 自動保存 + 入力途中下書き復元 + 日時入りバックアップ名 ===== */
(function(){
  const SNAP_KEY='kombu-v131-autosave-snapshot';
  const DRAFT_KEY='kombu-v131-form-drafts';
  let saveTimer=0;

  window.v131BackupTimestamp=function(){
  ...
  function safeAllSave(){
    ...
    }catch(e){console.warn('v131 autosave snapshot',e)}
  }
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v131';
    return r;
  };
})();
/* ===== /v131 ===== */
