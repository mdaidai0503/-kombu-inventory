...
async function parseInventoryPdf(file){
  ...
  PDFJS.GlobalWorkerOptions.workerSrc='./pdf-worker-v57.js';
...
async function v42BulkPdfImport(file){
  ...
  app.innerHTML=`<section class="card" style="margin-top:22px"><div class="row"><h2>📄 4種類 PDF一括入庫</h2><span class="pill">v55</span></div><p><b>${esc(file.name)}</b></p><div class="note">PDFを1回読み込み、釧路産昆布・日高昆布・根室産昆布・釧路産棹前昆布を自動判別しています。</div><p id="v42Progress" style="margin-top:16px;font-weight:700">PDFを準備しています…</p></section>`;
...
productChoicePage=function(mode){
  ...
  const pill=app.querySelector('.pill');if(pill)pill.textContent='v55';
  if(mode==='inventory'){
    const card=app.querySelector('.card'),back=document.getElementById('v38Back');
...
productLanding=function(){_v41LandingForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v55'};
const _v41CompanyForV42=companyMasterPage;
companyMasterPage=function(){_v41CompanyForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v55'};
...
function v55ShipmentPrintPreview(opts){
  ...
  <div id="v55ShipPrint">
    ...
    <div class="v55hint"><b>帳票プレビュー</b> — 下に出荷指示書が表示されていれば正常です。「PDF・印刷・FAXへ」からiPhoneの印刷・共有機能を使えます。</div>
...
*/
function v55RetitleStockCanvas(canvas, title, year, shipment, topClear){
...
function v55CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    return v55RetitleStockCanvas(_stockCanvasPage(y),'釧 路 産 昆 布　出 荷 指 示 書',y,s,100);
...
function v55CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    return v55RetitleStockCanvas(hStockCanvas(y),'日 高 昆 布　出 荷 指 示 書',y,s,74);
...
function v55CanvasNemuro(s,y){
  ...
  try{
    ...
    nState.shipments=[];
    return v55RetitleStockCanvas(nReportCanvas(y,null),'根 室 産 昆 布　出 荷 指 示 書',y,s,64);
...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    return v55RetitleStockCanvas(smReportCanvas(y,null),'釧 路 産 棹 前 昆 布　出 荷 指 示 書',y,s,64);
  ...
  app.innerHTML=`
   <style>
    ...
    @media print{
      ...
      main{padding:0!important;max-width:none!important}
      .v55preview{overflow:visible;background:#fff;padding:0}
      .v55page{width:297mm;height:210mm;margin:0;box-shadow:none;page-break-after:always;display:flex;align-items:flex-start;justify-content:center}
      .v55page:last-child{page-break-after:auto}
      .v55page canvas{width:297mm;height:210mm;object-fit:contain}
    }
   </style>
...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, topClear){
  ...
  const font=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans","Noto Sans JP",sans-serif`;
  const text=(t,xx,yy,z=18,a='left',b=false)=>{
    x.fillStyle='#000';x.font=font(z,b);x.textAlign=a;x.textBaseline='middle';
    x.fillText(String(t??''),xx,yy);
  };

  // Top title.
  text(title,44,30,27,'left',true);
  text(`${year}年産`,W-44,28,17,'right',true);
  text(`指示番号：${shipment?.id||''}　出荷日：${shipment?.shipDate||''}`,W-44,50,14,'right',false);

  // Large bordered recipient/sender blocks.
  const margin=44,gap=14,boxY=68,boxH=148,boxW=(W-margin*2-gap)/2;
  x.strokeStyle='#111';x.lineWidth=2.2;
  x.strokeRect(margin,boxY,boxW,boxH);
  x.strokeRect(margin+boxW+gap,boxY,boxW,boxH);

  const drawParty=(left,label,p)=>{
    text(label,left+18,boxY+34,45,'left',true);
    text(p.name||'',left+18,boxY+78,26,'left',true);
    text(`住所：${p.address||''}`,left+18,boxY+108,19,'left',false);
    text(`電話：${p.phone||''}`,left+18,boxY+134,19,'left',false);
  };
  drawParty(margin,'出荷先',dest);
  drawParty(margin+boxW+gap,'出荷元',src);

  // Keep the exact stock-table drawing, but remove its old title area and fit it below the new blocks.
  const srcY=Math.max(70,Number(topClear)||70);
  const destY=230;
  const srcH=H-srcY;
  const destH=H-destY-8;
  const scale=Math.min((W-8)/W,destH/srcH);
  const dw=W*scale,dh=srcH*scale,dx=(W-dw)/2;
  x.drawImage(sourceCanvas,0,srcY,W,srcH,dx,destY,dw,dh);
