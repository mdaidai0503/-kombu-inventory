...
(function(){
  ...
  function setMode(next,kind){
    ...
    if(mode===nm && screenKind===nk){
      if(mode==='shipment'){const fax=document.querySelector('#v119Fax');if(fax)fax.textContent='📥 ('+faxCount()+')'}
      return;
    }
    mode=nm;
  ...
  window.addEventListener('storage',()=>{if(mode==='shipment'){const fax=document.querySelector('#v119Fax');if(fax)fax.textContent='📥 ('+faxCount()+')'}});

  // 現在表示が在庫画面なら初期状態も6ボタンへ。
  standardNav();
  const oldLanding=globalThis.productLanding;
  if(typeof oldLanding==='function')globalThis.productLanding=function(){setMode('');const r=oldLanding.apply(this,arguments);const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v121';return r};
})();
/* ===== /v119 ===== */
...
(function(){
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    ...
    const r=oldLanding.apply(this,arguments);
    requestAnimationFrame(()=>{
      const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v121';
      renderTopNav();
    });
    return r;
  };
