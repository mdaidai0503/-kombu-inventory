...
(function () {
  ...
  // Supabase通信量削減: 通常画面再描画では5分間キャッシュを再利用する。
  // 明示更新・送り状確認画面を開く時だけ force=true で最新取得する。
  ...
  async function loadWaybills(force) {
    const now = Date.now();
    const shouldForce = force === true;

    if (
      !shouldForce &&
      waybillCache.length > 0 &&
      (now - lastLoadedAt) < WAYBILL_CACHE_TTL_MS
    ) {
      return waybillCache;
    }
  ...
  async function refreshWaybills(force) {
    if (refreshing) return;
    refreshing = true;

    try {
      await loadWaybills(force === true);
      patchHistoryTable();
      patchReviewButton();
    } finally {
      refreshing = false;
  ...
  const observer = new MutationObserver(function (mutations) {
    ...
    if (relevant) {
      // 履歴画面の再描画ではSupabaseへ取り直さず、
      // 既存キャッシュからPDF表示だけを再適用する。
      patchHistoryTable();
      patchReviewButton();
    }
  });
  ...
  window.KOMBU_WAYBILL_UI_VERSION = '160.8-egress-fix';

  // 外部からの明示更新は最新データを取得する。
  window.kombuWaybillInboxRefresh = function () {
    return refreshWaybills(true);
  };
