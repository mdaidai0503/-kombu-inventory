...
function bindNav(){
 if(shipNavBtnEl)shipNavBtnEl.onclick=()=>currentProduct==='hidaka'?hShipments():shipments();
 if(stockNavBtnEl)stockNavBtnEl.onclick=()=>currentProduct==='hidaka'?hStock():stock();
 if(logsNavBtnEl)logsNavBtnEl.onclick=()=>currentProduct==='hidaka'?hLogs():logs();
 if(inNavBtnEl)inNavBtnEl.onclick=()=>currentProduct==='hidaka'?hForm('in'):form('in');
 if(moreBtnEl)moreBtnEl.onclick=()=>currentProduct==='hidaka'?hMore():exportsPage();
}
function productLanding(){currentProduct=null;setHeader('昆布在庫管理');setNavVisible(false);app.innerHTML=`<section class="card" style="margin-top:22px"><h2>管理する昆布を選択</h2><p class="muted">釧路産昆布と日高昆布は、在庫・入出庫履歴・出荷指示を別々に管理します。</p><div class="grid" style="margin-top:16px"><button class="action orange" id="chooseK"><b style="font-size:20px">釧路産昆布</b><small>現在の在庫管理・PDF入庫・出荷指示</small></button><button class="action green" id="chooseH"><b style="font-size:20px">日高昆布</b><small>日高昆布専用の在庫管理・PDF入庫・出荷指示</small></button></div></section>`;document.getElementById('chooseK').onclick=()=>{currentProduct='kushiro';setHeader('釧路産昆布 在庫管理');setNavVisible(true);bindNav();home()};document.getElementById('chooseH').onclick=()=>{currentProduct='hidaka';setHeader('日高昆布 在庫管理');setNavVisible(true);bindNav();hHome()}}
