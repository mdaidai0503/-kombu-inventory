...
function _stockCanvasPage(year){
 ...
 let y=tableY+h1+h2;state.coops.forEach(coop=>{
  SEASONS.forEach((season,si)=>{ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();if(si===0)fit(coop,tableX,y+rowH/2,coopW,13,true);fit(season,xCoop,y+rowH/2,seasonW,14,true);let rt=0;cols.forEach((c,j)=>{const q=m[[year,coop,c.group,c.item,season].join('|')]||0;rt+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,12)});if(rt)fit(fmt(rt),xData+dataW,y+rowH/2,totalW,12,true);y+=rowH});
  ctx.fillStyle='#f6f8fb';ctx.fillRect(tableX,y,tableW,rowH);ctx.fillStyle='#000';ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();fit('小計',xCoop,y+rowH/2,seasonW,13,true);let ct=0;cols.forEach((c,j)=>{const q=SEASONS.reduce((a,se)=>a+(m[[year,coop,c.group,c.item,se].join('|')]||0),0);ct+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,12,true)});if(ct)fit(fmt(ct),xData+dataW,y+rowH/2,totalW,12,true);y+=rowH;
 });
 ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();fit('合計',tableX,y+footH/2,coopW+seasonW,14,true);cols.forEach((c,j)=>{const q=state.coops.reduce((a,coop)=>a+SEASONS.reduce((b,se)=>b+(m[[year,coop,c.group,c.item,se].join('|')]||0),0),0);const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+footH);ctx.stroke();if(q)fit(fmt(q),xx,y+footH/2,itemW,12,true)});const grand=total(year);if(grand)fit(fmt(grand),xData+dataW,y+footH/2,totalW,13,true);
...
function _shipmentCanvasPage(s,year){
 ...
 state.coops.forEach(coop=>{
  SEASONS.forEach((season,si)=>{ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();if(first){fit(year+'年産',tableX,y+rowH/2,yearW,13,true);first=false}if(si===0)fit(coop,xYear,y+rowH/2,coopW,13,true);fit(season,xCoop,y+rowH/2,seasonW,14,true);let rt=0;cols.forEach((c,j)=>{const q=lines.filter(l=>l.coop===coop&&l.season===season&&l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0);rt+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,12)});if(rt)fit(fmt(rt),xData+dataW,y+rowH/2,totalW,12,true);y+=rowH});
  ctx.fillStyle='#f6f8fb';ctx.fillRect(xYear,y,tableW-yearW,rowH);ctx.fillStyle='#000';ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();fit('小計',xCoop,y+rowH/2,seasonW,13,true);let ct=0;cols.forEach((c,j)=>{const q=lines.filter(l=>l.coop===coop&&l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0);ct+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,12,true)});if(ct)fit(fmt(ct),xData+dataW,y+rowH/2,totalW,12,true);y+=rowH;
 });
 ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();fit('合計',tableX,y+footH/2,yearW+coopW+seasonW,14,true);cols.forEach((c,j)=>{const q=lines.filter(l=>l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0);const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+footH);ctx.stroke();if(q)fit(fmt(q),xx,y+footH/2,itemW,12,true)});if(totalAll)fit(fmt(totalAll),xData+dataW,y+footH/2,totalW,13,true);y+=footH;
...
/* ナビ・その他を更新 */
shipNavBtn.onclick=shipments;stockNavBtn.onclick=stock;logsNavBtn.onclick=logs;inNavBtn.onclick=()=>form('in');moreBtn.onclick=exportsPage;
home();
