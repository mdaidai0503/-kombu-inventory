...
if('serviceWorker'in navigator)window.addEventListener('load',async()=>{try{const regs=await navigator.serviceWorker.getRegistrations();for(const r of regs)await r.unregister();}catch(e){}try{const ks=await caches.keys();for(const k of ks)if(k.startsWith('kombu-')||k.startsWith('kombu-inventory-'))await caches.delete(k);}catch(e){}});
...
function hShipDetail(id){const s=hState.shipments.find(x=>x.id===id);if(!s)return hShipments();app.innerHTML=`<section class="card"><div class="row"><h2>日高昆布 出荷指示 ${s.id}</h2><span class="pill">${s.status}</span></div><p><b>出荷先：</b>${esc(s.dest?.name||'')}　<b>出荷元：</b>${esc(s.source?.name||'')}</p><p><b>出荷日：</b>${s.shipDate||''}　<b>合計：</b>${fmt((s.lines||[]).reduce((a,l)=>a+Number(l.qty||0),0))}</p><div class="toolbar"><button class="btn" id="hpdfs">PDF・FAX用</button>${s.status==='draft'?'<button class="btn" id="hconf">確定・在庫反映</button><button class="btn secondary" id="hedit">修正</button>':''}${s.status==='confirmed'?'<button class="btn" id="hshipped">出荷済</button>':''}<button class="btn secondary" id="hback">一覧へ</button></div></section>`;hpdfs.onclick=()=>hOpenShipPdf(s);if(s.status==='draft'){hconf.onclick=()=>{for(const l of s.lines)if(Number(l.qty)>hAvail(l.year,l.location,l.section,l.grade,s.id))return alert('在庫不足があります。');s.status='confirmed';hSave();hShipDetail(id)};hedit.onclick=()=>hShipForm(id)}if(s.status==='confirmed')hshipped.onclick=()=>{if(!confirm('出荷済みにしますか？'))return;s.lines.forEach(l=>hState.records.push({id:crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random()),type:'out',year:l.year,location:l.location,section:l.section,grade:l.grade,qty:Number(l.qty),date:s.shipDate||today(),memo:`出荷指示 ${s.id}`}));s.status='shipped';hSave();hShipDetail(id)};hback.onclick=hShipments}
function hShipCanvas(s,year){const c=hStockCanvas(year),x=c.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,c.width,78);x.fillStyle='#000';x.font='700 30px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif';x.textAlign='left';x.fillText('出 荷 指 示 書（日高昆布）',35,43);x.font='400 15px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif';x.textAlign='right';x.fillText(`指示番号：${s.id}`,1649,28);x.fillText(`出荷日：${s.shipDate||''}`,1649,51);x.textAlign='left';x.fillText(`出荷先：${s.dest?.name||''}　${s.dest?.address||''}　TEL ${s.dest?.phone||''}`,35,66);x.fillText(`出荷元：${s.source?.name||''}　${s.source?.address||''}　TEL ${s.source?.phone||''}`,850,66);return c}
...
function nShipDetail(id){const s=nState.shipments.find(x=>x.id===id);if(!s)return nShipments();app.innerHTML=`<section class="card"><div class="row"><h2>根室産昆布 出荷指示 ${s.id}</h2><span class="pill">${s.status}</span></div><p><b>出荷先：</b>${esc(s.dest?.name||'')}　<b>出荷元：</b>${esc(s.source?.name||'')}</p><p><b>出荷日：</b>${s.shipDate||''}　<b>合計：</b>${fmt((s.lines||[]).reduce((a,l)=>a+Number(l.qty||0),0))}</p><div class="toolbar"><button class="btn" id="npdfs">帳票表示・PDF/FAX</button>${s.status==='draft'?'<button class="btn" id="nconf">確定・在庫反映</button><button class="btn secondary" id="nedit">修正</button>':''}${s.status==='confirmed'?'<button class="btn" id="nshipped">出荷済</button>':''}<button class="btn secondary" id="nback">一覧へ</button></div></section>`;npdfs.onclick=()=>nOpenShipPdf(s);if(s.status==='draft'){nconf.onclick=()=>{for(const l of s.lines)if(Number(l.qty)>nAvail(l.year,l.coop,l.season,l.group,l.item,s.id))return alert('在庫不足があります。');s.status='confirmed';nSave();nShipDetail(id)};nedit.onclick=()=>nShipForm(id)}if(s.status==='confirmed')nshipped.onclick=()=>{if(!confirm('出荷済みにすると、明細数量を在庫から出庫します。よろしいですか？'))return;s.lines.forEach(l=>nState.records.push({id:crypto.randomUUID?crypto.randomUUID():String(Date.now()+Math.random()),type:'out',year:l.year,coop:l.coop,season:l.season,group:l.group,item:l.item,qty:Number(l.qty),date:s.shipDate||today(),memo:`出荷指示 ${s.id}`}));s.status='shipped';nSave();nShipDetail(id)};nback.onclick=nShipments}
