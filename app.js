...
function _shipmentCanvasPage(s,year){
 ...
 [[dst,'出荷先'],[src,'出荷元']].forEach(([c,label],i)=>{const x=margin+i*half;box(x,infoY,half,infoH);text(`${label}：${c.name||''}`,x+10,infoY+18,16,'left',true);text(`住所：${c.address||''}`,x+10,infoY+40,14);text(`電話：${c.phone||''}`,x+10,infoY+60,14)});
