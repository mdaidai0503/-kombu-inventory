...
async function parseInventoryPdf(file){
  ...
  PDFJS.GlobalWorkerOptions.workerSrc='./vendor/pdf.worker.js?v=47';
...
async function v42BulkPdfImport(file){
  ...
  app.innerHTML=`<section class="card" style="margin-top:22px"><div class="row"><h2>📄 4種類 PDF一括入庫</h2><span class="pill">v47</span></div><p><b>${esc(file.name)}</b></p><div class="note">PDFを1回読み込み、釧路産昆布・日高昆布・根室産昆布・釧路産棹前昆布を自動判別しています。</div><p id="v42Progress" style="margin-top:16px;font-weight:700">PDFを準備しています…</p></section>`;
...
productChoicePage=function(mode){
  ...
  const pill=app.querySelector('.pill');if(pill)pill.textContent='v47';
  if(mode==='inventory'){
    const card=app.querySelector('.card'),back=document.getElementById('v38Back');
...
productLanding=function(){_v41LandingForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v47'};
const _v41CompanyForV42=companyMasterPage;
companyMasterPage=function(){_v41CompanyForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v47'};
