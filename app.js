...
function _shipmentCanvasPage(s,year){
 ...
 ctx.fillStyle='#fff';ctx.fillRect(0,0,W,H);ctx.strokeStyle='#000';ctx.fillStyle='#000';ctx.lineWidth=2;ctx.textBaseline='middle';
 ...
 const cols=allItems();const lines=s.lines.filter(l=>(l.year||DEFAULT_YEAR)===year);const totalAll=lines.reduce((a,l)=>a+Number(l.qty||0),0);
 text('出 荷 指 示 書',margin,58,34,'left',true);text(`指示番号：${s.id}`,W-margin,46,17,'right');text(`作成日：${today()}`,W-margin,72,17,'right');ctx.beginPath();ctx.moveTo(margin,88);ctx.lineTo(W-margin,88);ctx.stroke();
 const infoY=102,infoH=48,infoW=(W-margin*2)/3;['出荷先：'+(s.dest||'')+' 御中','出荷日：'+(s.shipDate||''),'希望着日：'+(s.arrivalDate||'')].forEach((v,i)=>{box(margin+i*infoW,infoY,infoW,infoH);fit(v,margin+i*infoW,infoY+infoH/2,infoW,18,i===0)});
 box(margin,162,W-margin*2,46);text(`生産年度：${year}年産`,margin+14,185,18,'left',true);text(`区分：${[...new Set(lines.map(x=>x.season))].join('・')}`,margin+430,185,18);text(`合計：${fmt(totalAll)}`,W-margin-14,185,18,'right',true);
 const tableX=margin,tableY=222,tableW=W-margin*2;const yearW=96,coopW=112,seasonW=48,totalW=58;const dataW=tableW-yearW-coopW-seasonW-totalW,itemW=dataW/cols.length;const h1=34,h2=34,rowH=42,footH=42;
 const x0=tableX;const xYear=x0+yearW,xCoop=xYear+coopW,xSeason=xCoop+seasonW,xData=xSeason;
 ctx.fillStyle='#f1f1f1';ctx.fillRect(tableX,tableY,tableW,h1+h2);ctx.fillStyle='#000';
 // outer/header grid
 box(tableX,tableY,tableW,h1+h2+state.coops.length*SEASONS.length*rowH+footH);
 [xYear,xCoop,xSeason,xData+dataW].forEach(x=>{ctx.beginPath();ctx.moveTo(x,tableY);ctx.lineTo(x,tableY+h1+h2+state.coops.length*SEASONS.length*rowH+footH);ctx.stroke()});
 text('生産年度',tableX+yearW/2,tableY+(h1+h2)/2,14,'center',true);text('組合名',xYear+coopW/2,tableY+(h1+h2)/2,14,'center',true);text('区分',xCoop+seasonW/2,tableY+(h1+h2)/2,14,'center',true);text('計',xData+dataW+totalW/2,tableY+(h1+h2)/2,14,'center',true);
 let ci=0;GROUPS.forEach(g=>{const gx=xData+ci*itemW,gw=g.items.length*itemW;box(gx,tableY,gw,h1);fit(g.name,gx,tableY+h1/2,gw,14,true);g.items.forEach((it,j)=>{const ix=gx+j*itemW;box(ix,tableY+h1,itemW,h2);fit(it,ix,tableY+h1+h2/2,itemW,13,true)});ci+=g.items.length});
 let y=tableY+h1+h2;let first=true;state.coops.forEach(coop=>SEASONS.forEach((season,si)=>{ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();if(first){fit(year+'年産',tableX,y+rowH/2,yearW,14,true);first=false}if(si===0)fit(coop,xYear,y+rowH/2,coopW,14,true);fit(season,xCoop,y+rowH/2,seasonW,15,true);let rt=0;cols.forEach((c,j)=>{const q=lines.filter(l=>l.coop===coop&&l.season===season&&l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0);rt+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,14)});if(rt)fit(fmt(rt),xData+dataW,y+rowH/2,totalW,14,true);y+=rowH}));
 ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();fit('合計',tableX,y+footH/2,yearW+coopW+seasonW,15,true);cols.forEach((c,j)=>{const q=lines.filter(l=>l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0);const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+footH);ctx.stroke();if(q)fit(fmt(q),xx,y+footH/2,itemW,14,true)});if(totalAll)fit(fmt(totalAll),xData+dataW,y+footH/2,totalW,14,true);y+=footH;
 const noteY=y+20;box(margin,noteY,W-margin*2,54);text('備考：'+(s.memo||''),margin+12,noteY+27,16);const footY=noteY+70,fw=(W-margin*2)/3;['出荷元：㈱浜中運輸','受注・配送指示：','FAX送信欄：'].forEach((v,i)=>{box(margin+i*fw,footY,fw,54);text(v,margin+i*fw+10,footY+27,15)});
 return canvas;
}
