...
function printShipment(id){
 ...
 w.document.write(`<!doctype html><html lang="ja"><head><meta charset="utf-8"><title>出荷指示書 ${esc(s.id)}</title><script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"><\/script><style>@page{size:297mm 210mm;margin:8mm}html,body{width:281mm;min-height:194mm}*{box-sizing:border-box}body{font-family:-apple-system,BlinkMacSystemFont,"Noto Sans JP",sans-serif;color:#000;margin:0;font-size:9px}.head{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #000;padding-bottom:4px}.title{font-size:22px;font-weight:700;letter-spacing:5px}.meta{text-align:right;line-height:1.6}.info{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin:7px 0}.box{border:1px solid #000;padding:5px;min-height:25px}.label{font-weight:700}.table{width:100%;border-collapse:collapse;table-layout:fixed}.table th,.table td{border:1px solid #000;padding:2px;text-align:center;height:19px;overflow:hidden;white-space:nowrap}.table thead th{background:#eee}.table .coop{width:55px}.table .season{width:24px}.foot{display:grid;grid-template-columns:1fr 2fr 1fr;margin-top:8px;gap:8px}.sign{height:38px;border:1px solid #000;padding:5px}.total{font-weight:700}.note{margin-top:6px;border:1px solid #000;padding:5px;min-height:28px}@media print{.no-print{display:none}}button{padding:8px 16px;font-size:16px}</style></head><body><div id="sheet"><div class="head"><div class="title">出 荷 指 示 書</div><div class="meta">指示番号：${esc(s.id)}<br>作成日：${esc(today())}</div></div><div class="info"><div class="box"><span class="label">出荷先：</span>${esc(s.dest)} 御中</div><div class="box"><span class="label">出荷日：</span>${esc(s.shipDate||'')}</div><div class="box"><span class="label">希望着日：</span>${esc(s.arrivalDate||'')}</div></div><div class="box" style="margin-bottom:6px"><span class="label">生産年度：</span>${esc([...new Set(s.lines.map(l=>(l.year||DEFAULT_YEAR)+'年産'))].join('・'))}　　<span class="label">区分：</span>${esc(seasons)}　　<span class="label">合計：</span>${fmt(s.lines.reduce((a,l)=>a+Number(l.qty||0),0))}</div><table class="table"><thead><tr><th rowspan="2">生産年度</th><th class="coop" rowspan="2">組合名</th><th class="season" rowspan="2">区分</th>${GROUPS.map(g=>`<th colspan="${g.items.length}">${esc(g.name)}</th>`).join('')}<th rowspan="2">計</th></tr><tr>${GROUPS.map(g=>g.items.map(i=>`<th>${esc(i)}</th>`).join('')).join('')}</tr></thead><tbody>${rows}</tbody><tfoot><tr class="total"><th colspan="3">合計</th>${cols.map(ci=>`<td>${fmtBlankZero(s.lines.filter(l=>l.group===ci.group&&l.item===ci.item).reduce((a,x)=>a+Number(x.qty||0),0))}</td>`).join('')}<td>${fmtBlankZero(s.lines.reduce((a,l)=>a+Number(l.qty||0),0))}</td></tr></tfoot></table><div class="note"><b>備考：</b>${esc(s.memo||'')}${s.lines.some(l=>l.memo)?'　明細備考：'+esc(s.lines.filter(l=>l.memo).map(l=>l.memo).join('／')):''}</div><div class="foot"><div class="sign">出荷元：㈱浜中運輸</div><div class="sign">受注・配送指示：</div><div class="sign">FAX送信欄：</div></div></div><div class="no-print" style="margin-top:12px;text-align:center;display:flex;gap:10px;justify-content:center;flex-wrap:wrap"><button onclick="makeLandscapePdf()">印刷／PDF保存</button><button onclick="if(window.opener){window.close()}else{history.back()}">元の画面に戻る</button></div><script>async function makeLandscapePdf(){const b=event&&event.target;b&&(b.disabled=true,b.textContent='横向きPDFを作成中…');try{if(!window.html2pdf)throw new Error('PDF作成ライブラリを読み込めませんでした。');const el=document.getElementById('sheet');const opt={margin:[8,8,8,8],filename:'出荷指示書_${esc(s.id)}.pdf',image:{type:'jpeg',quality:0.98},html2canvas:{scale:2,useCORS:true,backgroundColor:'#ffffff'},jsPDF:{unit:'mm',format:'a4',orientation:'landscape'},pagebreak:{mode:['avoid-all','css','legacy']}};const worker=html2pdf().set(opt).from(el);const url=await worker.outputPdf('bloburl');window.open(url,'_blank');}catch(e){alert('横向きPDFを作成できませんでした。\n'+(e&&e.message?e.message:e));}finally{b&&(b.disabled=false,b.textContent='印刷／PDF保存');}}</script></body></html>`);w.document.close();setTimeout(()=>w.focus(),300);
}
/* ナビ・その他を更新 */
