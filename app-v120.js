...
hStockCanvas=function(y){
  const c=_v68BaseHStockCanvas(y);
  ...
  for(const sec of H_SECTIONS){
    const blockH=sec.items.length*rowH;
...
v80Observer.observe(app,{childList:true});
...
(function(){
  ...
  const obs=new MutationObserver(()=>requestAnimationFrame(run));obs.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  const mo=new MutationObserver(()=>{if(app.querySelector('.v92-viewport'))syncHorizontal(app)});mo.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  const obs=new MutationObserver(()=>v93Decorate());obs.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  const mo=new MutationObserver(()=>{if(app.querySelector('.v92-viewport.v92-home'))requestAnimationFrame(injectHomePdfButton)});
...
(function(){
  ...
  ['home','hHome','nHome','smHome'].forEach(name=>{
    ...
    globalThis[name]=function(){
      const r=old.apply(this,arguments);
      requestAnimationFrame(()=>requestAnimationFrame(v96TidyHome));
      return r;
    };
  });

  const mo=new MutationObserver(()=>{
    if(app&&app.querySelector('.v92-viewport.v92-home')) requestAnimationFrame(v96TidyHome);
  });
...
(function(){
  ...
  ['home','hHome','nHome','smHome'].forEach(name=>{
    ...
    globalThis[name]=function(){
      const r=old.apply(this,arguments);
      requestAnimationFrame(()=>requestAnimationFrame(v97TidyHome));
      return r;
    };
  });

  const mo=new MutationObserver(()=>{
    if(app&&app.querySelector('.v92-viewport.v92-home')) requestAnimationFrame(v97TidyHome);
  });
  if(app)mo.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  const mo=new MutationObserver(()=>requestAnimationFrame(mountFixed));
  if(app)mo.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  async function canvasesFor(item){
    ...
    if(p==='kushiro')return ys.map(y=>_shipmentCanvasPage(s,y));
    if(p==='nemuro')return ys.map(y=>nReportCanvas(y,s));
    if(p==='sanmae')return ys.map(y=>smReportCanvas(y,s));
    if(p==='hidaka'){
      const out=[];const savedRecords=hState.records,savedShip=hState.shipments;
      try{
        for(const y of ys){
          hState.records=(s.lines||[]).filter(l=>(l.year||s.baseYear||DEFAULT_YEAR)===y).map(l=>({...l,year:y,type:'in'}));
          hState.shipments=[];
          out.push(hShipCanvas(s,y));
        }
      } finally {hState.records=savedRecords;hState.shipments=savedShip}
      return out;
    }
    return [];
  }
  ...
  async function openGroupPdf(keys){
    ...
    try{
      ...
      const blob=await multiCanvasPdfBlob(canv),url=URL.createObjectURL(blob);w.location.replace(url);setTimeout(()=>URL.revokeObjectURL(url),10*60*1000);
  ...
  function faxBoxPage(){
    ...
    const cards=[...groups.entries()].map(([g,items],gi)=>{
      const d=items[0]?.dest||{},rows=items.sort((a,b)=>String(a.shipDate||'').localeCompare(String(b.shipDate||''))).map(it=>`<tr>
        <td>${esc(it.shipDate||'')}</td><td><b>${esc(productLabel(it.product))}</b></td><td>${esc(it.id||'')}</td><td style="text-align:right">${fmt(it.qty||0)}</td>
        <td><button class="mini danger" data-fax-remove="${esc(it.key)}">削除</button></td></tr>`).join('');
      return `<section class="card" style="margin-top:12px;padding:14px"><div class="row" style="align-items:flex-start"><div><h3 style="margin:0 0 3px">${esc(d.name||'送信先未設定')}</h3><div class="muted" style="font-size:12px">${esc(d.phone?('TEL '+d.phone):'')} ${esc(d.address||'')}</div></div><span class="pill">${items.length}件</span></div>
        <div class="tablewrap" style="margin-top:10px"><table style="min-width:430px;font-size:12px"><thead><tr><th>出荷日</th><th>昆布</th><th>番号</th><th>個数</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>
        <button class="btn" style="margin-top:10px" data-fax-group="${gi}">📄 この送信先をまとめてPDF/FAX</button></section>`;
    }).join('');
    app.innerHTML=`<section class="card" style="margin-top:12px;padding:14px"><div class="row"><div><h2 style="margin:0">📥 FAX BOX</h2><p class="muted" style="margin:5px 0 0">出荷指示書を送信先ごとにまとめ、複数ページPDFとして開きます。</p></div><span class="pill">${box.length}件</span></div></section>${cards||'<section class="card"><div class="empty">FAX BOXは空です。出荷指示書の「FAX BOXへ追加」から登録してください。</div></section>'}<section class="card" style="margin-top:12px;padding:12px"><div style="display:grid;grid-template-columns:${box.length?'repeat(3,1fr)':'repeat(2,1fr)'};gap:9px"><button class="btn secondary" id="faxBoxHome">🏠</button><button class="btn secondary" id="faxBoxBack">⬅️</button>${box.length?'<button class="btn danger" id="faxBoxClear">全削除</button>':''}</div></section>`;
    const groupArrays=[...groups.values()];
    app.querySelectorAll('[data-fax-group]').forEach(b=>b.onclick=()=>openGroupPdf(groupArrays[+b.dataset.faxGroup].map(x=>x.key)));
...
(function(){
  ...
  function tuneShipmentMenu(){
    ...
    if(first){
      [...first.querySelectorAll(':scope > .row')].forEach(row=>{
        ...
        if(h && (h.textContent||'').trim()==='出荷指示')row.remove();
      });
  ...
  const obs=new MutationObserver(()=>{
    ...
    requestAnimationFrame(()=>{queued=false;tuneShipmentMenu()});
  });
  obs.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  ['home','hHome','nHome','smHome'].forEach(name=>{
    ...
    globalThis[name]=function(){
      const r=old.apply(this,arguments);
      requestAnimationFrame(()=>requestAnimationFrame(v109TidyInventoryHome));
      return r;
    };
  });

  const mo=new MutationObserver(()=>{
    if(app&&app.querySelector('.v92-viewport.v92-home')) requestAnimationFrame(v109TidyInventoryHome);
  });
  if(app)mo.observe(app,{childList:true,subtree:true});
...
(function(){
  ...
  function v114UnifiedShipmentForm(){
    currentProduct=null;v80InventoryMode=false;setHeader('出荷指示');setNavVisible(true);bindNav();
    let lines=[];
    const source0={name:'㈱浜中運輸',address:'',phone:''};
    app.innerHTML=`<section class="card v113-ship-form"><h2>📦 新規出荷指示</h2><div class="form"><datalist id="companyNames">${companyDatalist()}</datalist>
      <div class="v113-date-row"><label>出荷日<input id="v114ShipDate" type="date" value="${today()}"></label><label>着希望日<input id="v114ArrivalDate" type="date" value=""></label></div>
      <div class="v113-party-row"><div class="v113-darkbox"><h3>出荷元</h3><label>会社名<input id="v114SourceName" list="companyNames" value="${esc(source0.name)}"></label><label>住所<input id="v114SourceAddress" value=""></label><label>電話<input id="v114SourcePhone" value=""></label></div><div class="v113-darkbox"><h3>出荷先</h3><label>会社名<input id="v114DestName" list="companyNames" value=""></label><label>住所<input id="v114DestAddress" value=""></label><label>電話<input id="v114DestPhone" value=""></label></div></div>
      <label>備考<input id="v114Memo" placeholder="配送・梱包等の指示"></label>
      <div class="v113-lines-wrap"><div class="v113-lines-title">明細</div><div id="v114Lines"></div><button class="btn secondary" id="v114AddLine">＋ 明細追加</button></div>
      <div class="v113-actions"><button class="btn" id="v114Save">下書き保存</button><button class="btn secondary" id="v114Back">戻る</button></div>
    </div></section>`;
    ...
    const fill=(n,a,p)=>{const c=companyByName(n.value);if(c){a.value=c.address||'';p.value=c.phone||''}};
    sn.onchange=()=>fill(sn,sa,sp);dn.onchange=()=>fill(dn,da,dp);
    ...
    function rowFields(l){
      ...
      if(p==='hidaka'){
        ...
        return {
          year:hYearOptions(l.year||hState.activeYear),
          coop:H_LOCATIONS.map(x=>`<option ${x===l.location?'selected':''}>${esc(x)}</option>`).join(''),coopF:'location',
          section:H_SECTIONS.map(x=>`<option ${x.name===(l.section||'走り')?'selected':''}>${esc(x.name)}</option>`).join(''),sectionF:'section',
          grade:gradeOpts,gradeF:'grade'
        };
      }
    ...
    function render(){
      byId('v114Lines').innerHTML=lines.map((l,i)=>{const d=productDefs[l.product]||productDefs.kushiro,f=rowFields(l);let av=0;try{av=d.avail(l)}catch(e){}return `<div class="v113-line" data-v114-line="${i}"><div class="v118-row v118-row1"><div class="v118-line-no">明細 ${i+1}</div><label>生産年度<select data-f="year">${f.year}</select></label><label>昆布の種類<select class="v114-product-select" data-v114-product="${i}">${productOptions(l.product)}</select></label></div><div class="v118-row v118-row2"><label>漁協<select data-f="${f.coopF}">${f.coop}</select></label><label>区分<select data-f="${f.sectionF}">${f.section}</select></label><label>等級<select data-f="${f.gradeF}">${f.grade}</select></label></div><div class="v118-row v118-row3"><label>数量<input type="number" min="0.01" step="0.01" data-f="qty" value="${esc(l.qty||'')}"></label><div></div><div class="v118-delete-wrap"><button class="mini danger" data-v114-del="${i}">削除</button></div></div><div class="v114-avail">出荷可能在庫：${fmt(av)}</div></div>`}).join('')||'<div class="v113-empty">明細はまだありません。</div>';
      ...
      byId('v114Lines').querySelectorAll('[data-v114-del]').forEach(b=>b.onclick=()=>{lines.splice(+b.dataset.v114Del,1);render()});
      byId('v114Lines').querySelectorAll('[data-v114-line]').forEach(box=>{const i=+box.dataset.v114Line;box.querySelectorAll('[data-f]').forEach(el=>el.onchange=()=>{const d=productDefs[lines[i].product],f=el.dataset.f;d.apply(lines[i],f,el.value);if(lines[i].product==='hidaka'&&f==='section'){const sec=H_SECTIONS.find(x=>x.name===el.value)||H_SECTIONS[0];lines[i].grade=sec.items[0]}if(f!=='qty')render()})});
    }
    byId('v114AddLine').onclick=()=>{lines.push(productDefs.kushiro.make());render()};
    byId('v114Back').onclick=v76ShipmentMenu;
    byId('v114Save').onclick=()=>{
      ...
      const common={source,dest,shipDate:byId('v114ShipDate').value,arrivalDate:byId('v114ArrivalDate').value,memo:byId('v114Memo').value||'',batchId,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString(),status:'draft'};
      const groups={};for(const l of lines)(groups[l.product]||(groups[l.product]=[])).push({...l});
      const created=[];
      if(groups.kushiro){const ls=groups.kushiro.map(({product,...x})=>x),o={id:shipmentId(),status:'draft',source,destInfo:dest,dest:dest.name,baseYear:ls[0]?.year||state.activeYear,shipDate:common.shipDate,arrivalDate:common.arrivalDate,memo:common.memo,batchId,createdAt:common.createdAt,updatedAt:common.updatedAt,lines:ls};state.shipments.push(o);save();created.push(['kushiro',o.id])}
      if(groups.hidaka){const ls=groups.hidaka.map(({product,memo,...x})=>x),o={id:hShipId(),status:'draft',source,dest,shipDate:common.shipDate,arrivalDate:common.arrivalDate,memo:common.memo,batchId,createdAt:common.createdAt,updatedAt:common.updatedAt,lines:ls};hState.shipments.push(o);hSave();created.push(['hidaka',o.id])}
      if(groups.nemuro){const ls=groups.nemuro.map(({product,memo,...x})=>x),o={id:nShipId(),status:'draft',source,dest,shipDate:common.shipDate,arrivalDate:common.arrivalDate,memo:common.memo,batchId,createdAt:common.createdAt,updatedAt:common.updatedAt,lines:ls};nState.shipments.push(o);nSave();created.push(['nemuro',o.id])}
      if(groups.sanmae){const ls=groups.sanmae.map(({product,memo,...x})=>x),o={id:smShipId(),status:'draft',source,dest,shipDate:common.shipDate,arrivalDate:common.arrivalDate,memo:common.memo,batchId,createdAt:common.createdAt,updatedAt:common.updatedAt,lines:ls};smState.shipments.push(o);smSave();created.push(['sanmae',o.id])}
      alert(`出荷指示を保存しました。${created.length>1?'\n昆布種類ごとに '+created.length+'件の出荷指示として作成しています。':''}`);
      if(created.length===1){openGlobalShipment(created[0][0],created[0][1])}else v76ShipmentMenu();
    };
    lines.push(productDefs.kushiro.make());render();
  }
...
/* ===== v115: persistent 4-button shipment bottom navigation ===== */
(function(){
  ...
  css.textContent=`
    #v115ShipmentNav{
      position:fixed!important;left:0!important;right:0!important;bottom:0!important;
      z-index:130!important;background:#0b2b55!important;
    ...
    #v115ShipmentNav button{
      ...
      box-shadow:none!important;font-size:23px!important;line-height:1!important;font-weight:800!important;
      white-space:nowrap!important;
    }
    #v115ShipmentNav button:active{background:rgba(255,255,255,.12)!important}
    #v115ShipmentNav .v115-fax{font-size:20px!important}
    body.v115-shipment-mode main{padding-bottom:calc(76px + env(safe-area-inset-bottom))!important}
    body.v115-shipment-mode > nav{display:none!important}
    body.v115-shipment-mode #v101BackDock{display:none!important}
    body.v115-shipment-mode .v102-fixed-bottom-nav-card{display:none!important}
    @media(max-width:430px){
      #v115ShipmentNav{padding-left:7px!important;padding-right:7px!important;gap:3px!important}
      #v115ShipmentNav button{font-size:21px!important;min-height:48px!important}
      #v115ShipmentNav .v115-fax{font-size:18px!important}
    }
  `;
  document.head.appendChild(css);

  let shipmentMode=false;
  let screenKind='menu';
  ...
  function ensureNav(){
    let nav=document.getElementById('v115ShipmentNav');
    if(!nav){
      nav=document.createElement('nav');nav.id='v115ShipmentNav';nav.setAttribute('aria-label','出荷指示メニュー');
      nav.innerHTML=`<button id="v115ShipHome" aria-label="ホーム" title="ホーム">🏠</button><button id="v115ShipBack" aria-label="ひとつ前に戻る" title="ひとつ前に戻る">⬅️</button><button id="v115ShipList" aria-label="出荷指示一覧" title="出荷指示一覧">📋</button><button id="v115ShipFax" class="v115-fax" aria-label="FAX BOX" title="FAX BOX"></button>`;
      document.body.appendChild(nav);
      nav.querySelector('#v115ShipHome').onclick=()=>{leaveShipmentMode();productLanding()};
      nav.querySelector('#v115ShipBack').onclick=()=>{
        if(screenKind==='menu'){leaveShipmentMode();productLanding();}
        else {screenKind='menu';v76ShipmentMenu();}
      };
      nav.querySelector('#v115ShipList').onclick=()=>{screenKind='menu';v76ShipmentMenu()};
      nav.querySelector('#v115ShipFax').onclick=()=>{screenKind='fax';if(typeof window.v99FaxBoxPage==='function')window.v99FaxBoxPage()};
    }
    nav.querySelector('#v115ShipFax').textContent='📥 ('+faxCount()+')';
    nav.style.display=shipmentMode?'grid':'none';
    document.body.classList.toggle('v115-shipment-mode',shipmentMode);
  }
  function enter(kind){shipmentMode=true;screenKind=kind||'other';requestAnimationFrame(()=>requestAnimationFrame(ensureNav))}
  function leaveShipmentMode(){shipmentMode=false;screenKind='other';document.body.classList.remove('v115-shipment-mode');document.getElementById('v115ShipmentNav')?.remove()}

  function wrap(name,kind){
    const fn=globalThis[name];if(typeof fn!=='function')return;
    globalThis[name]=function(){enter(kind);const r=fn.apply(this,arguments);requestAnimationFrame(ensureNav);return r};
  }

  // 出荷指示のトップ・新規・一覧・詳細をすべて専用ナビ対象にする。
  wrap('v76ShipmentMenu','menu');
  wrap('v114UnifiedShipmentForm','form');
  wrap('shipments','list'); wrap('hShipments','list'); wrap('nShipments','list'); wrap('smShipments','list');
  wrap('shipmentForm','form'); wrap('hShipForm','form'); wrap('nShipForm','form'); wrap('smShipForm','form');
  wrap('shipmentDetail','detail'); wrap('hShipDetail','detail'); wrap('nShipDetail','detail'); wrap('smShipDetail','detail');
  wrap('globalShipmentList','list'); wrap('openGlobalShipment','detail');

  if(typeof window.v99FaxBoxPage==='function'){
    const f=window.v99FaxBoxPage;
    window.v99FaxBoxPage=function(){enter('fax');const r=f.apply(this,arguments);requestAnimationFrame(ensureNav);return r};
  }

  // v114は productChoicePage('shipment') から直接新規画面を開くため、ここも対象。
  const choice=globalThis.productChoicePage;
  if(typeof choice==='function')globalThis.productChoicePage=function(mode){
    if(mode==='shipment')enter('form'); else leaveShipmentMode();
    const r=choice.apply(this,arguments);requestAnimationFrame(ensureNav);return r;
  };

  // 在庫・トップ等へ移動したら専用ナビを外す。
  ['productLanding','home','hHome','nHome','smHome','v73SettingsPage','companyMasterPage'].forEach(name=>{
    const fn=globalThis[name];if(typeof fn!=='function')return;
    globalThis[name]=function(){leaveShipmentMode();return fn.apply(this,arguments)};
  });

  // FAX BOX追加/削除後など件数だけ変わった場合も即時更新。
  window.addEventListener('storage',ensureNav);
  const obs=new MutationObserver(()=>{if(shipmentMode)requestAnimationFrame(ensureNav)});
  obs.observe(app,{childList:true,subtree:true});

  // 表示バージョン
  const oldLanding=globalThis.productLanding;
  if(typeof oldLanding==='function')globalThis.productLanding=function(){const r=oldLanding.apply(this,arguments);const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v115';return r};
})();
/* ===== /v115 ===== */


/* ===== v116: shipment fixed nav visibility / stacking fix ===== */
(function(){
  const css=document.createElement('style');
  css.textContent=`
    /* 出荷指示専用4ボタンを旧トップ/設定ナビより必ず前面へ */
    #v115ShipmentNav{z-index:30050!important;display:grid!important}
    body.v115-shipment-mode #v107LandingNav,
    body.v115-shipment-mode .v106-settings-nav,
    body.v115-shipment-mode #v110HomeNav,
    body.v115-shipment-mode #v101BackDock,
    body.v115-shipment-mode .v102-fixed-bottom-nav-card,
    body.v115-shipment-mode > nav:not(#v115ShipmentNav){display:none!important}
    body.v115-shipment-mode{padding-bottom:0!important}
    body.v115-shipment-mode main{padding-bottom:calc(74px + env(safe-area-inset-bottom))!important}
  `;
  document.head.appendChild(css);

  function cleanupLegacyShipmentNavs(){
    document.body.classList.remove('v107-landing-open','v106-settings-open');
    document.getElementById('v107LandingNav')?.remove();
    document.querySelectorAll('.v106-settings-nav,#v110HomeNav,#v101BackDock,.v102-fixed-bottom-nav-card').forEach(el=>el.remove());
  }

  function forceShipmentNav(){
    cleanupLegacyShipmentNavs();
    document.body.classList.add('v115-shipment-mode');
    const nav=document.getElementById('v115ShipmentNav');
    if(nav){
      nav.style.setProperty('display','grid','important');
      nav.style.setProperty('z-index','30050','important');
      const fax=nav.querySelector('#v115ShipFax');
      if(fax){
        let n=0;try{const a=JSON.parse(localStorage.getItem('kombu-v99-fax-box')||'[]');n=Array.isArray(a)?a.length:0}catch(_e){}
        fax.textContent='📥 ('+n+')';
      }
    }
  }

  // v115で対象になっている各出荷指示関数の実行後に、旧ナビ除去＋専用ナビ前面化。
  const names=['v76ShipmentMenu','v114UnifiedShipmentForm','shipments','hShipments','nShipments','smShipments',
    'shipmentForm','hShipForm','nShipForm','smShipForm','shipmentDetail','hShipDetail','nShipDetail','smShipDetail',
    'globalShipmentList','openGlobalShipment'];
  names.forEach(name=>{
    const fn=globalThis[name]; if(typeof fn!=='function')return;
    globalThis[name]=function(){
      cleanupLegacyShipmentNavs();
      const r=fn.apply(this,arguments);
      requestAnimationFrame(()=>requestAnimationFrame(forceShipmentNav));
      return r;
    };
  });

  const pc=globalThis.productChoicePage;
  if(typeof pc==='function')globalThis.productChoicePage=function(mode){
    if(mode==='shipment')cleanupLegacyShipmentNavs();
    const r=pc.apply(this,arguments);
    if(mode==='shipment')requestAnimationFrame(()=>requestAnimationFrame(forceShipmentNav));
    return r;
  };

  if(typeof window.v99FaxBoxPage==='function'){
    const f=window.v99FaxBoxPage;
    window.v99FaxBoxPage=function(){cleanupLegacyShipmentNavs();const r=f.apply(this,arguments);requestAnimationFrame(()=>requestAnimationFrame(forceShipmentNav));return r};
  }

  // DOM再描画で旧ナビが復活しても出荷指示モード中は即除去。
  const mo=new MutationObserver(()=>{
    if(document.body.classList.contains('v115-shipment-mode'))requestAnimationFrame(forceShipmentNav);
  });
  mo.observe(document.body,{childList:true,subtree:false});

  const oldLanding=globalThis.productLanding;
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    document.body.classList.remove('v115-shipment-mode');
    document.getElementById('v115ShipmentNav')?.remove();
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v116';
    return r;
  };
})();
/* ===== /v116 ===== */

/* ===== v117: inventory screens always use standard 6-button fixed navigation ===== */
(function(){
  const css=document.createElement('style');
  css.textContent=`
    /* 在庫管理中は出荷指示専用4ボタンを必ず隠し、従来6ボタンを常駐 */
    body.v117-inventory-mode #v115ShipmentNav{display:none!important}
    body.v117-inventory-mode > nav:not(#v115ShipmentNav){
      display:flex!important;
      z-index:30060!important;
    }
    body.v117-inventory-mode #v107LandingNav,
    body.v117-inventory-mode .v106-settings-nav,
    body.v117-inventory-mode #v110HomeNav,
    body.v117-inventory-mode #v101BackDock,
    body.v117-inventory-mode .v102-fixed-bottom-nav-card{display:none!important}
    body.v117-inventory-mode main{padding-bottom:calc(78px + env(safe-area-inset-bottom))!important}
  `;
  document.head.appendChild(css);

  function standardNav(){
    return Array.from(document.body.children).find(el=>el.tagName==='NAV' && el.id!=='v115ShipmentNav') || null;
  }
  function enterInventoryMode(){
    document.body.classList.add('v117-inventory-mode');
    document.body.classList.remove('v115-shipment-mode','v107-landing-open','v106-settings-open');
    const shipNav=document.getElementById('v115ShipmentNav');
    if(shipNav)shipNav.style.setProperty('display','none','important');
    document.getElementById('v107LandingNav')?.remove();
    document.querySelectorAll('.v106-settings-nav,#v110HomeNav,#v101BackDock,.v102-fixed-bottom-nav-card').forEach(el=>el.remove());
    const nav=standardNav();
    if(nav){
      nav.style.setProperty('display','flex','important');
      nav.style.setProperty('z-index','30060','important');
    }
    try{bindNav()}catch(_e){}
  }
  function leaveInventoryMode(){
    document.body.classList.remove('v117-inventory-mode');
    const nav=standardNav();
    if(nav){nav.style.removeProperty('display');nav.style.removeProperty('z-index')}
  }

  // 在庫選択画面
  const choice=globalThis.productChoicePage;
  if(typeof choice==='function')globalThis.productChoicePage=function(mode){
    if(mode==='inventory')enterInventoryMode(); else leaveInventoryMode();
    const r=choice.apply(this,arguments);
    if(mode==='inventory')requestAnimationFrame(()=>requestAnimationFrame(enterInventoryMode));
    return r;
  };

  // 各昆布の在庫管理画面へ入る時
  const open=globalThis.openProductContext;
  if(typeof open==='function')globalThis.openProductContext=function(product,mode){
    if(mode==='inventory')enterInventoryMode(); else leaveInventoryMode();
  ...
  ['home','hHome','nHome','smHome'].forEach(name=>{
    ...
    globalThis[name]=function(){
      enterInventoryMode();
      const r=fn.apply(this,arguments);
      requestAnimationFrame(()=>requestAnimationFrame(enterInventoryMode));
      return r;
    };
  });

  // 出荷指示・トップ・設定へ移る時は在庫モードを解除
  ['v76ShipmentMenu','v114UnifiedShipmentForm','shipments','hShipments','nShipments','smShipments',
   'shipmentForm','hShipForm','nShipForm','smShipForm','shipmentDetail','hShipDetail','nShipDetail','smShipDetail',
   'globalShipmentList','openGlobalShipment','productLanding','v73SettingsPage','companyMasterPage'].forEach(name=>{
    const fn=globalThis[name];if(typeof fn!=='function')return;
    globalThis[name]=function(){leaveInventoryMode();return fn.apply(this,arguments)};
  });

  // v116の遅延描画が後から4ボタンを再表示しても在庫管理中なら即6ボタンへ戻す
  const mo=new MutationObserver(()=>{
    if(document.body.classList.contains('v117-inventory-mode'))requestAnimationFrame(enterInventoryMode);
  });
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    leaveInventoryMode();
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v118';
    return r;
  };
})();
/* ===== /v117 ===== */
...
(function(){
  ...
  function ensureShipmentNav(){
    ...
    if(!nav){
      ...
      nav.innerHTML=`<button id="v119Home" aria-label="ホーム" title="ホーム">🏠</button><button id="v119Back" aria-label="ひとつ前に戻る" title="ひとつ前に戻る">⬅️</button><button id="v119List" aria-label="出荷指示一覧" title="出荷指示一覧">📋</button><button id="v119Fax" class="v119-fax" aria-label="FAX BOX" title="FAX BOX"></button>`;
      document.body.appendChild(nav);
      nav.querySelector('#v119Home').onclick=()=>{setMode('');productLanding()};
      nav.querySelector('#v119Back').onclick=()=>{
        if(screenKind==='menu'){setMode('');productLanding()}
        else {screenKind='menu';setMode('shipment','menu');v76ShipmentMenu()}
      };
    ...
    const fax=nav.querySelector('#v119Fax');if(fax)fax.textContent='📥 ('+faxCount()+')';
  ...
  // 古い遅延処理が別ナビを復活させても、現在モードを最優先で再適用。
  const mo=new MutationObserver(()=>{if(mode)requestAnimationFrame(sync)});
  mo.observe(document.body,{childList:true,subtree:false,attributes:true,attributeFilter:['class']});
  window.addEventListener('storage',()=>{if(mode==='shipment')sync()});

  // 現在表示が在庫画面なら初期状態も6ボタンへ。
  standardNav();
  const oldLanding=globalThis.productLanding;
  if(typeof oldLanding==='function')globalThis.productLanding=function(){setMode('');const r=oldLanding.apply(this,arguments);const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v119';return r};
})();
/* ===== /v119 ===== */
...
(function(){
  ...
  function renderTopNav(){
    ...
    if(!nav){
      ...
      nav.innerHTML=`<button id="v120Backup" aria-label="バックアップ" title="バックアップ">💾</button><button id="v120Restore" aria-label="バックアップ復元" title="バックアップ復元">♻️</button><button id="v120Company" aria-label="会社マスター" title="会社マスター">🏢</button>`;
      document.body.appendChild(nav);
      nav.querySelector('#v120Backup').onclick=()=>v73BackupAll();
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    ...
    requestAnimationFrame(()=>requestAnimationFrame(()=>{
      const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v120';
      renderTopNav();
    }));
    return r;
  };
