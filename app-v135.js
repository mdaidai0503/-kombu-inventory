...
function v55CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    window.__v58ShipmentCoopLower=true;window.__v63ShipmentHeaderLarge=true;window.__v135KushiroShipmentCoopMerged=true;try{return v55RetitleStockCanvas(_stockCanvasPage(y),'釧路産昆布　出 荷 依 頼 書',y,s,112,44);}finally{window.__v58ShipmentCoopLower=false;window.__v63ShipmentHeaderLarge=false;window.__v135KushiroShipmentCoopMerged=false;}
...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    window.__v58ShipmentCoopLower=true;window.__v63ShipmentHeaderLarge=true;window.__v135SanmaeShipmentTall=true;window.__v135SanmaeShipmentMerged=true;try{return v55RetitleStockCanvas(smReportCanvas(y,null),'釧路産棹前昆布　出 荷 依 頼 書',y,s,70,34);}finally{window.__v58ShipmentCoopLower=false;window.__v63ShipmentHeaderLarge=false;window.__v135SanmaeShipmentTall=false;window.__v135SanmaeShipmentMerged=false;}
...
async function v130TopBackup(){
  try{
    ...
    const name='昆布在庫出荷管理_'+v135BackupTimestamp()+'.json';
...
/* ===== v135: 自動保存 + 入力途中下書き復元 + 日時入りバックアップ名 ===== */
(function(){
  const SNAP_KEY='kombu-v135-autosave-snapshot';
  const DRAFT_KEY='kombu-v135-form-drafts';
  let saveTimer=0;

  window.v135BackupTimestamp=function(){
  ...
  function safeAllSave(){
    ...
    }catch(e){console.warn('v135 autosave snapshot',e)}
  }
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v135';
    return r;
  };
})();
/* ===== /v135 ===== */
...
/* ===== v135: shipment list bulk-to-FAXBOX + shipment archive ===== */
(function(){
  const FAX_KEY='kombu-v99-fax-box', HIST_KEY='kombu-v135-shipment-history';
  const clone=o=>JSON.parse(JSON.stringify(o));
  ...
  function shipmentHistory(){
    ...
    app.innerHTML=`<section class="card" style="margin-top:14px;padding:14px">
      ...
      <tbody id="v135HistBody">${rows}</tbody></table></div></section>`;
    const body=document.getElementById('v135HistBody');if(body)body.onclick=e=>{const tr=e.target.closest('[data-hid]');if(tr&&e.target.closest('[data-hopen]'))openGlobalShipment(tr.dataset.hprod,tr.dataset.hid)};
    document.body.dataset.v119NavMode='shipment';
    if(typeof window.v135EnsureHistoryNav==='function')window.v135EnsureHistoryNav();
  }
  window.v135ShipmentHistory=shipmentHistory;
  ...
  globalThis.v76ShipmentMenu=function(){
    ...
    if(head && !document.getElementById('v135BulkFax')){
      const right=head.querySelector('.muted');
      const b=document.createElement('button');b.id='v135BulkFax';b.className='mini';
  ...
  function upgradeNav(){
    const nav=document.getElementById('v119ShipmentNav');if(!nav)return;
    if(!document.getElementById('v135History')){
      const b=document.createElement('button');b.id='v135History';b.setAttribute('aria-label','出荷指示履歴');b.title='出荷指示履歴';
  ...
  window.v135EnsureHistoryNav=upgradeNav;
