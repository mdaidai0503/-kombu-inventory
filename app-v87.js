...
/* ===== v85: 固定列をスクロール領域から物理分離（iPhone Safari完全対応） ===== */
(function(){
  const css=document.createElement('style');
  css.textContent=`
    html,body{width:100%!important;max-width:100%!important;overflow-x:clip!important}
    body{position:relative!important}
    main,#app{width:100%!important;max-width:100vw!important;min-width:0!important;overflow-x:clip!important;box-sizing:border-box!important}
    ...
    .v85-stock-split{display:grid!important;grid-template-columns:172px minmax(0,1fr)!important;width:100%!important;max-width:100%!important;min-width:0!important;overflow:hidden!important;border:1px solid #e1e7ee;border-radius:10px;background:#fff;position:relative!important;box-sizing:border-box!important;contain:layout paint!important}
    .v85-fixed-pane{grid-column:1!important;position:relative!important;left:auto!important;top:auto!important;bottom:auto!important;width:172px!important;min-width:172px!important;max-width:172px!important;overflow:hidden!important;background:#fff;z-index:20!important;border-right:2px solid #cfd8e3;box-sizing:border-box!important;transform:none!important}
    .v85-scroll-pane{grid-column:2!important;position:relative!important;left:auto!important;right:auto!important;top:auto!important;bottom:auto!important;width:100%!important;min-width:0!important;max-width:100%!important;overflow-x:auto!important;overflow-y:auto!important;-webkit-overflow-scrolling:touch!important;background:#fff;z-index:1!important;box-sizing:border-box!important;overscroll-behavior-x:contain!important;touch-action:pan-x pan-y!important}
    .v85-fixed-inner{width:172px;will-change:transform}
    .v85-fixed-table{width:172px!important;min-width:172px!important;max-width:172px!important;border-collapse:separate!important;border-spacing:0!important;table-layout:fixed!important;margin:0!important}
    .v85-fixed-table th,.v85-fixed-table td{box-sizing:border-box!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:clip!important;background:#fff!important}
    .v85-fixed-table .v85-f1{width:108px!important;min-width:108px!important;max-width:108px!important;text-align:left!important}
    .v85-fixed-table .v85-f2{width:64px!important;min-width:64px!important;max-width:64px!important;text-align:center!important}
    .v85-fixed-table .v85-ftotal{width:172px!important;min-width:172px!important;max-width:172px!important;text-align:center!important}
    .v85-right-table{margin:0!important;border-collapse:separate!important;border-spacing:0!important;table-layout:auto!important;width:max-content!important;min-width:100%!important}
    .v85-right-table .v85-f1,.v85-right-table .v85-f2,.v85-right-table .v85-ftotal{display:none!important}
    .v85-right-table th,.v85-right-table td{min-width:54px!important;max-width:68px!important;padding:4px 5px!important;white-space:nowrap!important}
    .v85-right-table.v85-hidaka th,.v85-right-table.v85-hidaka td,
    .v85-right-table.v85-sanmae th,.v85-right-table.v85-sanmae td{min-width:54px!important;max-width:68px!important}
    .v85-fixed-table tr:first-child th{background:#eaf0f7!important;font-weight:700!important}
  `;
  document.head.appendChild(css);

  function isInventoryTable(table){
    if(!window.v80InventoryMode || !table || table.dataset.v85Split==='1') return false;
  ...
  function markFixed(table){
    const rows=[...table.rows]; if(!rows.length)return;
    const first=rows[0];
    if(first.cells[0])first.cells[0].classList.add('v85-f1');
    if(first.cells[1])first.cells[1].classList.add('v85-f2');
    const hasTwoHeader=Number(first.cells[0]?.rowSpan||1)>1;
    rows.forEach((row,ri)=>{
      if(ri===0 || (hasTwoHeader&&ri===1))return;
      const cells=row.cells;if(!cells||!cells.length)return;
      if(Number(cells[0].colSpan||1)>=2){cells[0].classList.add('v85-ftotal');return;}
      if(cells.length>=2){cells[0].classList.add('v85-f1');cells[1].classList.add('v85-f2');}
    });
  }

  function splitTable(table){
    if(!isInventoryTable(table))return;
    const wrap=table.closest('.tablewrap,.v71-home-table'); if(!wrap || wrap.dataset.v85Done==='1')return;
    markFixed(table);
    table.dataset.v85Split='1';
    ...
    // 左固定表は元表の完全コピーから固定2列だけ残す
    const left=table.cloneNode(true);
    left.removeAttribute('id');
    left.classList.add('v85-fixed-table');
    left.classList.remove('v82-inventory-table','v85-right-table');
    left.querySelectorAll('tr').forEach((r,ri)=>{
      [...r.cells].forEach(c=>{
        if(!(c.classList.contains('v85-f1')||c.classList.contains('v85-f2')||c.classList.contains('v85-ftotal'))) c.style.display='none';
        else {c.style.transform='none';c.style.webkitTransform='none';c.style.position='static';}
      });
      if(rowHeights[ri]){r.style.height=rowHeights[ri]+'px';r.style.minHeight=rowHeights[ri]+'px';}
    });

    // 右表は固定2列だけ非表示。残りだけを横スクロールする
    table.classList.add('v85-right-table');
    const label=(document.querySelector('header')?.textContent||'');
    if(label.includes('日高'))table.classList.add('v85-hidaka');
    if(label.includes('棹前'))table.classList.add('v85-sanmae');
    table.querySelectorAll('.v85-f1,.v85-f2,.v85-ftotal').forEach(c=>{c.style.transform='none';c.style.webkitTransform='none';c.style.position='static';});
    [...table.rows].forEach((r,ri)=>{if(rowHeights[ri]){r.style.height=rowHeights[ri]+'px';r.style.minHeight=rowHeights[ri]+'px';}});

    const split=document.createElement('div');split.className='v85-stock-split';
    const naturalH=Math.max(1,rowHeights.reduce((a,b)=>a+b,0));
    split.style.height=(hadMax?visibleH:naturalH)+'px';
    split.style.maxWidth='100%';split.style.minWidth='0';
    const lp=document.createElement('div');lp.className='v85-fixed-pane';
    const li=document.createElement('div');li.className='v85-fixed-inner';li.appendChild(left);lp.appendChild(li);
    const rp=document.createElement('div');rp.className='v85-scroll-pane';
    rp.appendChild(table);
    split.appendChild(lp);split.appendChild(rp);

    wrap.dataset.v85Done='1';
    wrap.parentNode.replaceChild(split,wrap);

    // 縦スクロールのみ同期。横方向は左ペインがスクロール領域外なので絶対に動かない
    let raf=0;
    const sync=()=>{raf=0;li.style.transform=`translate3d(0,${-(rp.scrollTop||0)}px,0)`;};
    rp.addEventListener('scroll',()=>{if(raf)cancelAnimationFrame(raf);raf=requestAnimationFrame(sync)},{passive:true});
    sync();
  }
  ...
  ['productLanding','productChoicePage'].forEach(name=>{
    const fn=window[name];if(typeof fn!=='function')return;
    window[name]=function(){const r=fn.apply(this,arguments);const pill=app.querySelector('.pill');if(pill)pill.textContent='v87';return r};
  });
  requestAnimationFrame(run);
})();
/* ===== /v85 ===== */

/* v87: grid split panes; page width locked to viewport, only right pane scrolls horizontally */
