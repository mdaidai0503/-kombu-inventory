...
async function parseInventoryPdf(file){
  ...
  PDFJS.GlobalWorkerOptions.workerSrc='./pdf-worker-v52.js';
...
async function v42BulkPdfImport(file){
  ...
  app.innerHTML=`<section class="card" style="margin-top:22px"><div class="row"><h2>📄 4種類 PDF一括入庫</h2><span class="pill">v52</span></div><p><b>${esc(file.name)}</b></p><div class="note">PDFを1回読み込み、釧路産昆布・日高昆布・根室産昆布・釧路産棹前昆布を自動判別しています。</div><p id="v42Progress" style="margin-top:16px;font-weight:700">PDFを準備しています…</p></section>`;
...
productChoicePage=function(mode){
  ...
  const pill=app.querySelector('.pill');if(pill)pill.textContent='v52';
  if(mode==='inventory'){
    const card=app.querySelector('.card'),back=document.getElementById('v38Back');
...
productLanding=function(){_v41LandingForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v52'};
const _v41CompanyForV42=companyMasterPage;
companyMasterPage=function(){_v41CompanyForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v52'};

bindNav();productLanding();


/* ===== v52: iPhone Safari shipment PDF/FAX white-screen fix =====
...
*/
function v52ShipmentPrintPreview(opts){
  ...
  app.innerHTML=`
  <style id="v52ShipPrintStyle">
    #v52ShipPrint{margin:-14px -12px 0}
    .v52bar{position:sticky;top:0;z-index:30;background:#173661;color:#fff;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v52bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}
    .v52print{background:#fff;color:#173661}.v52back{background:#dfe7f1;color:#173661}
    .v52hint{background:#fff3bf;color:#5c4b00;padding:10px;text-align:center;font-size:13px}
    .v52preview{overflow:auto;background:#eef2f6;padding:8px}
    .v52sheet{width:281mm;min-height:194mm;margin:0 auto 10px;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v52head{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #000;padding-bottom:5px}
    .v52title{font-size:24px;font-weight:800;letter-spacing:5px}.v52meta{text-align:right;font-size:10px;line-height:1.6}
    .v52info{display:grid;grid-template-columns:1fr 1fr .8fr;gap:6px;margin:7px 0;font-size:10px}
    .v52box,.v52memo,.v52sign>div{border:1px solid #222;padding:5px}
    .v52summary{border:1px solid #222;padding:5px;margin-bottom:7px;font-size:10px}
    .v52table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:10px}
    .v52table th,.v52table td{border:.5px solid #333;padding:3px;text-align:center;height:22px;overflow:hidden;white-space:nowrap}
    .v52table thead th{background:#eee}.v52table tfoot th,.v52table tfoot td{border-top:1.5px solid #000;font-weight:700}
    .v52memo{margin-top:7px;min-height:32px;font-size:10px}
    .v52sign{display:grid;grid-template-columns:1fr 1.5fr 1fr;gap:6px;margin-top:7px;font-size:10px}.v52sign>div{height:34px}
    @media print{
      @page{size:A4 landscape;margin:7mm}
      header,nav,.v52bar,.v52hint{display:none!important}
      main{padding:0!important;max-width:none!important}
      .v52preview{overflow:visible;background:#fff;padding:0}
      .v52sheet{margin:0;width:auto;min-height:auto;padding:0;box-shadow:none}
    }
  </style>
  <div id="v52ShipPrint">
    <div class="v52bar">
      <button class="v52print" id="v52DoPrint">PDF・印刷・FAXへ</button>
      <button class="v52back" id="v52Back">出荷指示へ戻る</button>
    </div>
    <div class="v52hint"><b>帳票プレビュー</b> — 下に出荷指示書が表示されていれば正常です。「PDF・印刷・FAXへ」からiPhoneの印刷・共有機能を使えます。</div>
    <div class="v52preview">
      <div class="v52sheet">
        <div class="v52head">
          <div class="v52title">出 荷 指 示 書</div>
          <div class="v52meta">指示番号：${esc(s.id||'')}<br>作成日：${esc(today())}</div>
        </div>
        <div class="v52info">
          <div class="v52box"><b>出荷先：</b>${esc(dst.name||'')} 御中<br>住所：${esc(dst.address||'')}<br>電話：${esc(dst.phone||'')}</div>
          <div class="v52box"><b>出荷元：</b>${esc(src.name||'')}<br>住所：${esc(src.address||'')}<br>電話：${esc(src.phone||'')}</div>
          <div class="v52box"><b>出荷日：</b>${esc(s.shipDate||'')}<br><b>希望着日：</b>${esc(s.arrivalDate||'')}</div>
        </div>
        <div class="v52summary"><b>昆布：</b>${esc(opts.title)}　　<b>合計数量：</b>${fmt(total)}</div>
        ...
        <div class="v52memo"><b>備考：</b>${esc(s.memo||'')}</div>
        <div class="v52sign"><div>出荷元：${esc(src.name||'')}</div><div>受注・配送指示：</div><div>FAX送信欄：</div></div>
      </div>
    </div>
  </div>`;
  const p=document.getElementById('v52DoPrint');
  const b=document.getElementById('v52Back');
...
/* ===== v52 出荷指示書 = 各昆布の在庫集計表フォーマット ===== */
function v52MatrixShipmentPreview(o){
  ...
  app.innerHTML=`
  <style>
    .v52bar{position:sticky;top:0;z-index:30;background:#173661;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v52bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}.v52go{background:#fff;color:#173661}.v52back{background:#dfe7f1;color:#173661}
    .v52wrap{overflow:auto;background:#eef2f6;padding:8px}.v52sheet{width:281mm;min-height:194mm;margin:auto;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v52title{text-align:center;font-size:23px;font-weight:800;letter-spacing:4px;margin-bottom:5px}
    .v52sub{text-align:center;font-size:14px;font-weight:700;margin-bottom:7px}
    .v52info{display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;font-size:9px;margin-bottom:6px}.v52info>div{border:1px solid #333;padding:4px}
    .v52tbl{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9px}.v52tbl th,.v52tbl td{border:1px solid #333;text-align:center;padding:3px;height:21px}.v52tbl thead th,.v52tbl tfoot th{background:#e8edf3;font-weight:700}.v52tbl tbody th{background:#f4f6f8;text-align:left}
    .v52memo{border:1px solid #333;margin-top:6px;padding:5px;min-height:27px;font-size:9px}
    @media print{@page{size:A4 landscape;margin:7mm}header,nav,.v52bar{display:none!important}main{padding:0!important;max-width:none!important}.v52wrap{overflow:visible;background:#fff;padding:0}.v52sheet{width:auto;min-height:auto;padding:0;box-shadow:none}}
  </style>
  <div class="v52bar"><button class="v52go" id="v52print">PDF・印刷・FAXへ</button><button class="v52back" id="v52back">出荷指示へ戻る</button></div>
  <div class="v52wrap"><section class="v52sheet">
    <div class="v52title">${esc(o.reportTitle)}</div>
    <div class="v52sub">出　荷　指　示　書</div>
    <div class="v52info">
    ...
    <table class="v52tbl"><thead><tr><th>${esc(o.rowLabel)}</th>${cols}<th>合計</th></tr></thead>
      <tbody>${body}</tbody><tfoot><tr><th>合計</th>${foot}<th>${fmt(total)}</th></tr></tfoot></table>
    <div class="v52memo"><b>備考：</b>${esc(s.memo||'')}</div>
  </section></div>`;
  document.getElementById('v52print').onclick=()=>window.print();
  document.getElementById('v52back').onclick=()=>{setHeader(oldHeader);o.back();};
}
function v52Unique(a){return [...new Set(a.filter(Boolean))]}

/* 釧路：在庫集計表と同じ「漁協 × 等級/分類」の横持ち表 */
openShipmentPdfDirect=function(id){
 const s=state.shipments.find(x=>x.id===id); if(!s)return;
 v52MatrixShipmentPreview({title:'釧路産昆布 出荷指示',reportTitle:'釧 路 産 昆 布',
 shipment:s,source:shipmentSource(s),dest:shipmentDest(s),rowLabel:'漁協',
 rows:ls=>{const order=['東部漁協','昆布森漁協','厚岸漁協','散布漁協','浜中漁協'];const got=v52Unique(ls.map(l=>l.coop));return [...order.filter(x=>got.includes(x)),...got.filter(x=>!order.includes(x))]},
 cols:ls=>v52Unique(ls.map(l=>[l.season,l.group,l.item].filter(Boolean).join('・'))),
...
hOpenShipPdf=function(s){if(!s)return;
 v52MatrixShipmentPreview({title:'日高昆布 出荷指示',reportTitle:'日 高 昆 布',
 shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'産地',
 rows:ls=>v52Unique(ls.map(l=>l.location)),cols:ls=>v52Unique(ls.map(l=>[l.section,l.grade].filter(Boolean).join('・'))),
...
nOpenShipPdf=function(s){if(!s)return;
 v52MatrixShipmentPreview({title:'根室産昆布 出荷指示',reportTitle:'根 室 産 昆 布',
 shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'漁協',
 rows:ls=>v52Unique(ls.map(l=>l.coop)),cols:ls=>v52Unique(ls.map(l=>[l.season,l.group,l.item].filter(Boolean).join('・'))),
 rowKey:l=>l.coop||'',colKey:l=>[l.season,l.group,l.item].filter(Boolean).join('・'),
 back:()=>nShipDetail(s.id)});
};
...
if(typeof spOpenShipPdf==='function'){
 const v52OldSp=spOpenShipPdf;
 spOpenShipPdf=function(s){
   if(!s)return v52OldSp(s);
   v52MatrixShipmentPreview({title:'釧路産棹前昆布 出荷指示',reportTitle:'釧 路 産 棹 前 昆 布',
   shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'漁協',
   rows:ls=>v52Unique(ls.map(l=>l.coop||l.location)),cols:ls=>v52Unique(ls.map(l=>[l.season,l.group,l.item,l.grade].filter(Boolean).join('・'))),
