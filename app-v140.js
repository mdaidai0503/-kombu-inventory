...
function hStockCanvas(y){
 ...
 const hidakaShip=!!window.__v140HidakaShipment;
 ...
 rows.forEach(r=>{
   ...
   /* v140: 区分列は各区分ごとに縦結合。区分途中の横線は等級列以右だけに描画する。 */
 ...
 x.lineWidth=1.6;x.beginPath();x.moveTo(tx,yy);x.lineTo(tx+tw,yy);x.stroke();x.lineWidth=.55;
...
function v55CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    window.__v59HidakaSectionCentered=true;window.__v63ShipmentHeaderLarge=true;window.__v140HidakaShipment=true;try{return v55RetitleStockCanvas(hStockCanvas(y),'日高昆布　出 荷 依 頼 書',y,s,80,35);}finally{window.__v59HidakaSectionCentered=false;window.__v63ShipmentHeaderLarge=false;window.__v140HidakaShipment=false;}
...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, tableY, tableX){
  ...
  /* v140: 年産表示と上部罫線の干渉を避けるため、帳票本体を少し下へ移動。
  ...
  const isHidakaShipment=!!window.__v140HidakaShipment;
  if(isHidakaShipment){
    text(`依頼日：${shipment?.shipDate||''}`,tableX,30,32,'left',true);
    text(`依頼番号：${shipment?.id||''}`,W-tableX,18,12,'right',false);
  }else{
    text(`依頼番号：${shipment?.id||''}　出荷日：${shipment?.shipDate||''}`,W-tableX,18,12,'right',false);
  }
