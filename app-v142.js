...
function hStockCanvas(y){
 const W=1684,H=1191,m=35,c=document.createElement('canvas');c.width=W;c.height=H;
 const x=c.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,W,H);x.strokeStyle='#222';x.fillStyle='#000';
 const font=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif`;
 const txt=(t,xx,yy,z=14,a='center',b=false)=>{x.font=font(z,b);x.textAlign=a;x.textBaseline='middle';x.fillText(String(t??''),xx,yy)};
 const rows=H_SECTIONS.flatMap(s=>s.items.map(g=>({section:s.name,grade:g}))),mt=hMatrix();
 const hidakaShip=!!window.__v141HidakaShipment;
 txt('日 高 昆 布　在 庫 集 計 表',m,45,30,'left',true);txt(`${y}年産`,W-m,45,18,'right',true);
 const tx=m,ty=80,tw=W-m*2,secW=70,gradeW=95,totalW=60,colW=(tw-secW-gradeW-totalW)/H_LOCATIONS.length,baseRowH=26,runRowH=hidakaShip?36:baseRowH,headH=48,totalH=32;
 const rowHeight=r=>(hidakaShip&&r.section==='走り'?runRowH:baseRowH);
 /* v142: 日高出荷依頼書では区分セルの境界を等級行より3行分下へずらす。 */
 const sectionShiftRows=hidakaShip?3:0;
 const bodyH=rows.reduce((a,r)=>a+rowHeight(r),0);
 x.lineWidth=.55;x.strokeRect(tx,ty,tw,headH+bodyH+totalH);
 [tx+secW,tx+secW+gradeW,tx+tw-totalW].forEach(xx=>{x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+headH+bodyH+totalH);x.stroke()});
 const shHead=!!window.__v63ShipmentHeaderLarge;
 txt('区分',tx+secW/2,ty+headH/2,shHead?17:13);txt('等級',tx+secW+gradeW/2,ty+headH/2,shHead?17:13);
 H_LOCATIONS.forEach((l,i)=>{const xx=tx+secW+gradeW+i*colW;x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+headH+bodyH+totalH);x.stroke();txt(l,xx+colW/2,ty+headH/2,shHead?18:10)});
 txt('計',tx+tw-totalW/2,ty+headH/2,shHead?16:12);
 let yy=ty+headH,last=null;
 rows.forEach(r=>{
   const rh=rowHeight(r);
   if(r.section!==last){x.lineWidth=1.6;x.beginPath();x.moveTo(tx,yy);x.lineTo(tx+tw,yy);x.stroke();x.lineWidth=.55}
   if(r.section!==last){
     const secRows=rows.filter(z=>z.section===r.section),secHeight=secRows.reduce((a,z)=>a+rowHeight(z),0);
     const sectionY=window.__v59HidakaSectionCentered?yy+secHeight/2:yy+rh/2;
     txt(r.section,tx+secW/2,sectionY,hidakaShip?14:12);
   }
   txt(r.grade,tx+secW+gradeW/2,yy+rh/2,(hidakaShip&&r.section==='走り')?14:11);
   let rt=0;
   H_LOCATIONS.forEach((l,i)=>{const q=mt[[y,l,r.section,r.grade].join('|')]||0,xx=tx+secW+gradeW+i*colW;rt+=q;x.beginPath();x.moveTo(xx,yy);x.lineTo(xx,yy+rh);x.stroke();if(q)txt(fmt(q),xx+colW/2,yy+rh/2,shHead?18:14)});
   if(rt)txt(fmt(rt),tx+tw-totalW/2,yy+rh/2,shHead?18:14);
   /* v141: 区分列は各区分ごとに縦結合。区分途中の横線は等級列以右だけに描画する。 */
   const nextIndex=rows.indexOf(r)+1;
   const nextRow=rows[nextIndex];
   const isSectionEnd=!nextRow||nextRow.section!==r.section;
   x.beginPath();x.moveTo(isSectionEnd?tx:tx+secW,yy+rh);x.lineTo(tx+tw,yy+rh);x.stroke();
   yy+=rh;last=r.section
 });
 /* v141: 区分列を最後に再構築。
    走りは1等〜白2等までを完全な1セルとし、その下から后採・拾い・雑を順に開始する。
    これにより走り行高拡大分だけ後続区分も正確に下へスライドする。 */
 x.fillStyle='#fff';x.fillRect(tx,ty+headH,secW,bodyH);
 x.strokeStyle='#222';x.lineWidth=.55;
 x.beginPath();x.moveTo(tx,ty+headH);x.lineTo(tx,ty+headH+bodyH);x.stroke();
 x.beginPath();x.moveTo(tx+secW,ty+headH);x.lineTo(tx+secW,ty+headH+bodyH);x.stroke();
 let secY=ty+headH;
 H_SECTIONS.forEach((sec,si)=>{
   const secRows=rows.filter(r=>r.section===sec.name);
   const secH=secRows.reduce((a,r)=>a+rowHeight(r),0);
   const shift=(hidakaShip&&si>0)?sectionShiftRows*baseRowH:0;
   const nextShift=(hidakaShip&&si<H_SECTIONS.length-1)?sectionShiftRows*baseRowH:0;
   const drawY=secY+shift;
   const drawH=secH+nextShift-shift;
   x.lineWidth=1.6;
   x.beginPath();x.moveTo(tx,drawY);x.lineTo(tx+secW,drawY);x.stroke();
   x.lineWidth=.55;
   txt(sec.name,tx+secW/2,drawY+drawH/2,hidakaShip?14:12,'center',false);
   secY+=secH;
 });
 x.lineWidth=1.6;
 x.beginPath();x.moveTo(tx,secY);x.lineTo(tx+secW,secY);x.stroke();
 x.beginPath();x.moveTo(tx,yy);x.lineTo(tx+tw,yy);x.stroke();x.lineWidth=.55;
 txt('合計',tx+(secW+gradeW)/2,yy+16,13,'center',true);
 H_LOCATIONS.forEach((l,i)=>{const q=rows.reduce((a,r)=>a+(mt[[y,l,r.section,r.grade].join('|')]||0),0);if(q)txt(fmt(q),tx+secW+gradeW+i*colW+colW/2,yy+16,shHead?21:13)});
 txt(fmt(hTotal(y)),tx+tw-totalW/2,yy+16,shHead?21:13);
 return c
}
...
function v55CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    window.__v59HidakaSectionCentered=true;window.__v63ShipmentHeaderLarge=true;window.__v141HidakaShipment=true;try{return v55RetitleStockCanvas(hStockCanvas(y),'日高昆布　出 荷 依 頼 書',y,s,80,35);}finally{window.__v59HidakaSectionCentered=false;window.__v63ShipmentHeaderLarge=false;window.__v141HidakaShipment=false;}
...
(function(){
  ...
  function shipmentHistory(){
    ...
    app.innerHTML=`<section class="card" style="margin-top:14px;padding:14px">
      ...
      <tbody id="v136HistBody">${rows}</tbody></table></div></section>`;
    const body=document.getElementById('v136HistBody');if(body)body.onclick=e=>{const tr=e.target.closest('[data-hid]');if(tr&&e.target.closest('[data-hopen]'))openGlobalShipment(tr.dataset.hprod,tr.dataset.hid)};
