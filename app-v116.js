...
(function(){
  ...
  style.textContent=`
    ...
    .v114-product-select{width:auto!important;min-width:108px!important;max-width:145px!important;background:#fff!important;color:#102a43!important;padding:6px 8px!important;border-radius:8px!important;font-weight:800!important}
    .v114-avail{font-size:11.5px;color:#dce8f6;margin-top:5px;text-align:right}
    @media(max-width:520px){
      .v114-product-select{min-width:96px!important;max-width:120px!important;font-size:12px!important}
  ...
  function v114UnifiedShipmentForm(){
    ...
    function render(){
      byId('v114Lines').innerHTML=lines.map((l,i)=>{const d=productDefs[l.product]||productDefs.kushiro;let av=0;try{av=d.avail(l)}catch(e){}return `<div class="v113-line" data-v114-line="${i}"><div class="v114-line-head"><span class="v114-line-title">明細 ${i+1}</span><select class="v114-product-select" data-v114-product="${i}">${productOptions(l.product)}</select><button class="mini danger" data-v114-del="${i}">削除</button></div><div class="form">${d.fields(l)}<div class="subgrid"><label>数量<input type="number" min="0.01" step="0.01" data-f="qty" value="${esc(l.qty||'')}"></label><label>明細備考<input data-f="memo" value="${esc(l.memo||'')}"></label></div><div class="v114-avail">出荷可能在庫：${fmt(av)}</div></div></div>`}).join('')||'<div class="v113-empty">明細はまだありません。</div>';
      ...
      byId('v114Lines').querySelectorAll('[data-v114-line]').forEach(box=>{const i=+box.dataset.v114Line;box.querySelectorAll('[data-f]').forEach(el=>el.onchange=()=>{const d=productDefs[lines[i].product];d.apply(lines[i],el.dataset.f,el.value);if(el.dataset.f!=='qty'&&el.dataset.f!=='memo')render()})});
    }
