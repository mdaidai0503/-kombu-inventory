...
async function parseInventoryPdf(file){
  ...
  PDFJS.GlobalWorkerOptions.workerSrc='./pdf-worker-v53.js';
...
async function v42BulkPdfImport(file){
  ...
  app.innerHTML=`<section class="card" style="margin-top:22px"><div class="row"><h2>📄 4種類 PDF一括入庫</h2><span class="pill">v53</span></div><p><b>${esc(file.name)}</b></p><div class="note">PDFを1回読み込み、釧路産昆布・日高昆布・根室産昆布・釧路産棹前昆布を自動判別しています。</div><p id="v42Progress" style="margin-top:16px;font-weight:700">PDFを準備しています…</p></section>`;
...
productChoicePage=function(mode){
  ...
  const pill=app.querySelector('.pill');if(pill)pill.textContent='v53';
  if(mode==='inventory'){
    const card=app.querySelector('.card'),back=document.getElementById('v38Back');
...
productLanding=function(){_v41LandingForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v53'};
const _v41CompanyForV42=companyMasterPage;
companyMasterPage=function(){_v41CompanyForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v53'};

bindNav();productLanding();


/* ===== v53: iPhone Safari shipment PDF/FAX white-screen fix =====
...
*/
function v53ShipmentPrintPreview(opts){
  ...
  app.innerHTML=`
  <style id="v53ShipPrintStyle">
    #v53ShipPrint{margin:-14px -12px 0}
    .v53bar{position:sticky;top:0;z-index:30;background:#173661;color:#fff;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v53bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}
    .v53print{background:#fff;color:#173661}.v53back{background:#dfe7f1;color:#173661}
    .v53hint{background:#fff3bf;color:#5c4b00;padding:10px;text-align:center;font-size:13px}
    .v53preview{overflow:auto;background:#eef2f6;padding:8px}
    .v53sheet{width:281mm;min-height:194mm;margin:0 auto 10px;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v53head{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #000;padding-bottom:5px}
    .v53title{font-size:24px;font-weight:800;letter-spacing:5px}.v53meta{text-align:right;font-size:10px;line-height:1.6}
    .v53info{display:grid;grid-template-columns:1fr 1fr .8fr;gap:6px;margin:7px 0;font-size:10px}
    .v53box,.v53memo,.v53sign>div{border:1px solid #222;padding:5px}
    .v53summary{border:1px solid #222;padding:5px;margin-bottom:7px;font-size:10px}
    .v53table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:10px}
    .v53table th,.v53table td{border:.5px solid #333;padding:3px;text-align:center;height:22px;overflow:hidden;white-space:nowrap}
    .v53table thead th{background:#eee}.v53table tfoot th,.v53table tfoot td{border-top:1.5px solid #000;font-weight:700}
    .v53memo{margin-top:7px;min-height:32px;font-size:10px}
    .v53sign{display:grid;grid-template-columns:1fr 1.5fr 1fr;gap:6px;margin-top:7px;font-size:10px}.v53sign>div{height:34px}
    @media print{
      @page{size:A4 landscape;margin:7mm}
      header,nav,.v53bar,.v53hint{display:none!important}
      main{padding:0!important;max-width:none!important}
      .v53preview{overflow:visible;background:#fff;padding:0}
      .v53sheet{margin:0;width:auto;min-height:auto;padding:0;box-shadow:none}
    }
  </style>
  <div id="v53ShipPrint">
    <div class="v53bar">
      <button class="v53print" id="v53DoPrint">PDF・印刷・FAXへ</button>
      <button class="v53back" id="v53Back">出荷指示へ戻る</button>
    </div>
    <div class="v53hint"><b>帳票プレビュー</b> — 下に出荷指示書が表示されていれば正常です。「PDF・印刷・FAXへ」からiPhoneの印刷・共有機能を使えます。</div>
    <div class="v53preview">
      <div class="v53sheet">
        <div class="v53head">
          <div class="v53title">出 荷 指 示 書</div>
          <div class="v53meta">指示番号：${esc(s.id||'')}<br>作成日：${esc(today())}</div>
        </div>
        <div class="v53info">
          <div class="v53box"><b>出荷先：</b>${esc(dst.name||'')} 御中<br>住所：${esc(dst.address||'')}<br>電話：${esc(dst.phone||'')}</div>
          <div class="v53box"><b>出荷元：</b>${esc(src.name||'')}<br>住所：${esc(src.address||'')}<br>電話：${esc(src.phone||'')}</div>
          <div class="v53box"><b>出荷日：</b>${esc(s.shipDate||'')}<br><b>希望着日：</b>${esc(s.arrivalDate||'')}</div>
        </div>
        <div class="v53summary"><b>昆布：</b>${esc(opts.title)}　　<b>合計数量：</b>${fmt(total)}</div>
        <table class="v53table">
        ...
        <div class="v53memo"><b>備考：</b>${esc(s.memo||'')}</div>
        <div class="v53sign"><div>出荷元：${esc(src.name||'')}</div><div>受注・配送指示：</div><div>FAX送信欄：</div></div>
      </div>
    </div>
  </div>`;
  const p=document.getElementById('v53DoPrint');
  const b=document.getElementById('v53Back');
...
/* ===== v53 出荷指示書 = 各昆布の在庫集計表フォーマット ===== */
function v53MatrixShipmentPreview(o){
  ...
  app.innerHTML=`
  <style>
    .v53bar{position:sticky;top:0;z-index:30;background:#173661;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v53bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}.v53go{background:#fff;color:#173661}.v53back{background:#dfe7f1;color:#173661}
    .v53wrap{overflow:auto;background:#eef2f6;padding:8px}.v53sheet{width:281mm;min-height:194mm;margin:auto;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v53title{text-align:center;font-size:23px;font-weight:800;letter-spacing:4px;margin-bottom:5px}
    .v53sub{text-align:center;font-size:14px;font-weight:700;margin-bottom:7px}
    .v53info{display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;font-size:9px;margin-bottom:6px}.v53info>div{border:1px solid #333;padding:4px}
    .v53tbl{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9px}.v53tbl th,.v53tbl td{border:1px solid #333;text-align:center;padding:3px;height:21px}.v53tbl thead th,.v53tbl tfoot th{background:#e8edf3;font-weight:700}.v53tbl tbody th{background:#f4f6f8;text-align:left}
    .v53memo{border:1px solid #333;margin-top:6px;padding:5px;min-height:27px;font-size:9px}
    @media print{@page{size:A4 landscape;margin:7mm}header,nav,.v53bar{display:none!important}main{padding:0!important;max-width:none!important}.v53wrap{overflow:visible;background:#fff;padding:0}.v53sheet{width:auto;min-height:auto;padding:0;box-shadow:none}}
  </style>
  <div class="v53bar"><button class="v53go" id="v53print">PDF・印刷・FAXへ</button><button class="v53back" id="v53back">出荷指示へ戻る</button></div>
  <div class="v53wrap"><section class="v53sheet">
    <div class="v53title">${esc(o.reportTitle)}</div>
    <div class="v53sub">出　荷　指　示　書</div>
    <div class="v53info">
    ...
    <table class="v53tbl"><thead><tr><th>${esc(o.rowLabel)}</th>${cols}<th>合計</th></tr></thead>
  ...
  document.getElementById('v53back').onclick=()=>{setHeader(oldHeader);o.back();};
}
function v53Unique(a){return [...new Set(a.filter(Boolean))]}

/* 釧路：在庫集計表と同じ「漁協 × 等級/分類」の横持ち表 */
openShipmentPdfDirect=function(id){
 const s=state.shipments.find(x=>x.id===id); if(!s)return;
 v53MatrixShipmentPreview({title:'釧路産昆布 出荷指示',reportTitle:'釧 路 産 昆 布',
 shipment:s,source:shipmentSource(s),dest:shipmentDest(s),rowLabel:'漁協',
 rows:ls=>{const order=['東部漁協','昆布森漁協','厚岸漁協','散布漁協','浜中漁協'];const got=v53Unique(ls.map(l=>l.coop));return [...order.filter(x=>got.includes(x)),...got.filter(x=>!order.includes(x))]},
 cols:ls=>v53Unique(ls.map(l=>[l.season,l.group,l.item].filter(Boolean).join('・'))),
...
hOpenShipPdf=function(s){if(!s)return;
 v53MatrixShipmentPreview({title:'日高昆布 出荷指示',reportTitle:'日 高 昆 布',
 shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'産地',
 rows:ls=>v53Unique(ls.map(l=>l.location)),cols:ls=>v53Unique(ls.map(l=>[l.section,l.grade].filter(Boolean).join('・'))),
...
nOpenShipPdf=function(s){if(!s)return;
 v53MatrixShipmentPreview({title:'根室産昆布 出荷指示',reportTitle:'根 室 産 昆 布',
 shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'漁協',
 rows:ls=>v53Unique(ls.map(l=>l.coop)),cols:ls=>v53Unique(ls.map(l=>[l.season,l.group,l.item].filter(Boolean).join('・'))),
 rowKey:l=>l.coop||'',colKey:l=>[l.season,l.group,l.item].filter(Boolean).join('・'),
 back:()=>nShipDetail(s.id)});
};
...
if(typeof spOpenShipPdf==='function'){
 const v53OldSp=spOpenShipPdf;
 spOpenShipPdf=function(s){
   if(!s)return v53OldSp(s);
   v53MatrixShipmentPreview({title:'釧路産棹前昆布 出荷指示',reportTitle:'釧 路 産 棹 前 昆 布',
   shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'漁協',
   rows:ls=>v53Unique(ls.map(l=>l.coop||l.location)),cols:ls=>v53Unique(ls.map(l=>[l.season,l.group,l.item,l.grade].filter(Boolean).join('・'))),
...
/* ===== v53 出荷指示書PDFを在庫集計表PDFと完全に同じ表レイアウトへ =====
   在庫集計表Canvas関数をそのまま使用する。
   出荷数量だけを一時的に在庫レコードとして渡すため、
   0数量の行・列もマスター定義どおり全て表示される。
*/
function v53RetitleStockCanvas(canvas, title, year, shipment, topClear){
...
function v53CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    return v53RetitleStockCanvas(_stockCanvasPage(y),'釧 路 産 昆 布　出 荷 指 示 書',y,s,100);
  }finally{state.records=rec;state.shipments=ships}
}
function v53CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    return v53RetitleStockCanvas(hStockCanvas(y),'日 高 昆 布　出 荷 指 示 書',y,s,74);
  }finally{hState.records=rec;hState.shipments=ships}
}
function v53CanvasNemuro(s,y){
  ...
  try{
    ...
    nState.shipments=[];
    return v53RetitleStockCanvas(nReportCanvas(y,null),'根 室 産 昆 布　出 荷 指 示 書',y,s,64);
  }finally{nState.records=rec;nState.shipments=ships}
}
function v53CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    return v53RetitleStockCanvas(smReportCanvas(y,null),'釧 路 産 棹 前 昆 布　出 荷 指 示 書',y,s,64);
  }finally{smState.records=rec;smState.shipments=ships}
}
function v53ShowShipmentCanvasPreview(title, shipment, canvases, backFn){
  const oldHeader=document.querySelector('header h1')?.textContent||'昆布在庫管理';
  setHeader(title);
  app.innerHTML=`
   <style>
    ...
    .v53bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}
    .v53print{background:#fff;color:#173661}.v53back{background:#dfe7f1;color:#173661}
    .v53note{background:#fff3bf;color:#5c4b00;padding:8px 12px;text-align:center;font-size:13px}
    .v53preview{overflow:auto;background:#eef2f6;padding:8px}
    .v53page{width:281mm;margin:0 auto 10px;background:#fff;box-shadow:0 2px 8px #0002}
    .v53page canvas{display:block;width:281mm;height:auto;background:#fff}
    @media print{
      @page{size:A4 landscape;margin:0}
      header,nav,.v53bar,.v53note{display:none!important}
      main{padding:0!important;max-width:none!important}
      .v53preview{overflow:visible;background:#fff;padding:0}
      .v53page{width:297mm;height:210mm;margin:0;box-shadow:none;page-break-after:always;display:flex;align-items:flex-start;justify-content:center}
      .v53page:last-child{page-break-after:auto}
      .v53page canvas{width:297mm;height:210mm;object-fit:contain}
    }
   </style>
   <div class="v53bar"><button class="v53print" id="v53print">PDF・印刷・FAXへ</button><button class="v53back" id="v53back">出荷指示へ戻る</button></div>
   <div class="v53note">在庫集計表PDFと同じ固定行・固定列です。数量0の行・列も表示し、0のセルだけ空欄にしています。</div>
   <div class="v53preview" id="v53preview"></div>`;
  const preview=document.getElementById('v53preview');
  canvases.forEach(c=>{
    const d=document.createElement('div');d.className='v53page';d.appendChild(c);preview.appendChild(d);
  });
  document.getElementById('v53print').onclick=()=>window.print();
  document.getElementById('v53back').onclick=()=>{setHeader(oldHeader);backFn()};
}

/* 釧路産昆布 */
openShipmentPdfDirect=function(id){
  const s=state.shipments.find(x=>x.id===id);if(!s)return;
  const ys=v53ShipmentYears(s,state.activeYear);
  v53ShowShipmentCanvasPreview('釧路産昆布 出荷指示',s,ys.map(y=>v53CanvasKushiro(s,y)),()=>shipmentDetail(s.id));
};
/* 日高昆布 */
hOpenShipPdf=function(s){
  if(!s)return;
  const ys=v53ShipmentYears(s,hState.activeYear);
  v53ShowShipmentCanvasPreview('日高昆布 出荷指示',s,ys.map(y=>v53CanvasHidaka(s,y)),()=>hShipDetail(s.id));
};
/* 根室産昆布 */
nOpenShipPdf=function(s){
  if(!s)return;
  const ys=v53ShipmentYears(s,nState.activeYear);
  v53ShowShipmentCanvasPreview('根室産昆布 出荷指示',s,ys.map(y=>v53CanvasNemuro(s,y)),()=>nShipDetail(s.id));
};
/* 釧路産棹前昆布 */
smOpenShipPdf=function(s){
  if(!s)return;
  const ys=v53ShipmentYears(s,smState.activeYear);
  v53ShowShipmentCanvasPreview('釧路産棹前昆布 出荷指示',s,ys.map(y=>v53CanvasSanmae(s,y)),()=>smShipDetail(s.id));
};
