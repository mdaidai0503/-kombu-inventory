...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, tableY, tableX){
  ...
  /* v137: 「出荷依頼書(○○昆布)」を隙間なく1組として完全中央寄せ。年産は真下中央。 */
  const productLabel=productName?`(${productName})`:'';
  x.font=font(42,true);const mainW=x.measureText(mainTitle).width;
  x.font=font(30,true);const productW=x.measureText(productLabel).width;
  const titleStart=(W-mainW-productW)/2;
  text(mainTitle,titleStart,28,42,'left',true);
  if(productLabel)text(productLabel,titleStart+mainW,28,30,'left',true);
  text(`(${year}年産)`,W/2,62,27,'center',true);
  text(`依頼番号：${shipment?.id||''}　出荷日：${shipment?.shipDate||''}`,W-tableX,18,12,'right',false);

  const boxY=84,boxH=145,boxW=(W-tableX*2)/2;
