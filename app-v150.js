...
(function(){
  ...
  window.v150OpenItemsPdf=openItemsPdf;
  ...
  function faxBoxPage(){
    ...
    const cards=[...groups.entries()].map(([g,items],gi)=>{
      ...
      const rows=sorted.map(it=>`<tr data-fax-row="${esc(it.key)}">
        <td class="v150-check"><input type="checkbox" class="v150-fax-check" data-key="${esc(it.key)}"></td>
      ...
      return `<section class="card v150-fax-card" style="margin-top:12px;padding:14px" data-fax-card="${gi}"><div class="row" style="align-items:flex-start"><div><h3 style="margin:0 0 3px">${esc(d.name||'送信先未設定')}</h3></div><span class="pill">${items.length}件</span></div>
        <div class="tablewrap" style="margin-top:10px;overflow:auto"><table class="v150-compact-table"><colgroup><col class="c0"><col class="c1"><col class="c2"><col class="c3"><col class="c4"><col class="c5"><col class="c6"><col class="c7"></colgroup><thead><tr><th><input type="checkbox" class="v150-fax-all" aria-label="全選択"></th><th>依頼日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>削除</th><th>番号</th></tr></thead><tbody>${rows}</tbody></table></div>
    ...
    app.querySelectorAll('.v150-fax-all').forEach(all=>all.onchange=()=>{const card=all.closest('.v150-fax-card');card.querySelectorAll('.v150-fax-check').forEach(c=>c.checked=all.checked)});
    app.querySelectorAll('[data-fax-checked]').forEach(b=>b.onclick=async()=>{
      const card=b.closest('.v150-fax-card'),keys=[...card.querySelectorAll('.v150-fax-check:checked')].map(c=>c.dataset.key),map=new Map(box.map(x=>[x.key,x])),items=keys.map(k=>map.get(k)).filter(Boolean);
...
(function(){
  ...
  function v114UnifiedShipmentForm(){
    ...
    app.innerHTML=`<section class="card v113-ship-form"><h2>📦 新規出荷依頼</h2><div class="form"><datalist id="companyNames">${companyDatalist()}</datalist>
      <div class="v113-date-row"><label class="v150-date-box">依頼日<input id="v114ShipDate" type="date" value="${today()}"></label><label class="v150-date-box">着希望日<input id="v114ArrivalDate" type="date" value=""></label></div>
      ...
      <div class="v113-lines-wrap"><div class="v113-lines-title">明細</div><div id="v114Lines"></div>
        ...
        <button class="btn v150-draft-save" id="v114Save">💾 下書き保存</button>
      </div>
    </div></section>`;
  ...
  function ensureShipmentNav(){
    ...
    if(!nav){
      ...
      nav.querySelector('#v119Back').onclick=()=>{
        /* v150: DOM上の実画面を優先して判定。screenKindが古い状態でも
        ...
        const onHistory=!!document.querySelector('.v150-history-card,.v150-history-card');
...
(function(){
  ...
  function shipmentHistory(){
    ...
    app.innerHTML=`<section class="card v150-history-card v150-history-title-marker" style="margin-top:14px;padding:12px">
      <div class="tablewrap" style="overflow:auto">
        <table class="v150-compact-table v150-history-table">
    ...
    const selects=[...document.querySelectorAll('.v150-filter-row select[data-col]')];
    const stateFilter={};
    let sortCol='',sortDir='';
...
/* ===== v150: compact selectable FAXBOX/history tables ===== */
(function(){
 const st=document.createElement('style');st.textContent=`
 .v150-compact-table{min-width:720px;width:100%;font-size:11px;table-layout:fixed}
 .v150-compact-table th,.v150-compact-table td{padding:7px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center}
 .v150-compact-table .c0{width:30px}.v150-compact-table .c1{width:82px}.v150-compact-table .c2{width:48px}.v150-compact-table .c3{width:105px}.v150-compact-table .c4{width:105px}.v150-compact-table .c5{width:44px}.v150-compact-table .c6{width:50px}.v150-compact-table .c7{width:66px}
 .v150-compact-table input[type=checkbox]{width:18px;height:18px;margin:0;vertical-align:middle}
 `;document.head.appendChild(st);
})();
/* ===== /v150 ===== */


/* ===== v150: shipment history filters ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 body:has(.v150-history-card) header h1:empty{display:none}
 body:has(.v150-history-card) header:has(h1:empty){min-height:18px;padding-top:0;padding-bottom:0}
 .v150-history-card{margin-bottom:calc(120px + env(safe-area-inset-bottom))}
 .v150-history-table{min-width:680px}
 .v150-history-table th,.v150-history-table td{padding:6px 3px;font-size:10.5px}
 .v150-history-table col.c1{width:76px}
 .v150-history-table col.c2{width:44px}
 .v150-history-table col.c3{width:92px}
 .v150-history-table col.c4{width:92px}
 .v150-history-table col.c5{width:40px}
 .v150-history-table col.c6{width:44px}
 .v150-history-table col.c7{width:58px}
 .v150-filter-row th{padding:3px 2px;background:#f4f7fb}
 .v150-filter-row select{width:100%;min-width:0;height:28px;padding:1px 14px 1px 3px;font-size:9.5px;border:1px solid #cbd5e1;border-radius:5px;background:#fff;color:#17324f}
 .v150-filter-row select:disabled{opacity:.45}
 `;
 document.head.appendChild(st);
})();
/* ===== /v150 ===== */
...
/* ===== v150: shipment request UI refinements ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 /* unified shipment form */
 .v150-draft-save{margin-top:10px!important;width:100%!important}
 .v150-qty-label{display:flex!important;flex-direction:column!important;align-items:stretch!important}
 .v150-avail{display:block!important;margin-top:6px!important;text-align:left!important;font-size:11px!important;line-height:1.2!important;color:#e7eef8!important;white-space:nowrap!important}
 .v150-delete{font-size:12px!important;padding:6px 9px!important;min-width:56px!important;min-height:38px!important;border-radius:10px!important}
 ...
 body:has(.v150-history-title-marker) header{background:#0b2b55!important;color:#fff!important}
 .v150-history-table{min-width:560px!important;width:100%!important}
 .v150-history-table th,.v150-history-table td{padding:5px 2px!important;font-size:9.6px!important}
 .v150-history-table col.c1{width:66px!important}
 .v150-history-table col.c2{width:38px!important}
 .v150-history-table col.c3{width:76px!important}
 .v150-history-table col.c4{width:76px!important}
 .v150-history-table col.c5{width:34px!important}
 .v150-history-table col.c6{width:38px!important}
 .v150-history-table col.c7{width:50px!important}
 .v150-filter-row select{height:26px!important;font-size:8.8px!important;padding-left:2px!important;padding-right:12px!important}
 `;
 document.head.appendChild(st);
})();
/* ===== /v150 ===== */
...
/* ===== v150: back navigation / delete confirm / date styling ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v113-date-row .v150-date-box{
   display:flex!important;
   flex-direction:column!important;
   gap:6px!important;
 ...
 .v113-date-row .v150-date-box input{
   width:100%!important;
   box-sizing:border-box!important;
   background:#fff!important;
 ...
 @media(max-width:430px){
   .v113-date-row .v150-date-box{padding:8px!important;font-size:12px!important}
   .v113-date-row .v150-date-box input{font-size:12.5px!important;padding:8px 6px!important}
   .v150-delete{font-size:9.5px!important;min-width:42px!important;min-height:30px!important;padding:3px 6px!important}
 }
 `;
 document.head.appendChild(st);
})();
