...
(function(){
  ...
  async function openItemsPdf(items){
    if(!items||!items.length)return alert('PDF/FAXにする項目をチェックしてください。');
    const w=window.open('about:blank','_blank');if(!w)return alert('PDF画面を開けません。Safariのポップアップ設定を確認してください。');
    try{
      w.document.write('<meta name="viewport" content="width=device-width,initial-scale=1"><div style="font-family:-apple-system;padding:30px;text-align:center"><h3>FAX用PDFをまとめています…</h3><p>そのままお待ちください。</p></div>');w.document.close();
  ...
  window.v146OpenItemsPdf=openItemsPdf;
  ...
  function faxBoxPage(){
    ...
    const cards=[...groups.entries()].map(([g,items],gi)=>{
      ...
      const rows=sorted.map(it=>`<tr data-fax-row="${esc(it.key)}">
        <td class="v146-check"><input type="checkbox" class="v146-fax-check" data-key="${esc(it.key)}"></td>
      ...
      return `<section class="card v146-fax-card" style="margin-top:12px;padding:14px" data-fax-card="${gi}"><div class="row" style="align-items:flex-start"><div><h3 style="margin:0 0 3px">${esc(d.name||'送信先未設定')}</h3></div><span class="pill">${items.length}件</span></div>
        <div class="tablewrap" style="margin-top:10px;overflow:auto"><table class="v146-compact-table"><colgroup><col class="c0"><col class="c1"><col class="c2"><col class="c3"><col class="c4"><col class="c5"><col class="c6"><col class="c7"></colgroup><thead><tr><th><input type="checkbox" class="v146-fax-all" aria-label="全選択"></th><th>依頼日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>削除</th><th>番号</th></tr></thead><tbody>${rows}</tbody></table></div>
    ...
    app.querySelectorAll('.v146-fax-all').forEach(all=>all.onchange=()=>{const card=all.closest('.v146-fax-card');card.querySelectorAll('.v146-fax-check').forEach(c=>c.checked=all.checked)});
    app.querySelectorAll('[data-fax-checked]').forEach(b=>b.onclick=()=>{const card=b.closest('.v146-fax-card'),keys=[...card.querySelectorAll('.v146-fax-check:checked')].map(c=>c.dataset.key),map=new Map(box.map(x=>[x.key,x]));openItemsPdf(keys.map(k=>map.get(k)).filter(Boolean))});
...
(function(){
  ...
  function shipmentHistory(){
    ...
    const rows=hist.map(it=>`<tr data-hprod="${it.product}" data-hid="${esc(it.id||'')}" data-hkey="${esc(it.key)}">
      <td class="v146-check"><input type="checkbox" class="v146-hist-check" data-key="${esc(it.key)}"></td><td>${esc(it.shipDate||'')}</td><td><b>${esc(label(it.product))}</b></td>
      <td>${esc(it.source?.name||'')}</td><td>${esc(it.dest?.name||'')}</td><td style="text-align:center">${fmt(it.qty||0)}</td><td><button class="mini" data-hopen="1">開く</button></td><td>${esc(it.id||'')}</td>
    </tr>`).join('')||'<tr><td colspan="8" class="empty">出荷指示履歴はありません</td></tr>';
    app.innerHTML=`<section class="card" style="margin-top:14px;padding:14px">
      ...
      <div class="tablewrap" style="margin-top:10px;overflow:auto"><table class="v146-compact-table"><colgroup><col class="c0"><col class="c1"><col class="c2"><col class="c3"><col class="c4"><col class="c5"><col class="c6"><col class="c7"></colgroup>
      <thead><tr><th><input type="checkbox" id="v146HistAll" aria-label="全選択"></th><th>依頼日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>開く</th><th>番号</th></tr></thead>
      <tbody id="v136HistBody">${rows}</tbody></table></div>
      ${hist.length?'<button class="btn" id="v146HistPdf" style="margin-top:10px">📄 まとめてPDF/FAX</button>':''}</section>`;
    ...
    const all=document.getElementById('v146HistAll');if(all)all.onchange=()=>document.querySelectorAll('.v146-hist-check').forEach(c=>c.checked=all.checked);
    const pdf=document.getElementById('v146HistPdf');if(pdf)pdf.onclick=()=>{const keys=[...document.querySelectorAll('.v146-hist-check:checked')].map(c=>c.dataset.key),map=new Map(hist.map(x=>[x.key,x])),items=keys.map(k=>map.get(k)).filter(Boolean);if(window.v146OpenItemsPdf)window.v146OpenItemsPdf(items)};
...
/* ===== v146: compact selectable FAXBOX/history tables ===== */
(function(){
 const st=document.createElement('style');st.textContent=`
 .v146-compact-table{min-width:720px;width:100%;font-size:11px;table-layout:fixed}
 .v146-compact-table th,.v146-compact-table td{padding:7px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center}
 .v146-compact-table .c0{width:30px}.v146-compact-table .c1{width:82px}.v146-compact-table .c2{width:48px}.v146-compact-table .c3{width:105px}.v146-compact-table .c4{width:105px}.v146-compact-table .c5{width:44px}.v146-compact-table .c6{width:50px}.v146-compact-table .c7{width:66px}
 .v146-compact-table input[type=checkbox]{width:18px;height:18px;margin:0;vertical-align:middle}
 `;document.head.appendChild(st);
})();
/* ===== /v146 ===== */
