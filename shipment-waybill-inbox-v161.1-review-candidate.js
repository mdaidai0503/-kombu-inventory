...
(function () {
  ...
  function scoredCandidatesFromWaybill(waybill) {
    ...
    function pushCandidate(x, block, blockIndex) {
      if (!x) return;
      const manualReviewCandidate = x.__manual_review === true;
      if (!manualReviewCandidate && Number(x.score) < 90) return;
    ...
    // v161.1:
    // matcher が review_candidates / candidate_combinations を返した場合は、
    // それを最優先の手動添付候補として利用する。
    // 日付安全除外後に1候補だけ残ったケースでも、ユーザーが確認して選択できる。
    if (!rows.length) {
      const reviewSources = [];

      if (Array.isArray(match.review_candidates)) {
        reviewSources.push(...match.review_candidates);
      }
      ...
      if (
        ...
        reviewSources.push(...match.multiple_match.candidate_combinations);
      }

      reviewSources.forEach(function (entry, idx) {
        const shipments = Array.isArray(entry && entry.shipments)
          ? entry.shipments
          : [];

        shipments.forEach(function (x) {
          pushCandidate(
            Object.assign({}, x, { __manual_review: true }),
            {
              block_index: idx + 1,
              total_qty: Number(entry && entry.total_qty || x && x.qty || 0),
              product_name: '要確認候補'
            },
            idx + 1
          );
  ...
  function mergeScoredCandidate(scoreRow, shipment) {
    return Object.assign({}, shipment || {}, {
      ...
      __line_qty: Number(scoreRow.line_qty || scoreRow.qty || 0),
      __block_index: Number(scoreRow.__block_index || 1),
      __block_product_name: scoreRow.__block_product_name || '',
      __block_qty: Number(scoreRow.__block_qty || scoreRow.qty || 0),
      __manual_review: scoreRow.__manual_review === true
    });
  }

  function candidateOptionLabel(s) {
    const score = Number(s && s.__waybill_score);
    const prefix = s && s.__manual_review
      ? '【要確認候補】'
      : (
