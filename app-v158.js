...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, tableY, tableX){
  ...
  /* v158: 出荷元/出荷先の下に配送情報・備考欄を追加するため表を少し下へ移動。
  ...
  /* v158: 出荷先下＝着希望日／配送・袋入等（2分割）、出荷元下＝備考 */
...
function v76ShipmentMenu(){
  ...
  app.innerHTML=`
    ...
    <section class="card" id="v158HiddenProductHeader" style="display:none!important">
...
(function(){
  ...
  window.v158OpenItemsPdf=openItemsPdf;
  ...
  function faxBoxPage(){
    ...
    const cards=[...groups.entries()].map(([g,items],gi)=>{
      ...
      const rows=sorted.map(it=>`<tr data-fax-row="${esc(it.key)}">
        <td class="v158-check"><input type="checkbox" class="v158-fax-check" data-key="${esc(it.key)}"></td>
      ...
      return `<section class="card v158-fax-card" style="margin-top:12px;padding:14px" data-fax-card="${gi}"><div class="row" style="align-items:flex-start"><div><h3 style="margin:0 0 3px">${esc(d.name||'送信先未設定')}</h3></div><span class="pill">${items.length}件</span></div>
        <div class="tablewrap" style="margin-top:10px;overflow:auto"><table class="v158-compact-table"><colgroup><col class="c0"><col class="c1"><col class="c2"><col class="c3"><col class="c4"><col class="c5"><col class="c6"><col class="c7"></colgroup><thead><tr><th><input type="checkbox" class="v158-fax-all" aria-label="全選択"></th><th>依頼日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>削除</th><th>番号</th></tr></thead><tbody>${rows}</tbody></table></div>
    ...
    app.querySelectorAll('.v158-fax-all').forEach(all=>all.onchange=()=>{const card=all.closest('.v158-fax-card');card.querySelectorAll('.v158-fax-check').forEach(c=>c.checked=all.checked)});
    app.querySelectorAll('[data-fax-checked]').forEach(b=>b.onclick=async()=>{
      const card=b.closest('.v158-fax-card'),keys=[...card.querySelectorAll('.v158-fax-check:checked')].map(c=>c.dataset.key),map=new Map(box.map(x=>[x.key,x])),items=keys.map(k=>map.get(k)).filter(Boolean);
...
(function(){
  ...
  function v114UnifiedShipmentForm(editProduct=null,editId=null,preset=null){
    ...
    const source0=existingSrc||{name:'㈱浜中運輸',address:'',phone:''};
    app.innerHTML=`<section class="card v113-ship-form"><h2 class="v158-form-title">📦 ${editing?'出荷依頼修正':'新規出荷依頼'}</h2><div class="form"><datalist id="companyNames">${companyDatalist()}</datalist>
      <div class="v158-top-columns">
        <div class="v158-top-stack">
          <label class="v158-top-box v158-date-box">依頼日
            <input id="v114ShipDate" type="date" value="${existing?.shipDate||today()}">
          </label>
          <label class="v158-top-box">配送・袋入等
            <select id="v114DeliveryPack">
              <option value="" ${!existing?.deliveryPack?'selected':''}>　</option>
              <option value="ビニール袋入り" ${existing?.deliveryPack==='ビニール袋入り'?'selected':''}>ビニール袋入り</option>
              <option value="コンテナ対応" ${existing?.deliveryPack==='コンテナ対応'?'selected':''}>コンテナ対応</option>
            </select>
          </label>
        </div>
        <div class="v158-top-stack">
          <label class="v158-top-box v158-date-box">着希望日
            <input id="v114ArrivalDate" type="date" value="${existing?.arrivalDate||''}">
          </label>
          <label class="v158-top-box">備考
            <input id="v114Memo" type="text" placeholder="自由入力" value="${esc(existing?.memo||'')}">
          </label>
        </div>
      </div>
      <div class="v113-party-row"><div class="v113-darkbox"><h3>出荷元</h3><label>会社名<input id="v114SourceName" list="companyNames" value="${esc(source0.name)}"></label><label>住所<input id="v114SourceAddress" value="${esc(existingSrc?.address||'')}"></label><label>電話<input id="v114SourcePhone" value="${esc(existingSrc?.phone||'')}"></label></div><div class="v113-darkbox"><h3>出荷先</h3><label>会社名<input id="v114DestName" list="companyNames" value="${esc(existingDst?.name||'')}"></label><label>住所<input id="v114DestAddress" value="${esc(existingDst?.address||'')}"></label><label>電話<input id="v114DestPhone" value="${esc(existingDst?.phone||'')}"></label></div></div>
      <div class="v113-lines-wrap"><div class="v113-lines-title">明細</div><div id="v114Lines"></div>
        <button class="btn secondary" id="v114AddLine">➕ 明細追加</button>
        <button class="btn v158-draft-save" id="v114Save">💾 ${editing?'修正保存':'下書き保存'}</button>
      </div>
    </div></section>`;
    ...
    byId('v114Save').onclick=()=>{
      ...
      if(editing){
        ...
        if(editProduct==='kushiro'){
          Object.assign(existing,{source,destInfo:dest,dest:dest.name,shipDate:common.shipDate,arrivalDate:common.arrivalDate,deliveryPack:common.deliveryPack,memo:common.memo,updatedAt:common.updatedAt,lines:cleanLines});
        }else{
          Object.assign(existing,{source,dest,shipDate:common.shipDate,arrivalDate:common.arrivalDate,deliveryPack:common.deliveryPack,memo:common.memo,updatedAt:common.updatedAt,lines:cleanLines});
        }
        editStores[editProduct].save();
        alert('出荷依頼を修正しました。');
        openGlobalShipment(editProduct,editId);
        return;
      }
    ...
    if(!lines.length){
      const pp=preset&&productDefs[preset.product]?preset.product:'kushiro';
      const first=productDefs[pp].make();
      first.product=pp;
      if(preset&&preset.year)first.year=preset.year;
      lines.push(first);
    }
    render();
  }
  ...
  function ensureShipmentNav(){
    ...
    if(!nav){
      ...
      nav.querySelector('#v119Back').onclick=()=>{
        /* v158: DOM上の実画面を優先して判定。screenKindが古い状態でも
        ...
        const onHistory=!!document.querySelector('.v158-history-card,.v158-history-card');
...
(function(){
  ...
  function renderTopNav(){
    ...
    if(!nav){
      ...
      nav.innerHTML=`<button id="v120Backup" aria-label="バックアップ" title="バックアップ"><span class="v158-top-icon">💾</span><span class="v158-top-label">バックアップ</span></button><button id="v120Restore" aria-label="バックアップ復元" title="バックアップ復元"><span class="v158-top-icon">♻️</span><span class="v158-top-label">復元</span></button><button id="v120Company" aria-label="会社マスター" title="会社マスター"><span class="v158-top-icon">🏢</span><span class="v158-top-label">会社マスター</span></button>`;
      document.body.appendChild(nav);
...
(function(){
  ...
  function shipmentHistory(){
    ...
    app.innerHTML=`<section class="card v158-history-card v158-history-title-marker" style="margin-top:14px;padding:12px">
      <div class="tablewrap" style="overflow:auto">
        <table class="v158-compact-table v158-history-table">
    ...
    const selects=[...document.querySelectorAll('.v158-filter-row select[data-col]')];
    const stateFilter={};
    let sortCol='',sortDir='';
...
/* ===== v158: compact selectable FAXBOX/history tables ===== */
(function(){
 const st=document.createElement('style');st.textContent=`
 .v158-compact-table{min-width:720px;width:100%;font-size:11px;table-layout:fixed}
 .v158-compact-table th,.v158-compact-table td{padding:7px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center}
 .v158-compact-table .c0{width:30px}.v158-compact-table .c1{width:82px}.v158-compact-table .c2{width:48px}.v158-compact-table .c3{width:105px}.v158-compact-table .c4{width:105px}.v158-compact-table .c5{width:44px}.v158-compact-table .c6{width:50px}.v158-compact-table .c7{width:66px}
 .v158-compact-table input[type=checkbox]{width:18px;height:18px;margin:0;vertical-align:middle}
 `;document.head.appendChild(st);
})();
/* ===== /v158 ===== */


/* ===== v158: shipment history filters ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 body:has(.v158-history-card) header h1:empty{display:none}
 body:has(.v158-history-card) header:has(h1:empty){min-height:18px;padding-top:0;padding-bottom:0}
 .v158-history-card{margin-bottom:calc(120px + env(safe-area-inset-bottom))}
 .v158-history-table{min-width:680px}
 .v158-history-table th,.v158-history-table td{padding:6px 3px;font-size:10.5px}
 .v158-history-table col.c1{width:76px}
 .v158-history-table col.c2{width:44px}
 .v158-history-table col.c3{width:92px}
 .v158-history-table col.c4{width:92px}
 .v158-history-table col.c5{width:40px}
 .v158-history-table col.c6{width:44px}
 .v158-history-table col.c7{width:58px}
 .v158-filter-row th{padding:3px 2px;background:#f4f7fb}
 .v158-filter-row select{width:100%;min-width:0;height:28px;padding:1px 14px 1px 3px;font-size:9.5px;border:1px solid #cbd5e1;border-radius:5px;background:#fff;color:#17324f}
 .v158-filter-row select:disabled{opacity:.45}
 `;
 document.head.appendChild(st);
})();
/* ===== /v158 ===== */
...
/* ===== v158: shipment request UI refinements ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 /* unified shipment form */
 .v158-draft-save{margin-top:10px!important;width:100%!important}
 .v158-qty-label{display:flex!important;flex-direction:column!important;align-items:stretch!important}
 .v158-avail{display:block!important;margin-top:6px!important;text-align:left!important;font-size:11px!important;line-height:1.2!important;color:#e7eef8!important;white-space:nowrap!important}
 .v158-delete{font-size:12px!important;padding:6px 9px!important;min-width:56px!important;min-height:38px!important;border-radius:10px!important}
 ...
 body:has(.v158-history-title-marker) header{background:#0b2b55!important;color:#fff!important}
 .v158-history-table{min-width:560px!important;width:100%!important}
 .v158-history-table th,.v158-history-table td{padding:5px 2px!important;font-size:9.6px!important}
 .v158-history-table col.c1{width:66px!important}
 .v158-history-table col.c2{width:38px!important}
 .v158-history-table col.c3{width:76px!important}
 .v158-history-table col.c4{width:76px!important}
 .v158-history-table col.c5{width:34px!important}
 .v158-history-table col.c6{width:38px!important}
 .v158-history-table col.c7{width:50px!important}
 .v158-filter-row select{height:26px!important;font-size:8.8px!important;padding-left:2px!important;padding-right:12px!important}
 `;
 document.head.appendChild(st);
})();
/* ===== /v158 ===== */
...
/* ===== v158: back navigation / delete confirm / date styling ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v113-date-row .v158-date-box{
   display:flex!important;
   flex-direction:column!important;
   gap:6px!important;
 ...
 .v113-date-row .v158-date-box input{
   width:100%!important;
   box-sizing:border-box!important;
   background:#fff!important;
 ...
 @media(max-width:430px){
   .v113-date-row .v158-date-box{padding:8px!important;font-size:12px!important}
   .v113-date-row .v158-date-box input{font-size:12.5px!important;padding:8px 6px!important}
   .v158-delete{font-size:9.5px!important;min-width:42px!important;min-height:30px!important;padding:3px 6px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
(function(){
 ...
 .v158-date-stack{display:flex!important;flex-direction:column!important;gap:10px!important;min-width:0!important}
 .v158-date-stack .v158-date-box{width:100%!important;min-width:0!important;overflow:hidden!important}
 .v158-date-stack .v158-date-box input{display:block!important;max-width:100%!important;min-width:0!important;margin:0!important}
 .v158-extra-box{
 ...
 .v158-extra-box input,.v158-extra-box select{
 ...
 @media(max-width:430px){
   .v158-extra-box{padding:8px!important;font-size:12px!important}
   .v158-extra-box input,.v158-extra-box select{font-size:12.5px!important;padding:8px 6px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
/* ===== v158: compact top fields + unified edit form ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v158-form-title{text-align:center!important;width:100%!important}
 .v158-top-grid{
 ...
 .v158-top-box input,.v158-top-box select{
   display:block!important;width:100%!important;max-width:100%!important;min-width:0!important;
   box-sizing:border-box!important;margin:0!important;
   background:#fff!important;color:#102a43!important;border:1px solid #d4dde8!important;
 ...
 .v158-date-box{padding-top:7px!important;padding-bottom:7px!important}
 .v158-date-box input{height:40px!important;padding-top:5px!important;padding-bottom:5px!important}
 .v158-top-box select{
   -webkit-appearance:auto!important;appearance:auto!important;
 }
 @media(max-width:430px){
   .v158-top-grid{gap:8px!important}
   .v158-top-box{padding:7px 8px!important;font-size:12px!important}
   .v158-top-box input,.v158-top-box select{height:39px!important;font-size:12.5px!important;padding:5px 7px!important}
   .v158-date-box input{height:37px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
/* ===== v158: v151-style top input arrangement ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v158-top-columns{
 ...
 .v158-top-stack .v158-top-box{
   width:100%!important;
   min-width:0!important;
   box-sizing:border-box!important;
 }
 @media(max-width:430px){
   .v158-top-columns{gap:8px!important}
   .v158-top-stack{gap:8px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
/* ===== v158: inventory stock nav + labeled home nav ===== */
(function(){
  const st=document.createElement('style');
  st.textContent=`
    #v120TopNav button{
    ...
    #v120TopNav .v158-top-icon{
      display:block!important;
      font-size:22px!important;
      line-height:24px!important;
      font-weight:400!important;
    }
    #v120TopNav .v158-top-label{
      display:block!important;
      font-size:10px!important;
    ...
    @media(max-width:430px){
      ...
      #v120TopNav .v158-top-icon{font-size:21px!important;line-height:23px!important}
      #v120TopNav .v158-top-label{font-size:9.5px!important;line-height:12px!important}
    }
  `;
  document.head.appendChild(st);
...
/* ===== v158: direct inventory landing + simplified Other ===== */
(function(){
  ...
  /* 「その他」はPDF一括入庫だけに統一。 */
  function v158OtherPage(){
    v80InventoryMode=true;
    setNavVisible(true);
    if(typeof bindNav==='function')bindNav();
    app.innerHTML=`
      <section class="card v158-other-card" style="margin-top:22px">
        <button class="btn v158-bulk-btn" id="v158BulkPdfBtn">📄 PDFから4種類を一括入庫</button>
        <input id="v158BulkPdfFile" type="file" accept="application/pdf,.pdf" hidden>
      </section>`;
    const btn=document.getElementById('v158BulkPdfBtn');
    const inp=document.getElementById('v158BulkPdfFile');
    if(btn&&inp){
  ...
  globalThis.exportsPage=v158OtherPage;
  globalThis.hMore=v158OtherPage;
  globalThis.nMore=v158OtherPage;
  globalThis.smMore=v158OtherPage;

  /* 下部ナビの「その他」も現在の昆布に関係なく共通画面へ。 */
  const baseBind=globalThis.bindNav;
  globalThis.bindNav=function(){
    if(typeof baseBind==='function')baseBind.apply(this,arguments);
    if(moreBtnEl)moreBtnEl.onclick=v158OtherPage;
  };

  const style=document.createElement('style');
  style.textContent=`
    .v158-other-card{padding:18px!important}
    .v158-bulk-btn{
      width:100%!important;
      background:#0b2b55!important;
      color:#fff!important;
...
/* ===== v158: stock-header shipment button + inventory back navigation ===== */
(function(){
  let backStack=[];
  ...
  function injectStockShipmentButton(){
    ...
    let b=card.querySelector('#v158StockShip');
    if(!b){
      b=document.createElement('button');
      b.id='v158StockShip';b.type='button';
      b.className='btn v158-stock-ship-btn';
      b.innerHTML='<span aria-hidden="true">📦</span><span class="v158-ship-label">出荷依頼</span>';
  ...
  css.textContent=`
    ...
    .v158-stock-ship-btn>span:first-child{font-size:16px!important;line-height:1!important}
    .v158-ship-label{font-size:11px!important;white-space:nowrap!important}
    body[data-v119-nav-mode="inventory"] > nav.v119-standard-nav button{min-width:16.4%!important}
    @media(max-width:430px){
      .v158-stock-ship-btn{min-width:56px!important;padding:6px 7px!important}
      .v158-stock-ship-btn>span:first-child{font-size:15px!important}
      .v158-ship-label{font-size:10px!important}
    }
  `;
  document.head.appendChild(css);
...
/* ===== v158: reliable per-product history + atomic 4-product bulk import UI ===== */
(function(){
  ...
  function renderHistory(p){
    ...
    app.innerHTML=`<section class="card v158-history-card">
      <h2>入出庫履歴</h2>
      <div class="v158-history-caption">${labels[p]}の履歴　${a.length}件</div>
      <div class="tablewrap"><table class="v158-history-table">
        ...
        <tbody>${a.map(x=>`<tr>
          ...
          <td><button class="mini" data-v158-edit="${esc(x.raw.id)}">修正</button> <button class="mini danger" data-v158-del="${esc(x.raw.id)}">削除</button></td>
        </tr>`).join('')}</tbody>
      </table></div>
    </section>`;
    app.querySelectorAll('[data-v158-edit]').forEach(b=>b.onclick=()=>editRecord(p,b.dataset.v158Edit));
    app.querySelectorAll('[data-v158-del]').forEach(b=>b.onclick=()=>deleteRecord(p,b.dataset.v158Del));
    if(typeof bindNav==='function')bindNav();
    if(typeof v80InjectInventorySwitcher==='function')requestAnimationFrame(v80InjectInventorySwitcher);
  }
  ...
  const st=document.createElement('style');
  st.textContent=`
    .v158-history-card{padding:15px!important}
    .v158-history-card h2{margin:0 0 8px!important}
    .v158-history-caption{font-size:12px;color:#647b95;margin-bottom:8px}
    .v158-history-table{min-width:900px!important;border-collapse:collapse!important}
    .v158-history-table th,.v158-history-table td{padding:7px 8px!important;font-size:12px!important;white-space:nowrap!important}
    .v158-history-table th{background:#e8eff7!important}
  `;
  document.head.appendChild(st);
})();
/* ===== /v158 ===== */
