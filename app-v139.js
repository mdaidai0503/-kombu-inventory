...
function hStockCanvas(y){
 ...
 const hidakaShip=!!window.__v139HidakaShipment;
 ...
 rows.forEach(r=>{
   ...
   x.beginPath();x.moveTo(tx,yy+rh);x.lineTo(tx+tw,yy+rh);x.stroke();yy+=rh;last=r.section
 });
...
function v55CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    window.__v59HidakaSectionCentered=true;window.__v63ShipmentHeaderLarge=true;window.__v139HidakaShipment=true;try{return v55RetitleStockCanvas(hStockCanvas(y),'日高昆布　出 荷 依 頼 書',y,s,80,35);}finally{window.__v59HidakaSectionCentered=false;window.__v63ShipmentHeaderLarge=false;window.__v139HidakaShipment=false;}
...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, tableY, tableX){
  ...
  /* v139: 年産表示と上部罫線の干渉を避けるため、帳票本体を少し下へ移動。
  ...
  const isHidakaShipment=!!window.__v139HidakaShipment;
  if(isHidakaShipment){
    text(`依頼日：${shipment?.shipDate||''}`,tableX,28,24,'left',true);
