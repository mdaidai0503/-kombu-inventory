...
正常に戻ったことを確認しました。Step 2を作り直し、今回はお渡しする前に JavaScriptの構文チェックを実行し、エラーなしを確認済みです。

修正版 Step 2 完成TXT

今回は正常だったStep 1を基準に、v114UnifiedShipmentForm() の範囲だけを正確に置き換えています。既存の共通フォームは4種類の昆布を扱う保存処理を持っているため、その仕組みは維持しています。
...
削除
↓
app-v159-v161-step2-fixed.txt の全文を貼り付け
↓
Commit

してください。index.html は変更しません。

その後、PCで Ctrl + F5 を押します。まずトップ画面が正常に開くことを確認し、そのあと、
...
① 基本情報 → ② 出荷元（マスター選択）→ ③ 出荷先（マスター選択）→ ④ 出荷明細

の順になります。数量入力時には合計数量と在庫不足表示も更新します。

今回は 構文チェック済みなので、前回のような構文エラーによる起動フリーズは解消しています。

app-v159-v161-step2-fixed.txt
ドキュメント
