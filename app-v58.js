...
function _stockCanvasPage(year){
 ...
 let y=tableY+h1+h2;state.coops.forEach(coop=>{
  ...
  SEASONS.forEach((season,si)=>{ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();if(si===0)fit(coop,tableX,y+(window.__v58ShipmentCoopLower?rowH*1.5:rowH/2),coopW,13,true);fit(season,xCoop,y+rowH/2,seasonW,14,true);let rt=0;cols.forEach((c,j)=>{const q=m[[year,coop,c.group,c.item,season].join('|')]||0;rt+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,20,false)});if(rt)fit(fmt(rt),xData+dataW,y+rowH/2,totalW,20,false);y+=rowH});
...
function v55CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    window.__v58ShipmentCoopLower=true;try{return v55RetitleStockCanvas(_stockCanvasPage(y),'釧 路 産 昆 布　出 荷 指 示 書',y,s,100);}finally{window.__v58ShipmentCoopLower=false;}
...
function v55CanvasNemuro(s,y){
  ...
  try{
    ...
    nState.shipments=[];
    window.__v58ShipmentCoopLower=true;try{return v55RetitleStockCanvas(nReportCanvas(y,null),'根 室 産 昆 布　出 荷 指 示 書',y,s,64);}finally{window.__v58ShipmentCoopLower=false;}
...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    window.__v58ShipmentCoopLower=true;try{return v55RetitleStockCanvas(smReportCanvas(y,null),'釧 路 産 棹 前 昆 布　出 荷 指 示 書',y,s,64);}finally{window.__v58ShipmentCoopLower=false;}
