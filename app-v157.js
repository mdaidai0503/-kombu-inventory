...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, tableY, tableX){
  ...
  /* v157: 出荷元/出荷先の下に配送情報・備考欄を追加するため表を少し下へ移動。
  ...
  /* v157: 出荷先下＝着希望日／配送・袋入等（2分割）、出荷元下＝備考 */
...
function v76ShipmentMenu(){
  ...
  app.innerHTML=`
    ...
    <section class="card" id="v157HiddenProductHeader" style="display:none!important">
...
(function(){
  ...
  window.v157OpenItemsPdf=openItemsPdf;
  ...
  function faxBoxPage(){
    ...
    const cards=[...groups.entries()].map(([g,items],gi)=>{
      ...
      const rows=sorted.map(it=>`<tr data-fax-row="${esc(it.key)}">
        <td class="v157-check"><input type="checkbox" class="v157-fax-check" data-key="${esc(it.key)}"></td>
      ...
      return `<section class="card v157-fax-card" style="margin-top:12px;padding:14px" data-fax-card="${gi}"><div class="row" style="align-items:flex-start"><div><h3 style="margin:0 0 3px">${esc(d.name||'送信先未設定')}</h3></div><span class="pill">${items.length}件</span></div>
        <div class="tablewrap" style="margin-top:10px;overflow:auto"><table class="v157-compact-table"><colgroup><col class="c0"><col class="c1"><col class="c2"><col class="c3"><col class="c4"><col class="c5"><col class="c6"><col class="c7"></colgroup><thead><tr><th><input type="checkbox" class="v157-fax-all" aria-label="全選択"></th><th>依頼日</th><th>昆布</th><th>出荷元</th><th>出荷先</th><th>個数</th><th>削除</th><th>番号</th></tr></thead><tbody>${rows}</tbody></table></div>
    ...
    app.querySelectorAll('.v157-fax-all').forEach(all=>all.onchange=()=>{const card=all.closest('.v157-fax-card');card.querySelectorAll('.v157-fax-check').forEach(c=>c.checked=all.checked)});
    app.querySelectorAll('[data-fax-checked]').forEach(b=>b.onclick=async()=>{
      const card=b.closest('.v157-fax-card'),keys=[...card.querySelectorAll('.v157-fax-check:checked')].map(c=>c.dataset.key),map=new Map(box.map(x=>[x.key,x])),items=keys.map(k=>map.get(k)).filter(Boolean);
...
(function(){
  ...
  function v114UnifiedShipmentForm(editProduct=null,editId=null,preset=null){
    ...
    app.innerHTML=`<section class="card v113-ship-form"><h2 class="v157-form-title">📦 ${editing?'出荷依頼修正':'新規出荷依頼'}</h2><div class="form"><datalist id="companyNames">${companyDatalist()}</datalist>
      <div class="v157-top-columns">
        <div class="v157-top-stack">
          <label class="v157-top-box v157-date-box">依頼日
            <input id="v114ShipDate" type="date" value="${existing?.shipDate||today()}">
          </label>
          <label class="v157-top-box">配送・袋入等
        ...
        <div class="v157-top-stack">
          <label class="v157-top-box v157-date-box">着希望日
            <input id="v114ArrivalDate" type="date" value="${existing?.arrivalDate||''}">
          </label>
          <label class="v157-top-box">備考
      ...
      <div class="v113-lines-wrap"><div class="v113-lines-title">明細</div><div id="v114Lines"></div>
        ...
        <button class="btn v157-draft-save" id="v114Save">💾 ${editing?'修正保存':'下書き保存'}</button>
      </div>
    </div></section>`;
  ...
  function ensureShipmentNav(){
    ...
    if(!nav){
      ...
      nav.querySelector('#v119Back').onclick=()=>{
        /* v157: DOM上の実画面を優先して判定。screenKindが古い状態でも
        ...
        const onHistory=!!document.querySelector('.v157-history-card,.v157-history-card');
...
(function(){
  ...
  function renderTopNav(){
    ...
    if(!nav){
      ...
      nav.innerHTML=`<button id="v120Backup" aria-label="バックアップ" title="バックアップ"><span class="v157-top-icon">💾</span><span class="v157-top-label">バックアップ</span></button><button id="v120Restore" aria-label="バックアップ復元" title="バックアップ復元"><span class="v157-top-icon">♻️</span><span class="v157-top-label">復元</span></button><button id="v120Company" aria-label="会社マスター" title="会社マスター"><span class="v157-top-icon">🏢</span><span class="v157-top-label">会社マスター</span></button>`;
      document.body.appendChild(nav);
...
(function(){
  ...
  function shipmentHistory(){
    ...
    app.innerHTML=`<section class="card v157-history-card v157-history-title-marker" style="margin-top:14px;padding:12px">
      <div class="tablewrap" style="overflow:auto">
        <table class="v157-compact-table v157-history-table">
    ...
    const selects=[...document.querySelectorAll('.v157-filter-row select[data-col]')];
    const stateFilter={};
    let sortCol='',sortDir='';
...
/* ===== v157: compact selectable FAXBOX/history tables ===== */
(function(){
 const st=document.createElement('style');st.textContent=`
 .v157-compact-table{min-width:720px;width:100%;font-size:11px;table-layout:fixed}
 .v157-compact-table th,.v157-compact-table td{padding:7px 4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;text-align:center}
 .v157-compact-table .c0{width:30px}.v157-compact-table .c1{width:82px}.v157-compact-table .c2{width:48px}.v157-compact-table .c3{width:105px}.v157-compact-table .c4{width:105px}.v157-compact-table .c5{width:44px}.v157-compact-table .c6{width:50px}.v157-compact-table .c7{width:66px}
 .v157-compact-table input[type=checkbox]{width:18px;height:18px;margin:0;vertical-align:middle}
 `;document.head.appendChild(st);
})();
/* ===== /v157 ===== */


/* ===== v157: shipment history filters ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 body:has(.v157-history-card) header h1:empty{display:none}
 body:has(.v157-history-card) header:has(h1:empty){min-height:18px;padding-top:0;padding-bottom:0}
 .v157-history-card{margin-bottom:calc(120px + env(safe-area-inset-bottom))}
 .v157-history-table{min-width:680px}
 .v157-history-table th,.v157-history-table td{padding:6px 3px;font-size:10.5px}
 .v157-history-table col.c1{width:76px}
 .v157-history-table col.c2{width:44px}
 .v157-history-table col.c3{width:92px}
 .v157-history-table col.c4{width:92px}
 .v157-history-table col.c5{width:40px}
 .v157-history-table col.c6{width:44px}
 .v157-history-table col.c7{width:58px}
 .v157-filter-row th{padding:3px 2px;background:#f4f7fb}
 .v157-filter-row select{width:100%;min-width:0;height:28px;padding:1px 14px 1px 3px;font-size:9.5px;border:1px solid #cbd5e1;border-radius:5px;background:#fff;color:#17324f}
 .v157-filter-row select:disabled{opacity:.45}
 `;
 document.head.appendChild(st);
})();
/* ===== /v157 ===== */
...
/* ===== v157: shipment request UI refinements ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 /* unified shipment form */
 .v157-draft-save{margin-top:10px!important;width:100%!important}
 .v157-qty-label{display:flex!important;flex-direction:column!important;align-items:stretch!important}
 .v157-avail{display:block!important;margin-top:6px!important;text-align:left!important;font-size:11px!important;line-height:1.2!important;color:#e7eef8!important;white-space:nowrap!important}
 .v157-delete{font-size:12px!important;padding:6px 9px!important;min-width:56px!important;min-height:38px!important;border-radius:10px!important}
 ...
 body:has(.v157-history-title-marker) header{background:#0b2b55!important;color:#fff!important}
 .v157-history-table{min-width:560px!important;width:100%!important}
 .v157-history-table th,.v157-history-table td{padding:5px 2px!important;font-size:9.6px!important}
 .v157-history-table col.c1{width:66px!important}
 .v157-history-table col.c2{width:38px!important}
 .v157-history-table col.c3{width:76px!important}
 .v157-history-table col.c4{width:76px!important}
 .v157-history-table col.c5{width:34px!important}
 .v157-history-table col.c6{width:38px!important}
 .v157-history-table col.c7{width:50px!important}
 .v157-filter-row select{height:26px!important;font-size:8.8px!important;padding-left:2px!important;padding-right:12px!important}
 `;
 document.head.appendChild(st);
})();
/* ===== /v157 ===== */
...
/* ===== v157: back navigation / delete confirm / date styling ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v113-date-row .v157-date-box{
   display:flex!important;
   flex-direction:column!important;
   gap:6px!important;
 ...
 .v113-date-row .v157-date-box input{
   width:100%!important;
   box-sizing:border-box!important;
   background:#fff!important;
 ...
 @media(max-width:430px){
   .v113-date-row .v157-date-box{padding:8px!important;font-size:12px!important}
   .v113-date-row .v157-date-box input{font-size:12.5px!important;padding:8px 6px!important}
   .v157-delete{font-size:9.5px!important;min-width:42px!important;min-height:30px!important;padding:3px 6px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
(function(){
 ...
 .v157-date-stack{display:flex!important;flex-direction:column!important;gap:10px!important;min-width:0!important}
 .v157-date-stack .v157-date-box{width:100%!important;min-width:0!important;overflow:hidden!important}
 .v157-date-stack .v157-date-box input{display:block!important;max-width:100%!important;min-width:0!important;margin:0!important}
 .v157-extra-box{
 ...
 .v157-extra-box input,.v157-extra-box select{
 ...
 @media(max-width:430px){
   .v157-extra-box{padding:8px!important;font-size:12px!important}
   .v157-extra-box input,.v157-extra-box select{font-size:12.5px!important;padding:8px 6px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
/* ===== v157: compact top fields + unified edit form ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v157-form-title{text-align:center!important;width:100%!important}
 .v157-top-grid{
 ...
 .v157-top-box input,.v157-top-box select{
   display:block!important;width:100%!important;max-width:100%!important;min-width:0!important;
   box-sizing:border-box!important;margin:0!important;
   background:#fff!important;color:#102a43!important;border:1px solid #d4dde8!important;
 ...
 .v157-date-box{padding-top:7px!important;padding-bottom:7px!important}
 .v157-date-box input{height:40px!important;padding-top:5px!important;padding-bottom:5px!important}
 .v157-top-box select{
   -webkit-appearance:auto!important;appearance:auto!important;
 }
 @media(max-width:430px){
   .v157-top-grid{gap:8px!important}
   .v157-top-box{padding:7px 8px!important;font-size:12px!important}
   .v157-top-box input,.v157-top-box select{height:39px!important;font-size:12.5px!important;padding:5px 7px!important}
   .v157-date-box input{height:37px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
/* ===== v157: v151-style top input arrangement ===== */
(function(){
 const st=document.createElement('style');
 st.textContent=`
 .v157-top-columns{
 ...
 .v157-top-stack .v157-top-box{
   width:100%!important;
   min-width:0!important;
   box-sizing:border-box!important;
 }
 @media(max-width:430px){
   .v157-top-columns{gap:8px!important}
   .v157-top-stack{gap:8px!important}
 }
 `;
 document.head.appendChild(st);
})();
...
/* ===== v157: inventory stock nav + labeled home nav ===== */
(function(){
  const st=document.createElement('style');
  st.textContent=`
    #v120TopNav button{
    ...
    #v120TopNav .v157-top-icon{
      display:block!important;
      font-size:22px!important;
      line-height:24px!important;
      font-weight:400!important;
    }
    #v120TopNav .v157-top-label{
      display:block!important;
      font-size:10px!important;
    ...
    @media(max-width:430px){
      ...
      #v120TopNav .v157-top-icon{font-size:21px!important;line-height:23px!important}
      #v120TopNav .v157-top-label{font-size:9.5px!important;line-height:12px!important}
    }
  `;
  document.head.appendChild(st);
...
/* ===== v157: direct inventory landing + simplified Other ===== */
(function(){
  ...
  /* 「その他」はPDF一括入庫だけに統一。 */
  function v157OtherPage(){
    v80InventoryMode=true;
    setNavVisible(true);
    if(typeof bindNav==='function')bindNav();
    app.innerHTML=`
      <section class="card v157-other-card" style="margin-top:22px">
        <button class="btn v157-bulk-btn" id="v157BulkPdfBtn">📄 PDFから4種類を一括入庫</button>
        <input id="v157BulkPdfFile" type="file" accept="application/pdf,.pdf" hidden>
      </section>`;
    const btn=document.getElementById('v157BulkPdfBtn');
    const inp=document.getElementById('v157BulkPdfFile');
    if(btn&&inp){
  ...
  globalThis.exportsPage=v157OtherPage;
  globalThis.hMore=v157OtherPage;
  globalThis.nMore=v157OtherPage;
  globalThis.smMore=v157OtherPage;

  /* 下部ナビの「その他」も現在の昆布に関係なく共通画面へ。 */
  const baseBind=globalThis.bindNav;
  globalThis.bindNav=function(){
    if(typeof baseBind==='function')baseBind.apply(this,arguments);
    if(moreBtnEl)moreBtnEl.onclick=v157OtherPage;
  };

  const style=document.createElement('style');
  style.textContent=`
    .v157-other-card{padding:18px!important}
    .v157-bulk-btn{
      width:100%!important;
      background:#0b2b55!important;
      color:#fff!important;
...
/* ===== v157: stock-header shipment button + inventory back navigation ===== */
(function(){
  let backStack=[];
  ...
  function injectStockShipmentButton(){
    ...
    let b=card.querySelector('#v157StockShip');
    if(!b){
      b=document.createElement('button');
      b.id='v157StockShip';b.type='button';
      b.className='btn v157-stock-ship-btn';
      b.innerHTML='<span aria-hidden="true">📦</span><span class="v157-ship-label">出荷依頼</span>';
  ...
  css.textContent=`
    ...
    .v157-stock-ship-btn>span:first-child{font-size:16px!important;line-height:1!important}
    .v157-ship-label{font-size:11px!important;white-space:nowrap!important}
    body[data-v119-nav-mode="inventory"] > nav.v119-standard-nav button{min-width:16.4%!important}
    @media(max-width:430px){
      .v157-stock-ship-btn{min-width:56px!important;padding:6px 7px!important}
      .v157-stock-ship-btn>span:first-child{font-size:15px!important}
      .v157-ship-label{font-size:10px!important}
    }
  `;
  document.head.appendChild(css);
