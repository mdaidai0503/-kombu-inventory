...
function v55RetitleStockCanvas(canvas, title, year, shipment, tableY, tableX){
  const W=canvas.width,H=canvas.height, headerBottom=245;
  ...
  /* 在庫集計表の表部分をそのまま下へ移動。
     上部の出荷先/出荷元枠の左右端は tableX / W-tableX とし、
     下部表の外枠と完全に揃える。 */
  x.drawImage(canvas,0,tableY,W,H-tableY,0,headerBottom,W,H-tableY);

  x.fillStyle='#000';x.strokeStyle='#222';x.lineWidth=1.4;
  const font=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans","Noto Sans JP",sans-serif`;
  const text=(t,xx,yy,z=14,a='left',b=false)=>{
    x.font=font(z,b);x.textAlign=a;x.textBaseline='middle';
    x.fillText(String(t??''),xx,yy);
  };

  const src=shipment?.source&&typeof shipment.source==='object'
    ?shipment.source:shipmentSource(shipment);
  const dst=shipment?.destInfo&&typeof shipment.destInfo==='object'
    ?shipment.destInfo
    :(shipment?.dest&&typeof shipment.dest==='object'?shipment.dest:shipmentDest(shipment));
  ...
  const party=(left,label,o)=>{
    /* 確定仕様
       ...
       - 会社名：大型表示
       - 住所・電話番号：会社名より若干小さめ */
    text(label,left+16,boxY+24,22,'left',true);
    text(o?.name||'',left+16,boxY+70,42,'left',true);
    text(`住所：${o?.address||''}`,left+16,boxY+113,28,'left',false);
    text(`電話：${o?.phone||''}`,left+16,boxY+145,28,'left',false);
  };

  party(tableX,'出荷先',dst);
  party(tableX+boxW,'出荷元',src);

  text(`指示番号：${shipment?.id||''}　出荷日：${shipment?.shipDate||''}`,
       W-tableX,229,12,'right',false);
...
function v55CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    window.__v58ShipmentCoopLower=true;try{return v55RetitleStockCanvas(_stockCanvasPage(y),'釧 路 産 昆 布　出 荷 指 示 書',y,s,112,44);}finally{window.__v58ShipmentCoopLower=false;}
...
function v55CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    window.__v59HidakaSectionCentered=true;try{return v55RetitleStockCanvas(hStockCanvas(y),'日 高 昆 布　出 荷 指 示 書',y,s,80,35);}finally{window.__v59HidakaSectionCentered=false;}
...
function v55CanvasNemuro(s,y){
  ...
  try{
    ...
    nState.shipments=[];
    window.__v58ShipmentCoopLower=true;try{return v55RetitleStockCanvas(nReportCanvas(y,null),'根 室 産 昆 布　出 荷 指 示 書',y,s,70,34);}finally{window.__v58ShipmentCoopLower=false;}
...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    window.__v58ShipmentCoopLower=true;try{return v55RetitleStockCanvas(smReportCanvas(y,null),'釧 路 産 棹 前 昆 布　出 荷 指 示 書',y,s,70,34);}finally{window.__v58ShipmentCoopLower=false;}
