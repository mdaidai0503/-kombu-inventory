...
async function parseInventoryPdf(file){
  ...
  PDFJS.GlobalWorkerOptions.workerSrc='./pdf-worker-v51.js';
...
async function v42BulkPdfImport(file){
  ...
  app.innerHTML=`<section class="card" style="margin-top:22px"><div class="row"><h2>📄 4種類 PDF一括入庫</h2><span class="pill">v51</span></div><p><b>${esc(file.name)}</b></p><div class="note">PDFを1回読み込み、釧路産昆布・日高昆布・根室産昆布・釧路産棹前昆布を自動判別しています。</div><p id="v42Progress" style="margin-top:16px;font-weight:700">PDFを準備しています…</p></section>`;
...
productChoicePage=function(mode){
  ...
  const pill=app.querySelector('.pill');if(pill)pill.textContent='v51';
  if(mode==='inventory'){
    const card=app.querySelector('.card'),back=document.getElementById('v38Back');
...
productLanding=function(){_v41LandingForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v51'};
const _v41CompanyForV42=companyMasterPage;
companyMasterPage=function(){_v41CompanyForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v51'};

bindNav();productLanding();


/* ===== v51: iPhone Safari shipment PDF/FAX white-screen fix =====
...
*/
function v51ShipmentPrintPreview(opts){
  ...
  app.innerHTML=`
  <style id="v51ShipPrintStyle">
    #v51ShipPrint{margin:-14px -12px 0}
    .v51bar{position:sticky;top:0;z-index:30;background:#173661;color:#fff;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v51bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}
    .v51print{background:#fff;color:#173661}.v51back{background:#dfe7f1;color:#173661}
    .v51hint{background:#fff3bf;color:#5c4b00;padding:10px;text-align:center;font-size:13px}
    .v51preview{overflow:auto;background:#eef2f6;padding:8px}
    .v51sheet{width:281mm;min-height:194mm;margin:0 auto 10px;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v51head{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #000;padding-bottom:5px}
    .v51title{font-size:24px;font-weight:800;letter-spacing:5px}.v51meta{text-align:right;font-size:10px;line-height:1.6}
    .v51info{display:grid;grid-template-columns:1fr 1fr .8fr;gap:6px;margin:7px 0;font-size:10px}
    .v51box,.v51memo,.v51sign>div{border:1px solid #222;padding:5px}
    .v51summary{border:1px solid #222;padding:5px;margin-bottom:7px;font-size:10px}
    .v51table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:10px}
    .v51table th,.v51table td{border:.5px solid #333;padding:3px;text-align:center;height:22px;overflow:hidden;white-space:nowrap}
    .v51table thead th{background:#eee}.v51table tfoot th,.v51table tfoot td{border-top:1.5px solid #000;font-weight:700}
    .v51memo{margin-top:7px;min-height:32px;font-size:10px}
    .v51sign{display:grid;grid-template-columns:1fr 1.5fr 1fr;gap:6px;margin-top:7px;font-size:10px}.v51sign>div{height:34px}
    @media print{
      @page{size:A4 landscape;margin:7mm}
      header,nav,.v51bar,.v51hint{display:none!important}
      main{padding:0!important;max-width:none!important}
      .v51preview{overflow:visible;background:#fff;padding:0}
      .v51sheet{margin:0;width:auto;min-height:auto;padding:0;box-shadow:none}
    }
  </style>
  <div id="v51ShipPrint">
    <div class="v51bar">
      <button class="v51print" id="v51DoPrint">PDF・印刷・FAXへ</button>
      <button class="v51back" id="v51Back">出荷指示へ戻る</button>
    </div>
    <div class="v51hint"><b>帳票プレビュー</b> — 下に出荷指示書が表示されていれば正常です。「PDF・印刷・FAXへ」からiPhoneの印刷・共有機能を使えます。</div>
    <div class="v51preview">
      <div class="v51sheet">
        <div class="v51head">
          <div class="v51title">出 荷 指 示 書</div>
          <div class="v51meta">指示番号：${esc(s.id||'')}<br>作成日：${esc(today())}</div>
        </div>
        <div class="v51info">
          <div class="v51box"><b>出荷先：</b>${esc(dst.name||'')} 御中<br>住所：${esc(dst.address||'')}<br>電話：${esc(dst.phone||'')}</div>
          <div class="v51box"><b>出荷元：</b>${esc(src.name||'')}<br>住所：${esc(src.address||'')}<br>電話：${esc(src.phone||'')}</div>
          <div class="v51box"><b>出荷日：</b>${esc(s.shipDate||'')}<br><b>希望着日：</b>${esc(s.arrivalDate||'')}</div>
        </div>
        <div class="v51summary"><b>昆布：</b>${esc(opts.title)}　　<b>合計数量：</b>${fmt(total)}</div>
        ...
        <div class="v51memo"><b>備考：</b>${esc(s.memo||'')}</div>
        <div class="v51sign"><div>出荷元：${esc(src.name||'')}</div><div>受注・配送指示：</div><div>FAX送信欄：</div></div>
      </div>
    </div>
  </div>`;
  const p=document.getElementById('v51DoPrint');
  const b=document.getElementById('v51Back');
