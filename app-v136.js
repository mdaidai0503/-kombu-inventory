...
v55RetitleStockCanvas=function(sourceCanvas, title, year, shipment, tableY, tableX){
  ...
  /* v136: タイトルを大きく中央寄せ、昆布名を右横、年産をその下へ */
  const titleX=W*0.36, productX=W*0.535;
  text(mainTitle,titleX,31,42,'center',true);
  if(productName)text(`(${productName})`,productX,29,30,'left',true);
  text(`(${year}年産)`,productX,57,27,'left',true);
  text(`依頼番号：${shipment?.id||''}　出荷日：${shipment?.shipDate||''}`,W-tableX,23,12,'right',false);
...
(function(){
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v136';
    return r;
  };
})();
/* ===== /v136 ===== */
