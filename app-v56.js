...
async function parseInventoryPdf(file){
  ...
  PDFJS.GlobalWorkerOptions.workerSrc='./pdf-worker-v56.js';
...
function _stockCanvasPage(year){
 ...
 let y=tableY+h1+h2;state.coops.forEach(coop=>{
  ...
  SEASONS.forEach((season,si)=>{ctx.beginPath();ctx.moveTo(tableX,y);ctx.lineTo(tableX+tableW,y);ctx.stroke();if(si===0)fit(coop,tableX,y+rowH/2,coopW,13,true);fit(season,xCoop,y+rowH/2,seasonW,14,true);let rt=0;cols.forEach((c,j)=>{const q=m[[year,coop,c.group,c.item,season].join('|')]||0;rt+=q;const xx=xData+j*itemW;ctx.beginPath();ctx.moveTo(xx,y);ctx.lineTo(xx,y+rowH);ctx.stroke();if(q)fit(fmt(q),xx,y+rowH/2,itemW,20,false)});if(rt)fit(fmt(rt),xData+dataW,y+rowH/2,totalW,20,false);y+=rowH});
...
function hStockCanvas(y){const W=1684,H=1191,m=35,c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,W,H);x.strokeStyle='#222';x.fillStyle='#000';const font=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif`;const txt=(t,xx,yy,z=14,a='center',b=false)=>{x.font=font(z,b);x.textAlign=a;x.textBaseline='middle';x.fillText(String(t??''),xx,yy)};const rows=H_SECTIONS.flatMap(s=>s.items.map(g=>({section:s.name,grade:g}))),mt=hMatrix();txt('日 高 昆 布　在 庫 集 計 表',m,45,30,'left',true);txt(`${y}年産`,W-m,45,18,'right',true);const tx=m,ty=80,tw=W-m*2,secW=70,gradeW=95,totalW=60,colW=(tw-secW-gradeW-totalW)/H_LOCATIONS.length,rowH=26,headH=48; x.lineWidth=.55;x.strokeRect(tx,ty,tw,headH+rows.length*rowH+32);[tx+secW,tx+secW+gradeW,tx+tw-totalW].forEach(xx=>{x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+headH+rows.length*rowH+32);x.stroke()});txt('区分',tx+secW/2,ty+headH/2,13);txt('等級',tx+secW+gradeW/2,ty+headH/2,13);H_LOCATIONS.forEach((l,i)=>{const xx=tx+secW+gradeW+i*colW;x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+headH+rows.length*rowH+32);x.stroke();txt(l,xx+colW/2,ty+headH/2,10)});txt('計',tx+tw-totalW/2,ty+headH/2,12);let yy=ty+headH,last=null;rows.forEach(r=>{if(r.section!==last){x.lineWidth=1.6;x.beginPath();x.moveTo(tx,yy);x.lineTo(tx+tw,yy);x.stroke();x.lineWidth=.55}txt(r.section!==last?r.section:'',tx+secW/2,yy+rowH/2,12);txt(r.grade,tx+secW+gradeW/2,yy+rowH/2,11);let rt=0;H_LOCATIONS.forEach((l,i)=>{const q=mt[[y,l,r.section,r.grade].join('|')]||0,xx=tx+secW+gradeW+i*colW;rt+=q;x.beginPath();x.moveTo(xx,yy);x.lineTo(xx,yy+rowH);x.stroke();if(q)txt(fmt(q),xx+colW/2,yy+rowH/2,14)});if(rt)txt(fmt(rt),tx+tw-totalW/2,yy+rowH/2,14);x.beginPath();x.moveTo(tx,yy+rowH);x.lineTo(tx+tw,yy+rowH);x.stroke();yy+=rowH;last=r.section});x.lineWidth=1.6;x.beginPath();x.moveTo(tx,yy);x.lineTo(tx+tw,yy);x.stroke();x.lineWidth=.55;txt('合計',tx+(secW+gradeW)/2,yy+16,13,'center',true);H_LOCATIONS.forEach((l,i)=>{const q=rows.reduce((a,r)=>a+(mt[[y,l,r.section,r.grade].join('|')]||0),0);if(q)txt(fmt(q),tx+secW+gradeW+i*colW+colW/2,yy+16,13)});txt(fmt(hTotal(y)),tx+tw-totalW/2,yy+16,13);return c}
...
function nReportCanvas(y,ship=null){const W=1684,H=1191,margin=34,c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d'),cols=nItems(),title=ship?'出 荷 指 示 書（根室産昆布）':'根 室 産 昆 布　在 庫 集 計 表';x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle='#000';x.strokeStyle='#222';const f=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif`,t=(v,xx,yy,z=13,a='center',b=false)=>{x.font=f(z,b);x.textAlign=a;x.textBaseline='middle';x.fillText(String(v??''),xx,yy)};t(title,margin,38,28,'left',true);t(`${y}年産`,W-margin,38,16,'right',true);if(ship){t(`指示番号：${ship.id}　出荷日：${ship.shipDate||''}`,W-margin,62,13,'right');t(`出荷先：${ship.dest?.name||''}　${ship.dest?.address||''}　TEL ${ship.dest?.phone||''}`,margin,62,12,'left')}const ty=ship?85:70,tw=W-margin*2,coopW=92,seasonW=45,totalW=58,dataW=tw-coopW-seasonW-totalW,colW=dataW/cols.length,h1=28,h2=28,rowH=39,footH=34,tableH=h1+h2+N_COOPS.length*4*rowH+footH;x.lineWidth=.55;x.strokeRect(margin,ty,tw,tableH);const xData=margin+coopW+seasonW;[margin+coopW,xData,xData+dataW].forEach(xx=>{x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+tableH);x.stroke()});t('組合名',margin+coopW/2,ty+(h1+h2)/2,12);t('区分',margin+coopW+seasonW/2,ty+(h1+h2)/2,12);t('計',xData+dataW+totalW/2,ty+(h1+h2)/2,12);let ci=0;N_GROUPS.forEach(g=>{const gx=xData+ci*colW,gw=g.items.length*colW;x.strokeRect(gx,ty,gw,h1);t(g.name,gx+gw/2,ty+h1/2,10,'center',true);g.items.forEach((it,j)=>{const xx=gx+j*colW;x.strokeRect(xx,ty+h1,colW,h2);t(it,xx+colW/2,ty+h1+h2/2,8)});ci+=g.items.length});const mt=nMatrix(),lines=ship?.lines||null;const qFor=(coop,se,c)=>lines?lines.filter(l=>l.year===y&&l.coop===coop&&l.season===se&&l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0):(mt[[y,coop,se,c.group,c.item].join('|')]||0);let yy=ty+h1+h2;for(const coop of N_COOPS){x.lineWidth=1.5;x.beginPath();x.moveTo(margin,yy);x.lineTo(margin+tw,yy);x.stroke();x.lineWidth=.55;for(let si=0;si<N_SEASONS.length;si++){const se=N_SEASONS[si];if(si===0)t(coop,margin+coopW/2,yy+rowH/2,12,'center',true);t(se,margin+coopW+seasonW/2,yy+rowH/2,13,'center',true);let rt=0;cols.forEach((cc,j)=>{const q=qFor(coop,se,cc),xx=xData+j*colW;rt+=q;x.beginPath();x.moveTo(xx,yy);x.lineTo(xx,yy+rowH);x.stroke();if(q)t(fmt(q),xx+colW/2,yy+rowH/2,13)});if(rt)t(fmt(rt),xData+dataW+totalW/2,yy+rowH/2,13);x.beginPath();x.moveTo(margin,yy+rowH);x.lineTo(margin+tw,yy+rowH);x.stroke();yy+=rowH}t('小計',margin+coopW+seasonW/2,yy+rowH/2,11);let st=0;cols.forEach((cc,j)=>{const q=N_SEASONS.reduce((a,se)=>a+qFor(coop,se,cc),0),xx=xData+j*colW;st+=q;if(q)t(fmt(q),xx+colW/2,yy+rowH/2,11)});if(st)t(fmt(st),xData+dataW+totalW/2,yy+rowH/2,11);x.beginPath();x.moveTo(margin,yy+rowH);x.lineTo(margin+tw,yy+rowH);x.stroke();yy+=rowH}x.lineWidth=1.5;x.beginPath();x.moveTo(margin,yy);x.lineTo(margin+tw,yy);x.stroke();x.lineWidth=.55;t('合計',margin+(coopW+seasonW)/2,yy+footH/2,12,'center',true);let gt=0;cols.forEach((cc,j)=>{const q=N_COOPS.reduce((a,co)=>a+N_SEASONS.reduce((b,se)=>b+qFor(co,se,cc),0),0),xx=xData+j*colW;gt+=q;if(q)t(fmt(q),xx+colW/2,yy+footH/2,12)});if(gt)t(fmt(gt),xData+dataW+totalW/2,yy+footH/2,12);return c}
...
function smReportCanvas(y,ship=null){const W=1684,H=1191,margin=34,c=document.createElement('canvas');c.width=W;c.height=H;const x=c.getContext('2d'),cols=smItems(),title=ship?'出 荷 指 示 書（釧路産棹前昆布）':'釧 路 産 棹 前 昆 布　在 庫 集 計 表';x.fillStyle='#fff';x.fillRect(0,0,W,H);x.fillStyle='#000';x.strokeStyle='#222';const f=(z,b=false)=>`${b?'700 ':'400 '}${z}px -apple-system,BlinkMacSystemFont,"Hiragino Sans",sans-serif`,t=(v,xx,yy,z=13,a='center',b=false)=>{x.font=f(z,b);x.textAlign=a;x.textBaseline='middle';x.fillText(String(v??''),xx,yy)};t(title,margin,38,28,'left',true);t(`${y}年産`,W-margin,38,16,'right',true);if(ship){t(`指示番号：${ship.id}　出荷日：${ship.shipDate||''}`,W-margin,62,13,'right');t(`出荷先：${ship.dest?.name||''}　${ship.dest?.address||''}　TEL ${ship.dest?.phone||''}`,margin,62,12,'left')}const ty=ship?85:70,tw=W-margin*2,coopW=92,seasonW=45,totalW=58,dataW=tw-coopW-seasonW-totalW,colW=dataW/cols.length,h1=28,h2=28,rowH=39,footH=34,tableH=h1+h2+S_COOPS.length*(S_SEASONS.length+1)*rowH+footH;x.lineWidth=.55;x.strokeRect(margin,ty,tw,tableH);const xData=margin+coopW+seasonW;[margin+coopW,xData,xData+dataW].forEach(xx=>{x.beginPath();x.moveTo(xx,ty);x.lineTo(xx,ty+tableH);x.stroke()});t('組合名',margin+coopW/2,ty+(h1+h2)/2,12);t('区分',margin+coopW+seasonW/2,ty+(h1+h2)/2,12);t('計',xData+dataW+totalW/2,ty+(h1+h2)/2,12);let ci=0;S_GROUPS.forEach(g=>{const gx=xData+ci*colW,gw=g.items.length*colW;x.strokeRect(gx,ty,gw,h1);t(g.name,gx+gw/2,ty+h1/2,10,'center',true);g.items.forEach((it,j)=>{const xx=gx+j*colW;x.strokeRect(xx,ty+h1,colW,h2);t(it,xx+colW/2,ty+h1+h2/2,8)});ci+=g.items.length});const mt=smMatrix(),lines=ship?.lines||null;const qFor=(coop,se,c)=>lines?lines.filter(l=>l.year===y&&l.coop===coop&&l.season===se&&l.group===c.group&&l.item===c.item).reduce((a,l)=>a+Number(l.qty||0),0):(mt[[y,coop,se,c.group,c.item].join('|')]||0);let yy=ty+h1+h2;for(const coop of S_COOPS){x.lineWidth=1.5;x.beginPath();x.moveTo(margin,yy);x.lineTo(margin+tw,yy);x.stroke();x.lineWidth=.55;for(let si=0;si<S_SEASONS.length;si++){const se=S_SEASONS[si];if(si===0)t(coop,margin+coopW/2,yy+rowH/2,12,'center',true);t(se,margin+coopW+seasonW/2,yy+rowH/2,13,'center',true);let rt=0;cols.forEach((cc,j)=>{const q=qFor(coop,se,cc),xx=xData+j*colW;rt+=q;x.beginPath();x.moveTo(xx,yy);x.lineTo(xx,yy+rowH);x.stroke();if(q)t(fmt(q),xx+colW/2,yy+rowH/2,13)});if(rt)t(fmt(rt),xData+dataW+totalW/2,yy+rowH/2,13);x.beginPath();x.moveTo(margin,yy+rowH);x.lineTo(margin+tw,yy+rowH);x.stroke();yy+=rowH}t('小計',margin+coopW+seasonW/2,yy+rowH/2,11);let st=0;cols.forEach((cc,j)=>{const q=S_SEASONS.reduce((a,se)=>a+qFor(coop,se,cc),0),xx=xData+j*colW;st+=q;if(q)t(fmt(q),xx+colW/2,yy+rowH/2,11)});if(st)t(fmt(st),xData+dataW+totalW/2,yy+rowH/2,11);x.beginPath();x.moveTo(margin,yy+rowH);x.lineTo(margin+tw,yy+rowH);x.stroke();yy+=rowH}x.lineWidth=1.5;x.beginPath();x.moveTo(margin,yy);x.lineTo(margin+tw,yy);x.stroke();x.lineWidth=.55;t('合計',margin+(coopW+seasonW)/2,yy+footH/2,12,'center',true);let gt=0;cols.forEach((cc,j)=>{const q=S_COOPS.reduce((a,co)=>a+S_SEASONS.reduce((b,se)=>b+qFor(co,se,cc),0),0),xx=xData+j*colW;gt+=q;if(q)t(fmt(q),xx+colW/2,yy+footH/2,12)});if(gt)t(fmt(gt),xData+dataW+totalW/2,yy+footH/2,12);return c}
...
async function v42BulkPdfImport(file){
  ...
  app.innerHTML=`<section class="card" style="margin-top:22px"><div class="row"><h2>📄 4種類 PDF一括入庫</h2><span class="pill">v56</span></div><p><b>${esc(file.name)}</b></p><div class="note">PDFを1回読み込み、釧路産昆布・日高昆布・根室産昆布・釧路産棹前昆布を自動判別しています。</div><p id="v42Progress" style="margin-top:16px;font-weight:700">PDFを準備しています…</p></section>`;
...
productChoicePage=function(mode){
  ...
  const pill=app.querySelector('.pill');if(pill)pill.textContent='v56';
  if(mode==='inventory'){
    const card=app.querySelector('.card'),back=document.getElementById('v38Back');
...
productLanding=function(){_v41LandingForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v56'};
const _v41CompanyForV42=companyMasterPage;
companyMasterPage=function(){_v41CompanyForV42();const pill=app.querySelector('.pill');if(pill)pill.textContent='v56'};

bindNav();productLanding();


/* ===== v56: iPhone Safari shipment PDF/FAX white-screen fix =====
...
*/
function v56ShipmentPrintPreview(opts){
  ...
  app.innerHTML=`
  <style id="v56ShipPrintStyle">
    #v56ShipPrint{margin:-14px -12px 0}
    .v56bar{position:sticky;top:0;z-index:30;background:#173661;color:#fff;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v56bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}
    .v56print{background:#fff;color:#173661}.v56back{background:#dfe7f1;color:#173661}
    .v56hint{background:#fff3bf;color:#5c4b00;padding:10px;text-align:center;font-size:13px}
    .v56preview{overflow:auto;background:#eef2f6;padding:8px}
    .v56sheet{width:281mm;min-height:194mm;margin:0 auto 10px;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v56head{display:flex;justify-content:space-between;align-items:flex-end;border-bottom:2px solid #000;padding-bottom:5px}
    .v56title{font-size:24px;font-weight:800;letter-spacing:5px}.v56meta{text-align:right;font-size:10px;line-height:1.6}
    .v56info{display:grid;grid-template-columns:1fr 1fr .8fr;gap:6px;margin:7px 0;font-size:10px}
    .v56box,.v56memo,.v56sign>div{border:1px solid #222;padding:5px}
    .v56summary{border:1px solid #222;padding:5px;margin-bottom:7px;font-size:10px}
    .v56table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:10px}
    .v56table th,.v56table td{border:.5px solid #333;padding:3px;text-align:center;height:22px;overflow:hidden;white-space:nowrap}
    .v56table thead th{background:#eee}.v56table tfoot th,.v56table tfoot td{border-top:1.5px solid #000;font-weight:700}
    .v56memo{margin-top:7px;min-height:32px;font-size:10px}
    .v56sign{display:grid;grid-template-columns:1fr 1.5fr 1fr;gap:6px;margin-top:7px;font-size:10px}.v56sign>div{height:34px}
    @media print{
      @page{size:A4 landscape;margin:7mm}
      header,nav,.v56bar,.v56hint{display:none!important}
      main{padding:0!important;max-width:none!important}
      .v56preview{overflow:visible;background:#fff;padding:0}
      .v56sheet{margin:0;width:auto;min-height:auto;padding:0;box-shadow:none}
    }
  </style>
  <div id="v56ShipPrint">
    <div class="v56bar">
      <button class="v56print" id="v56DoPrint">PDF・印刷・FAXへ</button>
      <button class="v56back" id="v56Back">出荷指示へ戻る</button>
    </div>
    <div class="v56hint"><b>帳票プレビュー</b> — 下に出荷指示書が表示されていれば正常です。「PDF・印刷・FAXへ」からiPhoneの印刷・共有機能を使えます。</div>
    <div class="v56preview">
      <div class="v56sheet">
        <div class="v56head">
          <div class="v56title">出 荷 指 示 書</div>
          <div class="v56meta">指示番号：${esc(s.id||'')}<br>作成日：${esc(today())}</div>
        </div>
        <div class="v56info">
          <div class="v56box"><b>出荷先：</b>${esc(dst.name||'')} 御中<br>住所：${esc(dst.address||'')}<br>電話：${esc(dst.phone||'')}</div>
          <div class="v56box"><b>出荷元：</b>${esc(src.name||'')}<br>住所：${esc(src.address||'')}<br>電話：${esc(src.phone||'')}</div>
          <div class="v56box"><b>出荷日：</b>${esc(s.shipDate||'')}<br><b>希望着日：</b>${esc(s.arrivalDate||'')}</div>
        </div>
        <div class="v56summary"><b>昆布：</b>${esc(opts.title)}　　<b>合計数量：</b>${fmt(total)}</div>
        <table class="v56table">
        ...
        <div class="v56memo"><b>備考：</b>${esc(s.memo||'')}</div>
        <div class="v56sign"><div>出荷元：${esc(src.name||'')}</div><div>受注・配送指示：</div><div>FAX送信欄：</div></div>
      </div>
    </div>
  </div>`;
  const p=document.getElementById('v56DoPrint');
  const b=document.getElementById('v56Back');
...
/* ===== v56 出荷指示書 = 各昆布の在庫集計表フォーマット ===== */
function v56MatrixShipmentPreview(o){
  ...
  app.innerHTML=`
  <style>
    .v56bar{position:sticky;top:0;z-index:30;background:#173661;padding:10px;display:grid;grid-template-columns:1fr 1fr;gap:8px}
    .v56bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}.v56go{background:#fff;color:#173661}.v56back{background:#dfe7f1;color:#173661}
    .v56wrap{overflow:auto;background:#eef2f6;padding:8px}.v56sheet{width:281mm;min-height:194mm;margin:auto;background:#fff;color:#000;padding:7mm;box-shadow:0 2px 8px #0002}
    .v56title{text-align:center;font-size:23px;font-weight:800;letter-spacing:4px;margin-bottom:5px}
    .v56sub{text-align:center;font-size:14px;font-weight:700;margin-bottom:7px}
    .v56info{display:grid;grid-template-columns:1fr 1fr 1fr;gap:5px;font-size:9px;margin-bottom:6px}.v56info>div{border:1px solid #333;padding:4px}
    .v56tbl{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9px}.v56tbl th,.v56tbl td{border:1px solid #333;text-align:center;padding:3px;height:21px}.v56tbl thead th,.v56tbl tfoot th{background:#e8edf3;font-weight:700}.v56tbl tbody th{background:#f4f6f8;text-align:left}
    .v56memo{border:1px solid #333;margin-top:6px;padding:5px;min-height:27px;font-size:9px}
    @media print{@page{size:A4 landscape;margin:7mm}header,nav,.v56bar{display:none!important}main{padding:0!important;max-width:none!important}.v56wrap{overflow:visible;background:#fff;padding:0}.v56sheet{width:auto;min-height:auto;padding:0;box-shadow:none}}
  </style>
  <div class="v56bar"><button class="v56go" id="v56print">PDF・印刷・FAXへ</button><button class="v56back" id="v56back">出荷指示へ戻る</button></div>
  <div class="v56wrap"><section class="v56sheet">
    <div class="v56title">${esc(o.reportTitle)}</div>
    <div class="v56sub">出　荷　指　示　書</div>
    <div class="v56info">
    ...
    <table class="v56tbl"><thead><tr><th>${esc(o.rowLabel)}</th>${cols}<th>合計</th></tr></thead>
      <tbody>${body}</tbody><tfoot><tr><th>合計</th>${foot}<th>${fmt(total)}</th></tr></tfoot></table>
    <div class="v56memo"><b>備考：</b>${esc(s.memo||'')}</div>
  </section></div>`;
  document.getElementById('v56print').onclick=()=>window.print();
  document.getElementById('v56back').onclick=()=>{setHeader(oldHeader);o.back();};
}
function v56Unique(a){return [...new Set(a.filter(Boolean))]}

/* 釧路：在庫集計表と同じ「漁協 × 等級/分類」の横持ち表 */
openShipmentPdfDirect=function(id){
 const s=state.shipments.find(x=>x.id===id); if(!s)return;
 v56MatrixShipmentPreview({title:'釧路産昆布 出荷指示',reportTitle:'釧 路 産 昆 布',
 shipment:s,source:shipmentSource(s),dest:shipmentDest(s),rowLabel:'漁協',
 rows:ls=>{const order=['東部漁協','昆布森漁協','厚岸漁協','散布漁協','浜中漁協'];const got=v56Unique(ls.map(l=>l.coop));return [...order.filter(x=>got.includes(x)),...got.filter(x=>!order.includes(x))]},
 cols:ls=>v56Unique(ls.map(l=>[l.season,l.group,l.item].filter(Boolean).join('・'))),
...
hOpenShipPdf=function(s){if(!s)return;
 v56MatrixShipmentPreview({title:'日高昆布 出荷指示',reportTitle:'日 高 昆 布',
 shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'産地',
 rows:ls=>v56Unique(ls.map(l=>l.location)),cols:ls=>v56Unique(ls.map(l=>[l.section,l.grade].filter(Boolean).join('・'))),
...
nOpenShipPdf=function(s){if(!s)return;
 v56MatrixShipmentPreview({title:'根室産昆布 出荷指示',reportTitle:'根 室 産 昆 布',
 shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'漁協',
 rows:ls=>v56Unique(ls.map(l=>l.coop)),cols:ls=>v56Unique(ls.map(l=>[l.season,l.group,l.item].filter(Boolean).join('・'))),
 rowKey:l=>l.coop||'',colKey:l=>[l.season,l.group,l.item].filter(Boolean).join('・'),
 back:()=>nShipDetail(s.id)});
};
...
if(typeof spOpenShipPdf==='function'){
 const v56OldSp=spOpenShipPdf;
 spOpenShipPdf=function(s){
   if(!s)return v56OldSp(s);
   v56MatrixShipmentPreview({title:'釧路産棹前昆布 出荷指示',reportTitle:'釧 路 産 棹 前 昆 布',
   shipment:s,source:s.source||{},dest:s.dest||{},rowLabel:'漁協',
   rows:ls=>v56Unique(ls.map(l=>l.coop||l.location)),cols:ls=>v56Unique(ls.map(l=>[l.season,l.group,l.item,l.grade].filter(Boolean).join('・'))),
...
/* ===== v56 出荷指示書PDFを在庫集計表PDFと完全に同じ表レイアウトへ =====
   在庫集計表Canvas関数をそのまま使用する。
   出荷数量だけを一時的に在庫レコードとして渡すため、
   0数量の行・列もマスター定義どおり全て表示される。
*/
function v56RetitleStockCanvas(canvas, title, year, shipment, topClear){
  const x=canvas.getContext('2d'),W=canvas.width;
  x.fillStyle='#fff';x.fillRect(0,0,W,topClear);
  x.fillStyle='#000';x.strokeStyle='#222';
  ...
  text(title,44,38,28,'left',true);
  text(`${year}年産`,W-44,32,17,'right',true);
  text(`出荷先：${shipment?.dest?.name||''}　指示番号：${shipment?.id||''}　出荷日：${shipment?.shipDate||''}`,W-44,60,12,'right',false);
  return canvas;
}
function v56ShipmentYears(s, fallback){
  const ys=[...new Set((s.lines||[]).map(l=>l.year||fallback).filter(Boolean))];
  return ys.length?ys:[fallback];
}
function v56CanvasKushiro(s,y){
  ...
  try{
    ...
    state.shipments=[];
    return v56RetitleStockCanvas(_stockCanvasPage(y),'釧 路 産 昆 布　出 荷 指 示 書',y,s,100);
  }finally{state.records=rec;state.shipments=ships}
}
function v56CanvasHidaka(s,y){
  ...
  try{
    ...
    hState.shipments=[];
    return v56RetitleStockCanvas(hStockCanvas(y),'日 高 昆 布　出 荷 指 示 書',y,s,74);
  }finally{hState.records=rec;hState.shipments=ships}
}
function v56CanvasNemuro(s,y){
  ...
  try{
    ...
    nState.shipments=[];
    return v56RetitleStockCanvas(nReportCanvas(y,null),'根 室 産 昆 布　出 荷 指 示 書',y,s,64);
  }finally{nState.records=rec;nState.shipments=ships}
}
function v56CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    return v56RetitleStockCanvas(smReportCanvas(y,null),'釧 路 産 棹 前 昆 布　出 荷 指 示 書',y,s,64);
  }finally{smState.records=rec;smState.shipments=ships}
}
function v56ShowShipmentCanvasPreview(title, shipment, canvases, backFn){
  const oldHeader=document.querySelector('header h1')?.textContent||'昆布在庫管理';
  setHeader(title);
  app.innerHTML=`
   <style>
    ...
    .v56bar button{border:0;border-radius:10px;padding:12px;font-size:16px;font-weight:700}
    .v56print{background:#fff;color:#173661}.v56back{background:#dfe7f1;color:#173661}
    .v56note{background:#fff3bf;color:#5c4b00;padding:8px 12px;text-align:center;font-size:13px}
    .v56preview{overflow:auto;background:#eef2f6;padding:8px}
    .v56page{width:281mm;margin:0 auto 10px;background:#fff;box-shadow:0 2px 8px #0002}
    .v56page canvas{display:block;width:281mm;height:auto;background:#fff}
    @media print{
      @page{size:A4 landscape;margin:0}
      header,nav,.v56bar,.v56note{display:none!important}
      main{padding:0!important;max-width:none!important}
      .v56preview{overflow:visible;background:#fff;padding:0}
      .v56page{width:297mm;height:210mm;margin:0;box-shadow:none;page-break-after:always;display:flex;align-items:flex-start;justify-content:center}
      .v56page:last-child{page-break-after:auto}
      .v56page canvas{width:297mm;height:210mm;object-fit:contain}
    }
   </style>
   <div class="v56bar"><button class="v56print" id="v56print">PDF・印刷・FAXへ</button><button class="v56back" id="v56back">出荷指示へ戻る</button></div>
   <div class="v56note">在庫集計表PDFと同じ固定行・固定列です。数量0の行・列も表示し、0のセルだけ空欄にしています。</div>
   <div class="v56preview" id="v56preview"></div>`;
  const preview=document.getElementById('v56preview');
  canvases.forEach(c=>{
    const d=document.createElement('div');d.className='v56page';d.appendChild(c);preview.appendChild(d);
  });
  document.getElementById('v56print').onclick=()=>window.print();
  document.getElementById('v56back').onclick=()=>{setHeader(oldHeader);backFn()};
}

/* 釧路産昆布 */
openShipmentPdfDirect=function(id){
  const s=state.shipments.find(x=>x.id===id);if(!s)return;
  const ys=v56ShipmentYears(s,state.activeYear);
  v56ShowShipmentCanvasPreview('釧路産昆布 出荷指示',s,ys.map(y=>v56CanvasKushiro(s,y)),()=>shipmentDetail(s.id));
};
/* 日高昆布 */
hOpenShipPdf=function(s){
  if(!s)return;
  const ys=v56ShipmentYears(s,hState.activeYear);
  v56ShowShipmentCanvasPreview('日高昆布 出荷指示',s,ys.map(y=>v56CanvasHidaka(s,y)),()=>hShipDetail(s.id));
};
/* 根室産昆布 */
nOpenShipPdf=function(s){
  if(!s)return;
  const ys=v56ShipmentYears(s,nState.activeYear);
  v56ShowShipmentCanvasPreview('根室産昆布 出荷指示',s,ys.map(y=>v56CanvasNemuro(s,y)),()=>nShipDetail(s.id));
};
/* 釧路産棹前昆布 */
smOpenShipPdf=function(s){
  if(!s)return;
  const ys=v56ShipmentYears(s,smState.activeYear);
  v56ShowShipmentCanvasPreview('釧路産棹前昆布 出荷指示',s,ys.map(y=>v56CanvasSanmae(s,y)),()=>smShipDetail(s.id));
};



/* ===== v56 釧路産昆布トップ画面を他3種類と統一 ===== */
home=function(){
...
setTimeout(v56WireGlobalHome,0);
...
/* ===== v56 4種類の在庫状況トップを簡素化 =====
   ...
   下部: 年産プルダウン付き総在庫
*/
function v56SimpleTop(opts){
  const y=opts.year;
  app.innerHTML=`
    <section class="grid" style="margin-top:12px">
      <button class="action" id="v56Ship" style="border-left:6px solid #e05a47">
        📦 出荷指示<small>${esc(opts.shipSub)}</small>
      </button>
      <button class="action orange" id="v56Stock">
        ▦ 在庫表<small>${esc(opts.stockSub)}</small>
      </button>
      <button class="action gray" id="v56More" style="grid-column:1/-1">
        ⋯ その他<small>${esc(opts.moreSub)}</small>
      </button>
    </section>

    <section class="card" style="margin-top:18px">
      <div class="row" style="align-items:center">
        <h2 style="margin:0">総在庫</h2>
        <select id="v56Year" style="width:auto;min-width:110px">${opts.yearOptions(y)}</select>
      </div>
      <div style="margin-top:18px;background:#edf3fa;border-radius:18px;padding:22px">
        <div id="v56YearLabel" class="muted" style="font-size:18px">${esc(y)}年産</div>
        <div id="v56Total" style="font-size:42px;font-weight:800;line-height:1.15;margin-top:6px">${fmt(opts.total(y))}</div>
      </div>
    </section>`;

  document.getElementById('v56Ship').onclick=opts.ship;
  document.getElementById('v56Stock').onclick=opts.stock;
  document.getElementById('v56More').onclick=opts.more;
  document.getElementById('v56Year').onchange=()=>{
    const ny=document.getElementById('v56Year').value;
    opts.setYear(ny);
    document.getElementById('v56YearLabel').textContent=`${ny}年産`;
    document.getElementById('v56Total').textContent=fmt(opts.total(ny));
  };
}
...
home=function(){
  v56SimpleTop({
    ...
    total:total,
    setYear:y=>setActiveYear(y),
    ship:shipments,
    stock:stock,
    more:kMore,
    shipSub:'釧路産昆布専用・PDF/FAX',
    stockSub:'原票形式で集計・PDF出力',
    moreSub:'バックアップ・商品選択・設定'
  });
};
...
hHome=function(){
  v56SimpleTop({
    year:hState.activeYear,
    yearOptions:hYearOptions,
    total:hTotal,
    setYear:y=>{hState.activeYear=y;hSave()},
    ship:hShipments,
    stock:hStock,
    more:hMore,
    shipSub:'日高昆布専用・PDF/FAX',
    stockSub:'原票形式で集計・PDF出力',
    moreSub:'バックアップ・商品選択'
  });
};
...
nHome=function(){
  v56SimpleTop({
    year:nState.activeYear,
    yearOptions:nYearOptions,
    total:nTotal,
    setYear:y=>{nState.activeYear=y;nSave()},
    ship:nShipments,
    stock:nStock,
    more:nMore,
    shipSub:'根室産昆布専用・PDF/FAX',
    stockSub:'原票形式で集計・PDF出力',
    moreSub:'バックアップ・商品選択'
  });
};
...
smHome=function(){
  v56SimpleTop({
    year:smState.activeYear,
    yearOptions:smYearOptions,
    total:smTotal,
    setYear:y=>{smState.activeYear=y;smSave()},
    ship:smShipments,
    stock:smStock,
    more:smMore,
    shipSub:'釧路産棹前昆布専用・PDF/FAX',
    stockSub:'原票形式で集計・PDF出力',
    moreSub:'バックアップ・商品選択'
  });
};
