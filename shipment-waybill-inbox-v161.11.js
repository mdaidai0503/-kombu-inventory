/* =========================================================
   昆布在庫管理
   送り状PDF連携 v161.11（ignoredを送り状確認から完全除外）
...
(function () {
  ...
  function classifyWaybill(waybill) {
    ...
    if (waybill) {
      // v161.11:
      // ignored はスコア補完へ流さず、明示的に非表示扱いにする。
      // 以前は ignored でも score=0/5 が残っていると旧データ互換判定で
      // unmatched に戻り、送り状確認へ再表示されていた。
  ...
  // v161.4:
  // 送り状日より前の日付の出荷依頼PDFは候補から除外する。
  //
  // 例:
  // 送り状出荷日  = 2026-08-06
  // 出荷指示PDF名 = 2026.08.04_㈱和気食品_出荷指示 → 除外
  // 出荷指示PDF名 = 2026.08.06_㈱和気食品_出荷指示 → 候補可
  // 出荷指示PDF名 = 2026.08.07_㈱和気食品_出荷指示 → 候補可
  ...
  function openReviewModal() {
    ...
    sorted.forEach(function (w) {
      ...
      // v161.11:
      // ignored は「要確認」「不一致」「自動添付済み」「未判定」の
      // どの一覧にも表示しない。
      if (info.key === 'ignored') {
        return;
      }
