...
function v55CanvasSanmae(s,y){
  ...
  try{
    ...
    smState.shipments=[];
    window.__v58ShipmentCoopLower=true;window.__v63ShipmentHeaderLarge=true;window.__v132SanmaeShipmentTall=true;window.__v132SanmaeShipmentMerged=true;try{return v55RetitleStockCanvas(smReportCanvas(y,null),'釧路産棹前昆布　出 荷 依 頼 書',y,s,70,34);}finally{window.__v58ShipmentCoopLower=false;window.__v63ShipmentHeaderLarge=false;window.__v132SanmaeShipmentTall=false;window.__v132SanmaeShipmentMerged=false;}
...
async function v130TopBackup(){
  try{
    ...
    const name='昆布在庫出荷管理_'+v132BackupTimestamp()+'.json';
...
/* ===== v132: 自動保存 + 入力途中下書き復元 + 日時入りバックアップ名 ===== */
(function(){
  const SNAP_KEY='kombu-v132-autosave-snapshot';
  const DRAFT_KEY='kombu-v132-form-drafts';
  let saveTimer=0;

  window.v132BackupTimestamp=function(){
  ...
  function safeAllSave(){
    ...
    }catch(e){console.warn('v132 autosave snapshot',e)}
  }
  ...
  if(typeof oldLanding==='function')globalThis.productLanding=function(){
    const r=oldLanding.apply(this,arguments);
    const ver=app.querySelector('.v106-version,.pill');if(ver)ver.textContent='v132';
    return r;
  };
})();
/* ===== /v132 ===== */
